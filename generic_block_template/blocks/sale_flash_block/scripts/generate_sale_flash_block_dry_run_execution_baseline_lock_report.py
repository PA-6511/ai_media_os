...
POLICY_JSON = CONFIG_DIR / "dry_run_execution_baseline_lock_policy.json"
...
def generate_dry_run_execution_baseline_lock_report(policy_json: Path = POLICY_JSON) -> Dict[str, Any]:
    ...
    final_human_gate_status = str(
        (payloads.get("sfb6_wordpress_payload_validation") or {})
        .get("final_human_gate", {})
    ...
    execution_gate_status = str(
        (payloads.get("sfb7_final_human_approval") or {}).get("wordpress_dry_run_execution_gate", "BLOCKED")
    )
    ...
    simulated_execution_count = int(
        (payloads.get("sfb8_wordpress_dry_run_execution_evidence") or {}).get("simulated_execution_count", 0)
    )
    ...
    human_approval_unconsumed = (
        ...
        and (payloads.get("sfb7_final_human_approval") or {}).get("human_approval_consumed", False) is False
        and (payloads.get("sfb8_wordpress_dry_run_execution_evidence") or {}).get("human_approval_consumed", False) is False
    )
    ...
    output_json = BLOCK_DIR / str(policy.get("output_json", "logs/sale_flash_block_dry_run_execution_baseline_lock_report.json"))
    output_md = BLOCK_DIR / str(policy.get("output_markdown", "logs/sale_flash_block_dry_run_execution_baseline_lock_report.md"))
    ...
    result: Dict[str, Any] = {
        ...
        "baseline_name": str(policy.get("baseline_name", "sale_flash_block_sfb1_sfb8_dry_run_execution_locked")),
...
def main() -> int:
    payload = generate_dry_run_execution_baseline_lock_report()
