...
def evaluate_phase55_policy(
    ...
    if controls.get("final_readiness_controls_does_not_execute") is not True:
        reasons.append("controls.final_readiness_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("final_readiness_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.final_readiness_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("final_readiness_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.final_readiness_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("final_readiness_gate_does_not_execute") is not True:
        reasons.append("manual_gate.final_readiness_gate_does_not_execute must be True")
