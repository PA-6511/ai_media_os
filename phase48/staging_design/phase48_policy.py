...
def evaluate_phase48_policy(
    ...
    if controls.get("staging_controls_does_not_execute") is not True:
        reasons.append("controls.staging_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("staging_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.staging_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("staging_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.staging_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("staging_gate_does_not_execute") is not True:
        reasons.append("manual_gate.staging_gate_does_not_execute must be True")
