...
from scripts.validate_start_ls6ak_manual_publish_actual_publish_execution_runner_final_boundary import main
...
def make_inputs(tmp_path: Path) -> dict[str, Path]:
    ...
    write_json(files["policy"], {"phase": "LS-6AK", "execution_mode": "ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_ONLY_NO_PUBLISH"})
    ...
    run = {
        ...
        "ls6aj_execute_now_gate_validated": True,
    ...
    runtime = dict(run)
    runtime["status"] = "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_READY_NO_PUBLISH"
    runtime["document_type"] = "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_RESULT"
    ...
    write_json(files["ls6aj_execute"], {"execute_now_status": "ACTUAL_PUBLISH_EXECUTION_RUNNER_EXECUTE_NOW_RECORDED_NO_PUBLISH_EXECUTION"})
    ...
    write_json(files["ls6ai_final"], {"actual_publish_execution_runner_final_preflight_consumed": False})
    ...
    write_json(files["ls6af_runtime"], {"actual_publish_runner_final_gate_consumed": False})
...
def build_argv(files: dict[str, Path]) -> list[str]:
    return [
        ...
        "--final-boundary-result", str(files["runtime"]),
        "--final-boundary-lock", str(files["lock"]),
...
def test_38_validator_valid_result(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AK_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_VALIDATED_NO_PUBLISH"
...
def test_39_to_69_detect_true_flags(monkeypatch, tmp_path: Path, field: str) -> None:
    files = make_inputs(tmp_path)
    mutate(files["run"], field, True)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AK_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_NOT_READY"


def test_70_detect_next_phase_mismatch(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
...
def test_71_detect_requires_separated_phase_false(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    mutate(files["run"], "requires_separated_actual_publish_execution_runner_phase", False)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AK_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_NOT_READY"


def test_72_detect_publish_execution_still_blocked_false(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    mutate(files["run"], "publish_execution_still_blocked", False)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AK_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_NOT_READY"
