...
from scripts.validate_wordpress_draft_phase8_5_manual_publish_rehearsal import run_validation

ROOT = Path(__file__).resolve().parents[1]
VALID_CONFIG = ROOT / "config/wordpress_draft_phase8_5_manual_publish_rehearsal.json"
...
def run_case(payload, p84=None, p83=None, p82=None, p76=None):
    with tempfile.TemporaryDirectory() as td:
        ...
        import scripts.validate_wordpress_draft_phase8_5_manual_publish_rehearsal as mod
...
def test_valid_config_passes():
    ...
    assert saved["rehearsal_design_ready"] is True
...
def test_phase84_not_pass_aborts():
    p84 = _base_p84()
    p84["status"] = "ABORT"
    result, _ = run_case(_load_valid(), p84=p84)
...
def test_rehearsal_steps_short_aborts():
    payload = _load_valid()
    payload["rehearsal_steps"] = payload["rehearsal_steps"][:3]
    result, _ = run_case(payload)
    assert result["status"] == "ABORT"
