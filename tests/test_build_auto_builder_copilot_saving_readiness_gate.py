...
POLICY_PATH = ROOT / "config/auto_builder_copilot_saving_readiness_gate_policy.json"
SCRIPT_PATH = ROOT / "scripts/build_auto_builder_copilot_saving_readiness_gate.py"
...
def test_ab_t2_to_t8_result_files_exist() -> None:
    assert AB_T2.exists()
    assert AB_T3.exists()
    assert AB_T4.exists()
    assert AB_T5.exists()
...
def test_verified_phase_count_7(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["verified_phase_count"] == 7
...
def test_readiness_gate_verified_true(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["readiness_gate_verified"] is True
...
def test_verification_summary_generated_true(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["verification_summary_generated"] is True
...
def test_next_phase_phase_ab_t10(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["next_phase"]["phase"] == "AB-T10"
