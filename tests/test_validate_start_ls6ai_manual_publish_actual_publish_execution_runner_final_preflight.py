...
from scripts.validate_start_ls6ai_manual_publish_actual_publish_execution_runner_final_preflight import main
...
def make_inputs(tmp_path: Path) -> dict[str, Path]:
    ...
    write_json(files["policy"], {"phase": "LS-6AI"})
    ...
    write_json(files["wp"], wp)

    runtime = dict(run)
    runtime["status"] = "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_PREFLIGHT_PASSED_NO_PUBLISH"
    runtime["document_type"] = "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_PREFLIGHT_RESULT"
    runtime["current_post_status_verified"] = True
    write_json(files["runtime"], runtime)

    write_json(files["lock"], {"status": "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_PREFLIGHT_LOCKED_NO_PUBLISH", "locked": True, "rerun_allowed": False, "ls6oc1_rerun_executed": False, "requires_next_phase": "LS-6AJ"})

    write_json(files["ls6ah_run"], {"status": "LS6AH_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_PASSED_NO_PUBLISH"})
    ...
    write_json(files["ls6ah_boundary"], {"actual_publish_execution_runner_boundary_consumed": False})
    write_json(files["ls6ah_lock"], {"actual_publish_execution_runner_boundary_consumed": False})
    write_json(files["ls6ag_ready"], {"actual_publish_final_execution_command_consumed": False})
...
def build_argv(files: dict[str, Path]) -> list[str]:
    return [
        ...
        "--actual-publish-execution-runner-final-preflight-result", str(files["runtime"]),
        "--actual-publish-execution-runner-final-preflight-lock", str(files["lock"]),
...
def test_30_validator_valid_result(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AI_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_PREFLIGHT_VALIDATED_NO_PUBLISH"
...
def test_31_to_55_detect_true_flags(monkeypatch, tmp_path: Path, field: str) -> None:
    files = make_inputs(tmp_path)
...
def test_56_detect_returned_post_status_not_draft(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    run = read_json(files["run"])
    run["returned_post_status"] = "publish"
    write_json(files["run"], run)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AI_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_PREFLIGHT_NOT_READY"


def test_57_detect_next_phase_mismatch(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    run = read_json(files["run"])
...
def test_58_detect_requires_execute_now_false(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    run = read_json(files["run"])
    run["requires_actual_publish_execution_runner_execute_now"] = False
    write_json(files["run"], run)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AI_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_PREFLIGHT_NOT_READY"


def test_59_detect_publish_execution_still_blocked_false(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    run = read_json(files["run"])
