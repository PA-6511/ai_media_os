...
from scripts.validate_wordpress_draft_phase8_3_publish_gate_design import run_validation

ROOT = Path(__file__).resolve().parents[1]
VALID_CONFIG = ROOT / "config/wordpress_draft_phase8_3_publish_gate_design.json"
...
def run_case(payload, p81=None, p82=None, p76=None):
    with tempfile.TemporaryDirectory() as td:
        ...
        import scripts.validate_wordpress_draft_phase8_3_publish_gate_design as mod
...
def test_valid_config_passes():
    ...
    assert saved["gate_design_ready"] is True
...
def test_gate_requirements_short_aborts():
    payload = _load_valid()
    payload["gate_requirements"] = payload["gate_requirements"][:3]
    result, _ = run_case(payload)
    assert result["status"] == "ABORT"
...
def test_target_draft_id_mismatch_aborts():
    payload = _load_valid()
    payload["target_draft"]["wordpress_draft_id"] = 999
    result, _ = run_case(payload)
    assert result["status"] == "ABORT"
