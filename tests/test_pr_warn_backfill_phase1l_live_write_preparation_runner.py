...
def _load_module():
    ...
    script_path = root / "scripts/pr_warn_backfill_phase1l_live_write_preparation_runner.py"
    spec = importlib.util.spec_from_file_location("phase1l_live_write_preparation_runner", script_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("failed to load phase1l live write preparation runner module")
...
def test_pass_dry_run_with_write_blocked(tmp_path: Path):
    result = _run(tmp_path)
    assert result["status"] == "PASS_LIVE_WRITE_PREPARATION_DRY_RUN_ONLY"
    assert result["mode"] == "DRY_RUN_WITH_WRITE_BLOCKED"
    assert result["target_post_id"] == 101
    assert result["payload_preview_created"] is True
    assert result["payload_changed_fields"] == ["content"]
    assert result["payload_forbidden_fields_present"] is False
...
def test_abort_when_approval_not_approved(tmp_path: Path):
...
def test_abort_when_approval_target_mismatch(tmp_path: Path):
    result = _run(tmp_path, approval=_base_approval(target_post_id=202))
    assert result["status"] == "ABORT_APPROVAL_VALIDATION_FAILED"
...
def test_abort_when_example_approved_true(tmp_path: Path):
...
def test_abort_when_example_write_allowed_true(tmp_path: Path):
    result = _run(
        tmp_path,
...
def test_abort_when_snapshot_post_mismatch(tmp_path: Path):
...
def test_abort_when_snapshot_result_not_pass(tmp_path: Path):
    result = _run(tmp_path, snapshot_result=_base_snapshot_result(status="FAILED"))
    assert result["status"] == "ABORT_SNAPSHOT_RESULT_VALIDATION_FAILED"


def test_abort_when_phase1f_changed_fields_invalid(tmp_path: Path):
...
def test_abort_when_payload_contains_forbidden_fields(tmp_path: Path, monkeypatch):
    module = _load_module()

    def _bad_payload(post_id, snapshot_content):
        return {
            "post_id": post_id,
    ...
    monkeypatch.setattr(module, "_build_payload_preview", _bad_payload)
    ...
    _write_json(approval_path, _base_approval())
    _write_json(final_live_approval_path, _base_final_live_approval_example())
    _write_json(snapshot_path, _base_snapshot())
    _write_json(snapshot_result_path, _base_snapshot_result())
    _write_json(phase1f_dry_run_path, _base_phase1f_dry_run())
    ...
    assert result["status"] == "ABORT_PAYLOAD_FORBIDDEN_FIELDS_PRESENT"


def test_source_has_no_requests_write_calls():
    root = Path(__file__).resolve().parents[1]
    source = (root / "scripts/pr_warn_backfill_phase1l_live_write_preparation_runner.py").read_text(encoding="utf-8")
