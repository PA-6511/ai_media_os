...
from scripts.validate_phase8_svc_4_activation_preflight_design import (  # noqa: E402
    validate_activation_preflight_design,
)
...
def _good_runbook() -> str:
    return """# Phase 8-SVC-4: Activation Preflight Design / No Activation

## 1. Purpose
Design only.

## 4. Preflight Gate Checklist (Future SVC-5+)
- SVC-3 must pass first.
...
def test_pass(tmp_path: Path):
    ...
    result = validate_activation_preflight_design(policy, runbook, output)
    assert result["status"] == "PASS_DESIGN_ONLY_NO_ACTIVATION"
...
def test_fail_policy_violation(tmp_path: Path):
    policy = _base_policy()
    policy["activation_actions"]["systemctl_start_executed"] = True
    ...
    result = validate_activation_preflight_design(policy_path, runbook, output)
    assert result["status"] == "FAIL"
    assert any("systemctl_start_executed" in item for item in result["policy_violations"])
...
def test_abort_forbidden_executable_line(tmp_path: Path):
    ...
    runbook.parent.mkdir(parents=True, exist_ok=True)
    runbook.write_text(_good_runbook() + "\nsystemctl enable ai-media-os-credential-ready-validator.timer\n", encoding="utf-8")
