...
from scripts.validate_start_ls6al_manual_publish_actual_publish_execution_runner_separated_phase_gate import main
...
def make_inputs(tmp_path: Path) -> dict[str, Path]:
    files = {
        ...
        "ls6ak_run": tmp_path / "exchange/logs/ls6ak_run.json",
    ...
    run = {
        "status": "LS6AK_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_PASSED_NO_PUBLISH",
        "post_id": 183,
    ...
    write_json(files["ls6ak_run"], run)
    ...
    template_doc = {
        "phase": "LS-6AL",
        "document_type": "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_SEPARATED_PHASE_GATE_TEMPLATE",
        ...
        "separated_phase_gate": {
            "separated_actual_publish_execution_runner_phase_gate_label": "",
            "required_separated_actual_publish_execution_runner_phase_gate_label": "SEPARATED_ACTUAL_PUBLISH_EXECUTION_RUNNER_PHASE_GATE_ONLY",
            "gate_reason": "",
    ...
    gate_doc["document_type"] = "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_SEPARATED_PHASE_GATE"
    gate_doc["gate_status"] = "SEPARATED_ACTUAL_PUBLISH_EXECUTION_RUNNER_PHASE_GATE_RECORDED_NO_PUBLISH_EXECUTION"
    gate_doc["separated_phase_gate"]["separated_actual_publish_execution_runner_phase_gate_label"] = "SEPARATED_ACTUAL_PUBLISH_EXECUTION_RUNNER_PHASE_GATE_ONLY"
    gate_doc["separated_phase_gate"]["gate_reason"] = "LS-6AK actual publish execution runner final boundary was validated."
    gate_doc["separated_phase_gate"]["requires_next_phase"] = "LS-6AM"
...
def build_argv(files: dict[str, Path], allow_template: bool = False) -> list[str]:
    argv = [
        ...
        "--separated-phase-gate", str(files["gate"]),
        "--ls6ak-run-result", str(files["ls6ak_run"]),
...
def test_01_template_allow_template_ready(monkeypatch, tmp_path: Path) -> None:
    ...
    assert out["status"] == "LS6AL_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_SEPARATED_PHASE_GATE_TEMPLATE_READY_NO_PUBLISH"


def test_02_valid_separated_phase_gate_ready(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AL_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_SEPARATED_PHASE_GATE_READY_NO_PUBLISH"


def test_03_separated_phase_gate_missing_not_ready(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    files["gate"].unlink()
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AL_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_SEPARATED_PHASE_GATE_NOT_READY"


@pytest.mark.parametrize(
    "keys,value",
    [
        (("separated_phase_gate", "separated_actual_publish_execution_runner_phase_gate_label"), "WRONG_LABEL"),
        (("separated_phase_gate", "separated_actual_publish_execution_runner_phase_gate_consumed"), True),
        (("separated_phase_gate", "actual_publish_execution_runner_final_boundary_ready"), False),
        (("separated_phase_gate", "actual_publish_execution_runner_final_boundary_consumed"), True),
        (("separated_phase_gate", "actual_publish_execution_runner_execute_now_recorded"), False),
        (("separated_phase_gate", "actual_publish_execution_runner_execute_now_consumed"), True),
        (("separated_phase_gate", "actual_publish_execution_runner_final_preflight_ready"), False),
        (("separated_phase_gate", "actual_publish_execution_runner_final_preflight_consumed"), True),
        (("separated_phase_gate", "actual_publish_execution_runner_boundary_ready"), False),
        (("separated_phase_gate", "actual_publish_execution_runner_boundary_consumed"), True),
        (("separated_phase_gate", "actual_publish_final_execution_command_recorded"), False),
        (("separated_phase_gate", "actual_publish_final_execution_command_consumed"), True),
        (("separated_phase_gate", "actual_publish_runner_final_gate_ready"), False),
        (("separated_phase_gate", "actual_publish_runner_final_gate_consumed"), True),
        (("separated_phase_gate", "actual_publish_final_preflight_ready"), False),
        (("separated_phase_gate", "actual_publish_final_preflight_consumed"), True),
        (("separated_phase_gate", "actual_publish_execution_boundary_ready"), False),
        (("separated_phase_gate", "actual_publish_execution_boundary_consumed"), True),
        (("separated_phase_gate", "actual_publish_execute_now_final_confirmation_consumed"), True),
        (("separated_phase_gate", "explicit_execute_now_for_actual_publish_required"), False),
        (("separated_phase_gate", "explicit_execute_now_for_actual_publish_received"), False),
        (("separated_phase_gate", "explicit_execute_now_for_actual_publish_consumed"), True),
        (("separated_phase_gate", "actual_publish_execution_runner_ready"), False),
        (("separated_phase_gate", "actual_publish_execution_runner_executed"), True),
        (("separated_phase_gate", "actual_publish_execution_runner_blocked"), False),
        (("separated_phase_gate", "actual_publish_execution_allowed_by_this_phase"), True),
        (("separated_phase_gate", "actual_runner_execution_allowed_by_this_phase"), True),
        (("separated_phase_gate", "manual_publish_allowed_by_this_phase"), True),
        (("separated_phase_gate", "manual_publish_execution_allowed_by_this_phase"), True),
        (("separated_phase_gate", "manual_publish_executed"), True),
    ],
)
def test_04_to_33_gate_field_violations(monkeypatch, tmp_path: Path, keys: tuple[str, ...], value) -> None:
    files = make_inputs(tmp_path)
...
def test_34_to_40_upstream_consumed_true(monkeypatch, tmp_path: Path, keys: tuple[str, ...]) -> None:
    files = make_inputs(tmp_path)
...
def test_41_to_49_current_phase_execution_true(monkeypatch, tmp_path: Path, field: str) -> None:
    files = make_inputs(tmp_path)
...
def test_50_post_id_mismatch(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
...
def test_51_expected_current_status_not_draft(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
...
@pytest.mark.parametrize(
    "path_key,keys,value",
    [
        ("ls6ak_validation", ("status",), "WRONG_STATUS"),
...
def test_52_to_70_upstream_state_violations(monkeypatch, tmp_path: Path, path_key: str, keys: tuple[str, ...], value) -> None:
    files = make_inputs(tmp_path)
...
def test_71_to_79_result_false_flags(monkeypatch, tmp_path: Path, field: str, expected: bool) -> None:
    files = make_inputs(tmp_path)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AL_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_SEPARATED_PHASE_GATE_READY_NO_PUBLISH"
...
def test_80_to_84_result_handoff_and_requirements(monkeypatch, tmp_path: Path, path: tuple[str, ...], value) -> None:
    files = make_inputs(tmp_path)
    out = invoke(monkeypatch, files)
    cur = out
    for key in path:
