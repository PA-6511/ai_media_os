...
from scripts.validate_phase8_svc_6_daemon_reload_post_check_design import (  # noqa: E402
    validate_daemon_reload_post_check_design,
)
...
def _good_runbook() -> str:
    return """# Phase 8-SVC-6: Daemon-Reload Post-Check Design

## 1. Purpose
Design only.

## 2. Preconditions
SVC-3 and SVC-5 should pass.
...
def test_pass(tmp_path: Path):
    ...
    result = validate_daemon_reload_post_check_design(policy, runbook, output)
    assert result["status"] == "PASS_DESIGN_ONLY_DAEMON_RELOAD_POST_CHECK"
...
def test_fail_policy_violation(tmp_path: Path):
    ...
    result = validate_daemon_reload_post_check_design(policy_path, runbook, output)
    assert result["status"] == "FAIL"
