...
POLICY_PATH = ROOT / "config/auto_builder_copilot_saving_integration_gate_policy.json"
SCRIPT_PATH = ROOT / "scripts/build_auto_builder_copilot_saving_integration_gate.py"
...
def test_ab_t2_to_t6_result_files_exist() -> None:
    assert AB_T2.exists()
    assert AB_T3.exists()
    assert AB_T4.exists()
    assert AB_T5.exists()
...
def test_verified_phase_count_5(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["verified_phase_count"] == 5


def test_pipeline_order_verified_true(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["pipeline_order_verified"] is True
...
def test_integration_summary_generated_true(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["integration_summary_generated"] is True
...
def test_token_saving_pipeline_summary_generated_true(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["token_saving_pipeline_summary_generated"] is True
...
def test_next_phase_phase_ab_t8(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["next_phase"]["phase"] == "AB-T8"
