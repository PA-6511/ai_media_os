...
POLICY_PATH = ROOT / "config/auto_builder_copilot_saving_baseline_fixed_policy.json"
SCRIPT_PATH = ROOT / "scripts/build_auto_builder_copilot_saving_baseline_fixed.py"
...
AB_T8 = ROOT / "exchange/logs/ab_t8_copilot_saving_dry_run_result.json"
AB_T9 = ROOT / "exchange/logs/ab_t9_copilot_saving_readiness_gate_result.json"
...
def test_ab_t2_to_t9_result_files_exist() -> None:
    assert AB_T2.exists()
    assert AB_T3.exists()
    assert AB_T4.exists()
    assert AB_T5.exists()
...
def test_verified_phase_count_8(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["verified_phase_count"] == 8


def test_baseline_fixed_true(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["baseline_fixed"] is True


def test_baseline_summary_generated_true(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["baseline_summary_generated"] is True


def test_pipeline_status_table_generated_true(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["pipeline_status_table_generated"] is True
...
def test_baseline_readiness_generated_true(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["baseline_readiness_generated"] is True
...
def test_next_phase_phase_ab_t11(tmp_path: Path) -> None:
    _, out, _ = run_script(tmp_path)
    result = json.loads(out.read_text(encoding="utf-8"))
    assert result["next_phase"]["phase"] == "AB-T11"
