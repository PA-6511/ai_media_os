...
SCRIPT = Path("scripts/validate_start_ls6x_manual_publish_separated_actual_publish_execution_gate.py")
...
def make_files(tmp_path: Path) -> dict[str, Path]:
    files = {
        ...
        "ls6w_run": tmp_path / "exchange/logs/ls6w_run.json",
    ...
    write_json(
        files["policy"],
        {
            ...
            "next_phase": {
                ...
                "requires_actual_publish_runner_boundary": True,
    ...
    write_json(
        ...
        {
            "phase": "LS-6X",
            "document_type": "MANUAL_PUBLISH_SEPARATED_ACTUAL_PUBLISH_EXECUTION_GATE_TEMPLATE",
            "gate_status": "TEMPLATE_NOT_APPROVED",
            "target_post": {
                "post_id": 183,
            ...
            "actual_publish_execution_gate": {
                ...
                "required_actual_publish_execution_gate_label": "APPROVED_FOR_SEPARATED_ACTUAL_PUBLISH_EXECUTION_GATE_ONLY",
    ...
    write_json(
        ...
        {
            "phase": "LS-6X",
            "document_type": "MANUAL_PUBLISH_SEPARATED_ACTUAL_PUBLISH_EXECUTION_GATE",
    ...
    write_json(
        files["ls6w_validation"],
        {
            ...
            "publish_execution_still_blocked": True,
            "next_phase": {"phase": "LS-6X"},
        },
    )
    write_json(files["ls6w_run"], {"status": "LS6W_MANUAL_PUBLISH_ACTUAL_EXECUTION_FINAL_RUNNER_PREFLIGHT_PASSED_NO_PUBLISH"})
...
def run_validator(files: dict[str, Path], *, allow_template: bool = False) -> None:
    cmd = [
        ...
        "--ls6w-run-result", str(files["ls6w_run"]),
...
def assert_not_ready(files: dict[str, Path], *, allow_template: bool = False) -> None:
    ...
    assert read_json(files["output"])["status"] == "LS6X_MANUAL_PUBLISH_SEPARATED_ACTUAL_PUBLISH_EXECUTION_GATE_NOT_READY"


def test_01_template_allow_template_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f, allow_template=True)
    assert read_json(f["output"])["status"] == "LS6X_MANUAL_PUBLISH_SEPARATED_ACTUAL_PUBLISH_EXECUTION_GATE_TEMPLATE_READY_NO_PUBLISH"


def test_02_valid_gate_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["status"] == "LS6X_MANUAL_PUBLISH_SEPARATED_ACTUAL_PUBLISH_EXECUTION_GATE_READY_NO_PUBLISH"


def test_03_gate_file_missing_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    f["gate"].unlink()
    assert_not_ready(f)


def test_04_wrong_actual_publish_execution_gate_label_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
    p["actual_publish_execution_gate"]["actual_publish_execution_gate_label"] = "WRONG"
    write_json(f["gate"], p)
    assert_not_ready(f)


def test_05_actual_publish_execution_gate_consumed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
    p["actual_publish_execution_gate"]["actual_publish_execution_gate_consumed"] = True
    write_json(f["gate"], p)
    assert_not_ready(f)


def test_06_actual_publish_execution_allowed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
    p["actual_publish_execution_gate"]["actual_publish_execution_allowed_by_this_phase"] = True
    write_json(f["gate"], p)
    assert_not_ready(f)


def test_07_actual_runner_execution_allowed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
    p["actual_publish_execution_gate"]["actual_runner_execution_allowed_by_this_phase"] = True
    write_json(f["gate"], p)
    assert_not_ready(f)


def test_08_manual_publish_allowed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
    p["actual_publish_execution_gate"]["manual_publish_allowed_by_this_phase"] = True
    write_json(f["gate"], p)
    assert_not_ready(f)


def test_09_manual_publish_execution_allowed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
    p["actual_publish_execution_gate"]["manual_publish_execution_allowed_by_this_phase"] = True
    write_json(f["gate"], p)
    assert_not_ready(f)


def test_10_manual_publish_executed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
    p["actual_publish_execution_gate"]["manual_publish_executed"] = True
    write_json(f["gate"], p)
    assert_not_ready(f)


def test_11_current_phase_wordpress_api_call_executed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_12_current_phase_wordpress_get_executed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_13_current_phase_wordpress_post_executed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_14_current_phase_wordpress_write_executed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_15_current_phase_wordpress_publish_executed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_16_current_phase_publish_executed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_17_current_phase_credential_env_read_executed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_18_current_phase_credential_value_output_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_19_current_phase_authorization_header_output_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_20_final_execution_command_consumed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
    p["current_phase_execution"]["final_execution_command_consumed"] = True
    write_json(f["gate"], p)
    assert_not_ready(f)


def test_21_approval_label_consumed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_22_execute_now_confirmation_consumed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_23_post_id_mismatch_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_24_expected_current_status_not_draft_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["gate"])
...
def test_25_ls6w_validation_status_mismatch_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
...
def test_26_ls6w_run_status_mismatch_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["ls6w_run"])
    p["status"] = "WRONG"
    write_json(f["ls6w_run"], p)
    assert_not_ready(f)


def test_27_ls6w_final_runner_preflight_status_mismatch_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["ls6w_preflight"])
    p["status"] = "WRONG"
    write_json(f["ls6w_preflight"], p)
    assert_not_ready(f)


def test_28_ls6w_final_runner_preflight_lock_status_mismatch_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["ls6w_lock"])
    p["status"] = "WRONG"
    write_json(f["ls6w_lock"], p)
    assert_not_ready(f)


def test_29_ls6w_returned_post_status_not_draft_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["ls6w_validation"])
...
def test_30_ls6w_final_execution_command_consumed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["ls6w_validation"])
...
def test_31_ls6w_publish_execution_still_blocked_false_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["ls6w_validation"])
...
def test_32_ls6w_next_phase_not_ls6x_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    p = read_json(f["ls6w_validation"])
    p["next_phase"] = {"phase": "LS-6Y"}
    write_json(f["ls6w_validation"], p)
    assert_not_ready(f)


def test_33_ls6v_ready_status_mismatch_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
...
def test_34_ls6t_execute_now_confirmation_consumed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
...
def test_35_ls6r_approval_label_consumed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
...
def test_36_ls6p_rerun_allowed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
...
def test_37_ls6oc1_consumption_rerun_allowed_true_not_ready(tmp_path: Path) -> None:
    f = make_files(tmp_path)
...
def test_38_result_keeps_actual_publish_execution_gate_consumed_false(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["actual_publish_execution_gate_consumed"] is False


def test_39_result_keeps_final_execution_command_consumed_false(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["final_execution_command_consumed"] is False


def test_40_result_keeps_approval_label_consumed_false(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["approval_label_consumed"] is False


def test_41_result_keeps_execute_now_confirmation_consumed_false(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["execute_now_confirmation_consumed"] is False


def test_42_result_keeps_actual_publish_execution_allowed_false(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["actual_publish_execution_allowed_by_this_phase"] is False


def test_43_result_keeps_actual_runner_execution_allowed_false(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["actual_runner_execution_allowed_by_this_phase"] is False


def test_44_result_keeps_manual_publish_executed_false(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["manual_publish_executed"] is False


def test_45_result_keeps_wordpress_api_call_executed_false(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["wordpress_api_call_executed"] is False


def test_46_result_keeps_credential_env_read_executed_false(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["credential_env_read_executed"] is False


def test_47_result_next_phase_ls6y(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["next_phase"]["phase"] == "LS-6Y"


def test_48_result_requires_actual_publish_runner_boundary_true(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["next_phase"]["requires_actual_publish_runner_boundary"] is True


def test_49_result_requires_final_explicit_publish_execution_command_true(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
    assert read_json(f["output"])["next_phase"]["requires_final_explicit_publish_execution_command"] is True


def test_50_result_publish_execution_still_blocked_true(tmp_path: Path) -> None:
    f = make_files(tmp_path)
    run_validator(f)
