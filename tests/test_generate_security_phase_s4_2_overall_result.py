...
from scripts.generate_security_phase_s4_2_overall_result import (  # noqa: E402
    generate_security_phase_s4_2_overall_result,
)
...
def _build_replay(tmp_path: Path, replay_result: str, mismatched: int = 0) -> Path:
    ...
    if mismatched > 0:
        result["matched_expected_count"] = max(0, int(result.get("scenario_count", 0)) - mismatched)
...
def test_overall_pass_dry_run_only(tmp_path: Path) -> None:
    ...
    assert result["next_step"] == "phase_s4_3_isolation_audit_or_design_review"
...
def test_overall_review_required_on_mismatch(tmp_path: Path) -> None:
    ...
    assert result["final_status"] == "ISOLATION_SIMULATION_REVIEW_REQUIRED"
