...
from scripts.validate_start_ls6ad_manual_publish_actual_publish_execution_boundary import main
...
def make_inputs(tmp_path: Path) -> dict[str, Path]:
    files = {
        ...
        "boundary": tmp_path / "exchange/runtime/boundary.json",
    ...
    write_json(files["policy"], {"phase": "LS-6AD"})
    ...
    write_json(
        files["run"],
        {
            "status": "LS6AD_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_BOUNDARY_PASSED_NO_PUBLISH",
    ...
    write_json(
        files["boundary"],
        {
            ...
            "status": "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_BOUNDARY_READY_NO_PUBLISH",
            **{k: v for k, v in base.items() if k not in {"locked", "rerun_allowed", "ls6oc1_rerun_executed"}},
        },
    )
    ...
    write_json(files["ls6ac_ready"], {"actual_publish_execute_now_final_confirmation_consumed": False, "explicit_execute_now_for_actual_publish_consumed": False, "actual_publish_execution_runner_executed": False, "manual_publish_executed": False})
    ...
    write_json(files["ls6ab_blocked"], {"actual_publish_execution_runner_executed": False, "explicit_execute_now_for_actual_publish_consumed": False})
...
def build_argv(files: dict[str, Path]) -> list[str]:
    return [
        ...
        "--boundary-result", str(files["boundary"]),
        "--boundary-lock", str(files["lock"]),
...
def test_31_validator_valid_result(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AD_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_BOUNDARY_VALIDATED_NO_PUBLISH"
...
def test_32_to_54_validator_detects_true_flags(monkeypatch, tmp_path: Path, field: str) -> None:
    files = make_inputs(tmp_path)
...
def test_55_validator_detects_rerun_allowed_true(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    lock = read_json(files["lock"])
...
def test_56_validator_detects_next_phase_mismatch(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    run = read_json(files["run"])
...
def test_57_validator_detects_requires_actual_publish_final_preflight_false(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    run = read_json(files["run"])
    run["requires_actual_publish_final_preflight"] = False
    write_json(files["run"], run)
    out = invoke(monkeypatch, files)
    assert out["status"] == "LS6AD_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_BOUNDARY_NOT_READY"


def test_58_validator_detects_publish_execution_still_blocked_false(monkeypatch, tmp_path: Path) -> None:
    files = make_inputs(tmp_path)
    run = read_json(files["run"])
