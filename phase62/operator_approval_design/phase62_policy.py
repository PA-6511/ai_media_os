...
def evaluate_phase62_policy(
    ...
    if controls.get("operator_approval_controls_does_not_execute") is not True:
        reasons.append("controls.operator_approval_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("operator_approval_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.operator_approval_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("operator_approval_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.operator_approval_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("operator_approval_gate_does_not_execute") is not True:
        reasons.append("manual_gate.operator_approval_gate_does_not_execute must be True")
