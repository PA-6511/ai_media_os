from phase33.approval_design.phase33_policy import evaluate_phase33_policy


def _base_approval_package() -> dict:
    return {
        "mode": "DRY_RUN",
...
def test_policy_fails_when_max_files_invalid() -> None:
    package = _base_approval_package()
    package["max_files_to_execute"] = 2
    result = evaluate_phase33_policy(
        package,
        _base_controls(),
...
def test_policy_fails_when_controls_non_execute_false() -> None:
    ...
    controls["final_approval_controls_does_not_execute"] = False
    result = evaluate_phase33_policy(
        _base_approval_package(),
        controls,
        _base_stop_conditions(),
        _base_evidence(),
        _base_gate(),
...
def test_policy_fails_when_stop_non_execute_false() -> None:
    ...
    stop_conditions["final_approval_stop_conditions_does_not_execute"] = False
    result = evaluate_phase33_policy(
        _base_approval_package(),
        _base_controls(),
        stop_conditions,
...
def test_policy_fails_when_evidence_non_execute_false() -> None:
    ...
    evidence["final_approval_evidence_does_not_execute"] = False
    result = evaluate_phase33_policy(
        _base_approval_package(),
        _base_controls(),
        _base_stop_conditions(),
