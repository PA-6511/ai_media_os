...
def evaluate_phase44_policy(
    ...
    if controls.get("closure_controls_does_not_execute") is not True:
        reasons.append("controls.closure_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("closure_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.closure_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("closure_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.closure_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("closure_gate_does_not_execute") is not True:
        reasons.append("manual_gate.closure_gate_does_not_execute must be True")
