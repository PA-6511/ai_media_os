...
def evaluate_phase58_policy(
    ...
    if controls.get("execution_plan_controls_does_not_execute") is not True:
        reasons.append("controls.execution_plan_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("execution_plan_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.execution_plan_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("execution_plan_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.execution_plan_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("execution_plan_gate_does_not_execute") is not True:
        reasons.append("manual_gate.execution_plan_gate_does_not_execute must be True")
