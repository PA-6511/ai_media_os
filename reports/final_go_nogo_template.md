...
- [x] バックアップ方針を確定
  ...
  - 頻度: 毎日1回（推奨 03:00 JST）
- [x] 復元手順の文書化と、復元確認履歴テンプレートの整備を完了
...
- 判定日: 2026-04-12
- 理由:
  - 運用・監視・切り戻し・バックアップ方針の文書化は完了
  - ただし、権限最小化の実画面確認と本運用後の実復元記録が未完了

## 5. Go に上げるための最終条件
次の 1 から 4 をすべて満たした時点で Go に引き上げる。
1. 権限最小化の実画面確認が完了し、確認結果を文書化する
2. 本運用後の実復元確認を 1 回以上実施し、結果と証跡を runbook に記録する
3. 初回運用日チェック項目（py_compile / pytest / queue / logs / rollback再確認）を記録する
4. 判定者が最終レビューを行い、判定欄と判定メモを確定する
