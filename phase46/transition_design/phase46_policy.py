...
def evaluate_phase46_policy(
    ...
    if controls.get("transition_controls_does_not_execute") is not True:
        reasons.append("controls.transition_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("transition_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.transition_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("transition_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.transition_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("transition_gate_does_not_execute") is not True:
        reasons.append("manual_gate.transition_gate_does_not_execute must be True")
