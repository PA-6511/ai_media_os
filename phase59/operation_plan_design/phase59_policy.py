...
def evaluate_phase59_policy(
    ...
    if controls.get("operation_plan_controls_does_not_execute") is not True:
        reasons.append("controls.operation_plan_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("operation_plan_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.operation_plan_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("operation_plan_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.operation_plan_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("operation_plan_gate_does_not_execute") is not True:
        reasons.append("manual_gate.operation_plan_gate_does_not_execute must be True")
