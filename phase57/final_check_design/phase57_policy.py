...
def evaluate_phase57_policy(
    ...
    if controls.get("final_check_controls_does_not_execute") is not True:
        reasons.append("controls.final_check_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("final_check_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.final_check_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("final_check_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.final_check_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("final_check_gate_does_not_execute") is not True:
        reasons.append("manual_gate.final_check_gate_does_not_execute must be True")
