...
def evaluate_phase52_policy(
    ...
    if controls.get("go_live_review_controls_does_not_execute") is not True:
        reasons.append("controls.go_live_review_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("go_live_review_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.go_live_review_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("go_live_review_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.go_live_review_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("go_live_review_gate_does_not_execute") is not True:
        reasons.append("manual_gate.go_live_review_gate_does_not_execute must be True")
