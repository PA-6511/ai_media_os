...
def evaluate_phase45_policy(
    ...
    if controls.get("handoff_controls_does_not_execute") is not True:
        reasons.append("controls.handoff_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("handoff_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.handoff_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("handoff_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.handoff_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("handoff_gate_does_not_execute") is not True:
        reasons.append("manual_gate.handoff_gate_does_not_execute must be True")
