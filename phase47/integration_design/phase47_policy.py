...
def evaluate_phase47_policy(
    ...
    if controls.get("integration_controls_does_not_execute") is not True:
        reasons.append("controls.integration_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("integration_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.integration_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("integration_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.integration_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("integration_gate_does_not_execute") is not True:
        reasons.append("manual_gate.integration_gate_does_not_execute must be True")
