...
DEFAULT_INPUT = ROOT / "exchange/human_review/phase8_9_manual_publish_go_redecision.json"
FALLBACK_INPUT = ROOT / "exchange/human_review/phase8_9_manual_publish_go_redecision.example.json"
PHASE8_8_RESULT = ROOT / "exchange/logs/phase8_8_keep_no_go_completion_report.json"
DEFAULT_OUTPUT = ROOT / "exchange/logs/phase8_9_manual_publish_go_redecision_result.json"
...
CONFIRMATION_KEYS = [
    ...
    "publish_not_executed_in_phase8_9_confirmed",
]
...
def abort_result(reason: str) -> dict:
    return {
        "package_type": "phase8_9_manual_publish_go_redecision_result",
        "phase": "Phase 8-9",
        "status": "ABORT",
        "decision": "ABORT",
        "reason": reason,
...
def validate_no_go_flags(data: dict) -> dict | None:
    ...
    for flag in FORBIDDEN_TRUE_FLAGS:
        if flags.get(flag) is True or data.get(flag) is True:
            return abort_result(f"{flag}=true is forbidden in Phase 8-9")

    return None


def read_manual_publish_go_redecision(input_path=None, output_path=None) -> dict:
    input_path = (
        Path(input_path)
        if input_path
    ...
    if not PHASE8_8_RESULT.exists():
        result = abort_result(f"phase8_8 result file not found: {PHASE8_8_RESULT}")
    elif not input_path.exists():
        result = abort_result(f"manual publish go redecision file not found: {input_path}")
    else:
        p88 = load_json(PHASE8_8_RESULT)
        ...
        if data.get("mode") != "CONNECTION_TEST":
        ...
        elif p88.get("status") != "PASS":
            result = abort_result("phase8_8 status must be PASS")
        elif p88.get("phase8_7_decision") != "KEEP_NO_GO":
            result = abort_result("phase8_8 phase8_7_decision must be KEEP_NO_GO")
        else:
            ...
            if no_go_err:
                ...
                if decision not in VALID_DECISIONS:
                    ...
                    expected_draft_id = p88.get("target_draft_id")
                    provided_draft_id = data.get("wordpress_draft_id")
                    if expected_draft_id is None:
                        result = abort_result("expected draft id not found in phase8_8 log")
                        ...
                        if decision == "GO_PUBLISH_ONE_TIME_MANUAL_ONLY":
                            if data.get("approval_token") != VALID_GO_TOKEN:
                                ...
                                result = {
                                    **base,
                                    "status": "PASS",
                                    "reason": "manual publish GO redecision recorded; publish remains locked in phase8_9",
                                    "publish_candidate_unlocked_for_operator": True,
                                    "next_step": "phase8_10_manual_publish_candidate_only",
                                }
                        elif decision == "KEEP_NO_GO":
                            result = {
                                **base,
                                "status": "PASS",
                                "reason": "manual KEEP_NO_GO redecision recorded",
                                "publish_candidate_unlocked_for_operator": False,
                            ...
                            result = {
                                **base,
                                "status": "WARN",
                                "reason": "manual redecision requested fixes before publish candidate",
                                "publish_candidate_unlocked_for_operator": False,
...
def main() -> int:
    result = read_manual_publish_go_redecision()
