...
OUTPUT_JSON_PATH = ROOT / "exchange" / "logs" / "security_read_only_policy_phase_s2_validation_result.json"
OUTPUT_MD_PATH = ROOT / "exchange" / "logs" / "security_read_only_policy_phase_s2_validation_result.md"
...
def validate_security_read_only_policy_phase_s2(data: dict[str, Any]) -> dict[str, Any]:
    warnings: list[str] = []
    ...
    if data.get("wordpress_mode") != "READ_ONLY":
        fail_reasons.append("wordpress_mode must be READ_ONLY")
    if data.get("external_api_mode") != "READ_ONLY":
        fail_reasons.append("external_api_mode must be READ_ONLY")
    if data.get("scheduler_mode") != "SAFE_HOLD":
        fail_reasons.append("scheduler_mode must be SAFE_HOLD")
    ...
    result = {
        "phase_id": "PHASE_S2",
        "validator": "validate_security_read_only_policy_phase_s2",
        ...
        "warnings": warnings,
        "timestamp": _now_iso(),
        "next_step": "validate_security_api_permission_map_phase_s2" if validator_result in {"PASS", "WARN"} else "manual_security_review",
    }
    return result
...
def _build_markdown(result: dict[str, Any]) -> str:
    lines = [
        "# Security Read-Only Policy Phase S-2 Validation",
        "",
