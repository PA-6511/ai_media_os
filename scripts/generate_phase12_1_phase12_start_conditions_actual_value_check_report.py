#!/usr/bin/env python3
"""Generate Phase 12-1 report for Phase 12 start conditions actual value check."""
...
DEFAULT_OUTPUT_MD = (
    LOG_DIR / "phase12_1_phase12_start_conditions_actual_value_check_report.md"
)
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase12_1_phase12_start_conditions_actual_value_check_report",
        "phase": "Phase 12-1",
        "status": "ABORT",
        "reason": reason,
...
def validate_phase11_11(source: dict) -> str | None:
    if source.get("phase") != "Phase 11-11":
        return "phase11_11 input phase must be Phase 11-11"
    if source.get("status") != "PASS":
        return "phase11_11 status must be PASS"
    if source.get("phase11_11_design_status") != "PASS":
        return "phase11_11_design_status must be PASS"
...
def build_checks(source: dict) -> list[dict]:
    return [
        ...
        {
            "check": "phase11_11_design_status=PASS",
            "passed": source.get("phase11_11_design_status") == "PASS",
            "actual": source.get("phase11_11_design_status"),
        },
        {
...
def build_report(source: dict, input_path: Path) -> dict:
    ...
    return {
        "package_type": "phase12_1_phase12_start_conditions_actual_value_check_report",
        "phase": "Phase 12-1",
        "title": "Phase 12-1 Phase 12 Start Conditions Actual Value Check Report",
        "status": "PASS" if all_passed else "ABORT",
        "phase12_1_check_status": "PASS" if all_passed else "ABORT",
        ...
        "summary": {
            "phase12_start_conditions_matched": True,
            "publish_still_not_executed": True,
            "no_go_boundary_maintained": True,
        },
        "next_step": "phase12_2_single_publish_final_go_redecision_input_design",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 12-1 Phase 12 Start Conditions Actual Value Check Report",
        "",
        ...
        f"- phase12_1_check_status: {report['phase12_1_check_status']}",
        "",
        "## Validation Checks",
        "",
        "| Check | Result |",
...
def generate_report(input_11_11: Path, output_json: Path, output_md: Path) -> dict:
    if not input_11_11.exists():
        return abort_result(f"required input not found: {input_11_11}", output_json)

    source = load_json(input_11_11)
    err = validate_phase11_11(source)
    if err:
        return abort_result(err, output_json)

    report = build_report(source, input_11_11)
    write_json(output_json, report)
    output_md.parent.mkdir(parents=True, exist_ok=True)
    output_md.write_text(build_markdown(report), encoding="utf-8")

    return {
        "package_type": "phase12_1_phase12_start_conditions_actual_value_check_report_result",
        "phase": "Phase 12-1",
        "status": report["status"],
        "phase12_1_check_status": report["phase12_1_check_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 12-1 start conditions actual value check report"
    )
    parser.add_argument("--input-11-11", type=Path, default=DEFAULT_INPUT_11_11)
    parser.add_argument("--output-json", type=Path, default=DEFAULT_OUTPUT_JSON)
    parser.add_argument("--output-md", type=Path, default=DEFAULT_OUTPUT_MD)
    args = parser.parse_args()

    result = generate_report(args.input_11_11, args.output_json, args.output_md)
