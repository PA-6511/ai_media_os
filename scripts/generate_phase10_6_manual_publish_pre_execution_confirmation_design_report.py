#!/usr/bin/env python3
"""Generate Phase 10-6 design report for manual publish pre-execution confirmation."""
...
DEFAULT_OUTPUT_MD = (
    LOG_DIR
    / "phase10_6_manual_publish_pre_execution_confirmation_design_report.md"
)
...
def validate_phase10_5(source: dict) -> str | None:
    if source.get("phase") != "Phase 10-5":
        return "phase10_5 input phase must be Phase 10-5"
    if source.get("status") != "PASS":
        return "phase10_5 input status must be PASS"
    if source.get("current_decision") != "KEEP_NO_GO":
        return "phase10_5 current_decision must be KEEP_NO_GO"
    if source.get("publish_candidate_unlocked_for_operator") is not False:
        return "phase10_5 publish_candidate_unlocked_for_operator must be false"
    if source.get("wordpress_publish_execution") != "NO_GO":
        return "phase10_5 wordpress_publish_execution must be NO_GO"
    if source.get("wordpress_write_executed") is not False:
        return "phase10_5 wordpress_write_executed must be false"
    if source.get("production_status") != "NO_GO":
        return "phase10_5 production_status must be NO_GO"
    if source.get("wordpress_draft_id") != 110:
        return "phase10_5 wordpress_draft_id must be 110"
    if source.get("target_draft_status") != "draft":
        return "phase10_5 target_draft_status must be draft"

    guard_design = source.get("execution_guard_design", {})
    if guard_design.get("target_scope") != "draft_to_publish_only":
        return "phase10_5 target_scope must be draft_to_publish_only"
    if guard_design.get("token_expiry_minutes") != 30:
        return "phase10_5 token_expiry_minutes must be 30"
    if guard_design.get("max_publish_count") != 1:
        return "phase10_5 max_publish_count must be 1"
    if guard_design.get("relock_required_after_attempt") is not True:
        return "phase10_5 relock_required_after_attempt must be true"
...
def build_checks(source: dict) -> list[dict]:
    return [
        {"check": "phase10_5_status=PASS", "passed": source.get("status") == "PASS", "actual": source.get("status")},
        {
        ...
        {
            "check": "execution_guard_checks_count=5",
            "passed": source.get("execution_guard_design", {}).get("execution_guard_checks_count") == 5,
            "actual": source.get("execution_guard_design", {}).get("execution_guard_checks_count"),
        },
    ]
...
def build_report(source: dict, input_path: Path) -> dict:
    ...
    pre_execution_confirmation_design = {
        ...
        "checklist": [
            ...
            "human_reviewer_identity_verified",
            "forbidden_operations_absent",
            "relock_plan_prepared",
            "publish_execution_in_phase10_6_must_remain_no_go",
        ],
    ...
    confirmation_outcomes = [
        {
            "confirmation_result": "PRE_EXECUTION_CONFIRMED",
            "publish_execution_in_phase10_6": "NO_GO",
            "wordpress_write_executed_in_phase10_6": False,
            "next_step": "phase10_7_manual_publish_immediate_pre_execution_go_freeze_decision_design",
        },
        {
            "confirmation_result": "PRE_EXECUTION_BLOCKED_KEEP_LOCKED",
            "publish_execution_in_phase10_6": "NO_GO",
            "wordpress_write_executed_in_phase10_6": False,
            "next_step": "maintain_no_go",
        },
        {
            "confirmation_result": "REQUEST_FIX_REQUIRED",
            "publish_execution_in_phase10_6": "NO_GO",
            "wordpress_write_executed_in_phase10_6": False,
            "next_step": "apply_requested_fix_before_next_redecision",
        },
        {
            "confirmation_result": "ABORT",
            "publish_execution_in_phase10_6": "NO_GO",
            "wordpress_write_executed_in_phase10_6": False,
    ...
    return {
        "package_type": "phase10_6_manual_publish_pre_execution_confirmation_design_report",
        "phase": "Phase 10-6",
        "title": "Phase 10-6 Manual Publish Pre Execution Confirmation Design Report",
        "status": "PASS" if all_passed else "ABORT",
        "phase10_6_design_status": "PASS" if all_passed else "ABORT",
        ...
        "wordpress_write_executed": False,
        "production_status": "NO_GO",
        "pre_execution_confirmation_design": pre_execution_confirmation_design,
        ...
        "summary": {
            "pre_execution_confirmation_design_fixed": True,
            "publish_still_not_executed_in_phase10_6": True,
            "wordpress_write_still_not_executed_in_phase10_6": True,
        },
        "next_step": "phase10_7_manual_publish_immediate_pre_execution_go_freeze_decision_design",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        ...
        f"- phase10_6_design_status: {report['phase10_6_design_status']}",
    ...
    for key, value in report["pre_execution_confirmation_design"].items():
    ...
    for step in report["confirmation_flow"]:
        lines.append(f"- {step}")
    ...
    for row in report["confirmation_outcomes"]:
        lines.append(
            "| "
            f"{row['confirmation_result']} | {row['publish_execution_in_phase10_6']} | "
            f"{row['wordpress_write_executed_in_phase10_6']} | {row['next_step']} |"
        )
...
def generate_report(input_10_5: Path, output_json: Path, output_md: Path) -> dict:
    if not input_10_5.exists():
        return abort_result(f"required input not found: {input_10_5}", output_json)

    source = load_json(input_10_5)
    err = validate_phase10_5(source)
    if err:
        return abort_result(err, output_json)

    report = build_report(source, input_10_5)
    ...
    return {
        "package_type": "phase10_6_manual_publish_pre_execution_confirmation_design_report_result",
        "phase": "Phase 10-6",
        "status": report["status"],
        "phase10_6_design_status": report["phase10_6_design_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 10-6 manual publish pre execution confirmation design report"
    )
    parser.add_argument("--input-10-5", type=Path, default=DEFAULT_INPUT_10_5)
    parser.add_argument("--output-json", type=Path, default=DEFAULT_OUTPUT_JSON)
    parser.add_argument("--output-md", type=Path, default=DEFAULT_OUTPUT_MD)
    args = parser.parse_args()

    result = generate_report(args.input_10_5, args.output_json, args.output_md)
