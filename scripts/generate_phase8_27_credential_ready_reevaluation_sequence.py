#!/usr/bin/env python3
import json
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

ROOT = Path(__file__).resolve().parents[1]
DEFAULT_POLICY = ROOT / "config/phase8_27_credential_ready_reevaluation_sequence_policy.json"
DEFAULT_OUTPUT_JSON = ROOT / "exchange/logs/phase8_27_credential_ready_reevaluation_sequence.json"
DEFAULT_OUTPUT_MD = ROOT / "exchange/logs/phase8_27_credential_ready_reevaluation_sequence.md"


def _now_iso() -> str:
    return datetime.now(timezone.utc).isoformat()


def _load_json(path: Path) -> dict[str, Any]:
    return json.loads(path.read_text(encoding="utf-8"))


def _resolve_root(path: Path) -> Path:
    if path.parent.name == "config":
        return path.parent.parent
    return path.parent


def generate_credential_ready_reevaluation_sequence(
    policy_path: Path = DEFAULT_POLICY,
    output_json_path: Path = DEFAULT_OUTPUT_JSON,
    output_md_path: Path = DEFAULT_OUTPUT_MD,
) -> dict[str, Any]:
    policy_path = Path(policy_path)
    output_json_path = Path(output_json_path)
    output_md_path = Path(output_md_path)

    errors: list[str] = []
    warnings: list[str] = []
    safety_violations: list[str] = []
    evidence_summary: list[dict[str, Any]] = []
    planned_steps: list[str] = []

    if not policy_path.exists():
        safety_violations.append(f"missing_policy: {policy_path}")
        result = _build_result("ABORT", planned_steps, evidence_summary, errors, warnings, safety_violations, {})
        _write_outputs(result, output_json_path, output_md_path)
        return result

    policy = _load_json(policy_path)
    for flag in [
        "reevaluation_plan_is_execution_permission",
        "commands_executed_in_this_phase",
        "wordpress_api_call_allowed",
        "wordpress_write_executed",
        "publish_allowed",
        "auto_post",
    ]:
        if policy.get(flag) is not False:
            safety_violations.append(f"{flag} must be false")

    root = _resolve_root(policy_path)
    declaration_status: str | None = None
    evidence_missing = False
    for rel in policy.get("required_evidence", []):
        ev_path = root / rel
        if not ev_path.exists():
            evidence_missing = True
            evidence_summary.append({"path": rel, "exists": False, "status": None})
            errors.append(f"missing_evidence: {rel}")
            continue
        payload = _load_json(ev_path)
        status = payload.get("status") or payload.get("overall_status")
        declaration_status = status
        evidence_summary.append({"path": rel, "exists": True, "status": status})
        if payload.get("secret_values_written") is True:
            safety_violations.append(f"secret_values_written=true in {rel}")

    if evidence_missing:
        status = "REEVALUATION_SEQUENCE_NOT_READY"
    elif safety_violations:
        status = "ABORT"
    elif declaration_status in {"FAIL", "ABORT"}:
        status = "ABORT"
        safety_violations.append(f"phase8_26 terminal status: {declaration_status}")
    elif declaration_status == policy.get("ready_declaration_status"):
        planned_steps = list(policy.get("planned_reevaluation_steps_if_ready", []))
        status = "REEVALUATION_SEQUENCE_READY_BUT_NOT_EXECUTED"
    elif declaration_status == policy.get("not_ready_declaration_status"):
        planned_steps = list(policy.get("planned_steps_if_not_ready", []))
        status = "REEVALUATION_SEQUENCE_NOT_READY_CREDENTIALS_MISSING"
    else:
        planned_steps = list(policy.get("planned_steps_if_not_ready", []))
        status = "REEVALUATION_SEQUENCE_NOT_READY"
        warnings.append(f"unexpected_phase8_26_status: {declaration_status}")

    for step in planned_steps:
        for pattern in policy.get("forbidden_command_patterns", []):
            if pattern in step:
                safety_violations.append(f"forbidden_command_pattern_detected: {pattern}")

    if safety_violations:
        status = "ABORT"

    result = _build_result(status, planned_steps, evidence_summary, errors, warnings, safety_violations, policy)
    _write_outputs(result, output_json_path, output_md_path)
    return result


def _build_result(
    status: str,
    planned_steps: list[str],
    evidence_summary: list[dict[str, Any]],
    errors: list[str],
    warnings: list[str],
    safety_violations: list[str],
    policy: dict[str, Any],
) -> dict[str, Any]:
    return {
        "phase": "Phase 8-27",
        "status": status,
        "production_status": "NO_GO",
        "wordpress_api_call_allowed": False,
        "wordpress_write_executed": False,
        "publish_allowed": False,
        "reevaluation_plan_is_execution_permission": False,
        "commands_executed_in_this_phase": False,
        "target_item_count": policy.get("target_item_count", 1),
        "planned_steps": planned_steps,
        "secret_values_written": False,
        "evidence_summary": evidence_summary,
        "errors": errors,
        "warnings": warnings,
        "safety_violations": safety_violations,
        "allowed_next_step": policy.get("allowed_next_step", "Phase 8-28 manual rerun dry command checklist"),
        "checked_at": _now_iso(),
    }


def _build_markdown(result: dict[str, Any]) -> str:
    lines = [
        "# Phase 8-27 Credential-Ready Re-Evaluation Sequence",
        "",
        "## Purpose",
        "Create reevaluation sequence planning without command execution.",
        "",
        "## Prior Evidence",
    ]
    for item in result.get("evidence_summary", []):
        lines.append(f"- {item.get('path')}: exists={item.get('exists')} status={item.get('status')}")

    lines.extend([
        "",
        "## Credential Declaration",
        f"- status: {result.get('status')}",
        "",
        "## Planned Re-Evaluation Steps",
    ])
    for step in result.get("planned_steps", []):
        lines.append(f"- {step}")
    if not result.get("planned_steps"):
        lines.append("- none")

    lines.extend([
        "",
        "## Safety Flags",
        f"- wordpress_api_call_allowed: {result.get('wordpress_api_call_allowed')}",
        f"- wordpress_write_executed: {result.get('wordpress_write_executed')}",
        f"- publish_allowed: {result.get('publish_allowed')}",
        f"- reevaluation_plan_is_execution_permission: {result.get('reevaluation_plan_is_execution_permission')}",
        f"- commands_executed_in_this_phase: {result.get('commands_executed_in_this_phase')}",
        f"- secret_values_written: {result.get('secret_values_written')}",
        "",
        "## Final Judgment",
        f"- {result.get('status')}",
        "",
        "## Next Step",
        f"- {result.get('allowed_next_step')}",
    ])
    return "\n".join(lines) + "\n"


def _write_outputs(result: dict[str, Any], output_json_path: Path, output_md_path: Path) -> None:
    output_json_path.parent.mkdir(parents=True, exist_ok=True)
    output_md_path.parent.mkdir(parents=True, exist_ok=True)
    output_json_path.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    output_md_path.write_text(_build_markdown(result), encoding="utf-8")


def main() -> int:
    result = generate_credential_ready_reevaluation_sequence()
    print(json.dumps(result, ensure_ascii=False, indent=2))
    acceptable = {
        "REEVALUATION_SEQUENCE_READY_BUT_NOT_EXECUTED",
        "REEVALUATION_SEQUENCE_NOT_READY_CREDENTIALS_MISSING",
        "REEVALUATION_SEQUENCE_NOT_READY",
    }
    return 0 if result.get("status") in acceptable else 2


if __name__ == "__main__":
    raise SystemExit(main())
