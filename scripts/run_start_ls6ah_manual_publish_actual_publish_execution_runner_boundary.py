...
STATUS_PASSED = "LS6AH_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_PASSED_NO_PUBLISH"
STATUS_FAILED = "LS6AH_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_FAILED_NO_PUBLISH"
STATUS_NOT_READY = "LS6AH_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_NOT_READY"
STATUS_NOT_READY_MISSING_RECORD_FLAG = "LS6AH_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_NOT_READY_MISSING_RECORD_FLAG"
STATUS_NOT_READY_MISSING_FINAL_PREFLIGHT_FLAG = "LS6AH_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_NOT_READY_MISSING_FINAL_PREFLIGHT_FLAG"
STATUS_NOT_READY_MISSING_SEPARATE_EXECUTION_PHASE_FLAG = "LS6AH_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_NOT_READY_MISSING_SEPARATE_EXECUTION_PHASE_FLAG"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--policy", default="config/start_ls6ah_manual_publish_actual_publish_execution_runner_boundary_policy.json")
    ...
    parser.add_argument("--actual-publish-execution-runner-boundary-output", default="exchange/runtime/start_ls6ah_manual_publish_actual_publish_execution_runner_boundary_result.json")
    parser.add_argument("--actual-publish-execution-runner-boundary-lock-output", default="exchange/locks/start_ls6ah_manual_publish_actual_publish_execution_runner_boundary.lock.json")
    parser.add_argument("--output", default="exchange/logs/start_ls6ah_manual_publish_actual_publish_execution_runner_boundary_result.json")
    parser.add_argument("--report", default="reports/start_ls6ah_manual_publish_actual_publish_execution_runner_boundary_report.md")
    parser.add_argument("--record-actual-publish-execution-runner-boundary", action="store_true")
    parser.add_argument("--require-actual-publish-execution-runner-final-preflight", action="store_true")
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# LS-6AH Manual Publish Actual Publish Execution Runner Boundary Report",
        "",
        f"- generated_at: {result['generated_at']}",
        f"- status: {result['status']}",
        f"- post_id: {result['post_id']}",
        f"- returned_post_status: {result['returned_post_status']}",
        f"- actual_publish_execution_runner_boundary_ready: {result['actual_publish_execution_runner_boundary_ready']}",
...
def build_runtime_result(errors: list[str], returned_post_status: str) -> dict[str, Any]:
    return {
        "phase": "LS-6AH",
        "document_type": "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_RESULT",
        "status": "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_READY_NO_PUBLISH" if not errors else "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_NOT_READY_NO_PUBLISH",
        "post_id": 183,
        "post_link": "https://hoshido.jp/?p=183",
        "payload_title": "2.5次元の誘惑",
        "payload_asin": "B07X2G67B4",
        "returned_post_status": returned_post_status,
        "ls6ag_final_execution_command_validated": not errors,
        "actual_publish_execution_runner_boundary_ready": not errors,
...
def build_lock() -> dict[str, Any]:
    return {
        "phase": "LS-6AH",
        "document_type": "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_LOCK",
...
def build_run_result(status: str, errors: list[str], returned_post_status: str) -> dict[str, Any]:
    ...
    return {
        ...
        "ls6ag_final_execution_command_validated": runtime_result["ls6ag_final_execution_command_validated"],
        "actual_publish_execution_runner_boundary_ready": runtime_result["actual_publish_execution_runner_boundary_ready"],
        "actual_publish_execution_runner_boundary_consumed": runtime_result["actual_publish_execution_runner_boundary_consumed"],
        "actual_publish_final_execution_command_recorded": runtime_result["actual_publish_final_execution_command_recorded"],
        "actual_publish_final_execution_command_label": runtime_result["actual_publish_final_execution_command_label"],
        "actual_publish_final_execution_command_consumed": runtime_result["actual_publish_final_execution_command_consumed"],
        ...
        "rerun_allowed": False,
        "ls6oc1_rerun_executed": False,
        "requires_actual_publish_execution_runner_final_preflight": runtime_result["requires_actual_publish_execution_runner_final_preflight"],
...
def main() -> int:
    ...
    if not args.record_actual_publish_execution_runner_boundary:
        return write_not_ready(output_path, report_path, STATUS_NOT_READY_MISSING_RECORD_FLAG, "missing --record-actual-publish-execution-runner-boundary")
    if not args.require_actual_publish_execution_runner_final_preflight:
        return write_not_ready(output_path, report_path, STATUS_NOT_READY_MISSING_FINAL_PREFLIGHT_FLAG, "missing --require-actual-publish-execution-runner-final-preflight")
    ...
    req(policy.get("phase") == "LS-6AH", "policy.phase mismatch", errors)
    req(policy.get("execution_mode") == "ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_ONLY_NO_PUBLISH", "policy.execution_mode mismatch", errors)
    ...
    req(ls6ag_ready.get("post_id") == 183, "LS-6AG post_id mismatch", errors)
    req(ls6ag_ready.get("returned_post_status") == "draft", "LS-6AG returned_post_status mismatch", errors)
    ...
    req(ls6ag_ready.get("actual_publish_runner_final_gate_consumed") is False, "LS-6AG runner final gate consumed must be false", errors)
    req(ls6ag_ready.get("actual_publish_final_preflight_consumed") is False, "LS-6AG final preflight consumed must be false", errors)
    req(ls6ag_ready.get("actual_publish_execution_boundary_consumed") is False, "LS-6AG execution boundary consumed must be false", errors)
    req(ls6ag_ready.get("actual_publish_execute_now_final_confirmation_consumed") is False, "LS-6AG execute-now final confirmation consumed must be false", errors)
    req(ls6ag_ready.get("explicit_execute_now_for_actual_publish_consumed") is False, "LS-6AG explicit execute-now consumed must be false", errors)
    req(ls6ag_ready.get("actual_publish_execution_runner_executed") is False, "LS-6AG runner executed must be false", errors)
    req(ls6ag_ready.get("publish_execution_still_blocked") is True, "LS-6AG publish_execution_still_blocked must be true", errors)
    req(ls6ag_ready.get("requires_actual_publish_execution_runner_boundary") is True, "LS-6AG requires_actual_publish_execution_runner_boundary must be true", errors)
    req(safe_get(ls6ag_ready, "next_phase", "phase") == "LS-6AH", "LS-6AG next_phase mismatch", errors)

    cmd = ls6ag_command.get("actual_publish_final_execution_command", {})
    ...
    req(cmd.get("actual_publish_final_execution_command_label") == "FINAL_COMMAND_FOR_ACTUAL_PUBLISH_EXECUTION_RUNNER_GATE_ONLY", "LS-6AG command result label mismatch", errors)
    req(cmd.get("actual_publish_final_execution_command_consumed") is False, "LS-6AG command result consumed must be false", errors)
    req(cmd.get("requires_next_phase") == "LS-6AH", "LS-6AG command result requires_next_phase mismatch", errors)
    ...
    returned_post_status = str(ls6ag_ready.get("returned_post_status", ""))
    ...
    write_json(Path(args.actual_publish_execution_runner_boundary_output), runtime_result)
    write_json(Path(args.actual_publish_execution_runner_boundary_lock_output), lock)
    write_json(output_path, run_result)
    write_report(report_path, run_result)
    print(json.dumps(run_result, ensure_ascii=False, indent=2))
    return 0
