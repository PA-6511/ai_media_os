...
DEFAULT_INPUT = ROOT / "exchange" / "logs" / "phase68_preparation_review_report.json"
DEFAULT_OUTPUT = ROOT / "exchange" / "logs" / "phase69_preparation_approval_report.json"
...
def abort_result(reason: str) -> dict:
    return {
        "package_type": "phase69_preparation_approval_report_result",
        "phase": "Phase 69",
        "status": "ABORT",
        "reason": reason,
...
def validate_phase68_report(data: dict) -> dict | None:
    if data.get("phase") != "Phase 68":
        return abort_result("input phase must be 'Phase 68'")
    if data.get("package_type") != "phase68_preparation_review_report":
    ...
    if data.get("review_status") != "PASS":
        return abort_result("input review_status must be PASS")
    ...
    review_package = data.get("review_package", {})
    if review_package.get("does_not_execute") is not True:
        return abort_result("review_package.does_not_execute must be true")

    controls = data.get("review_controls", {})
    if controls.get("does_not_execute") is not True:
        return abort_result("review_controls.does_not_execute must be true")

    evidence_checks = data.get("review_evidence_checks", {})
    if evidence_checks.get("does_not_execute") is not True:
        return abort_result("review_evidence_checks.does_not_execute must be true")
    ...
    gate = data.get("manual_review_gate", {})
    if gate.get("does_not_execute") is not True:
        return abort_result("manual_review_gate.does_not_execute must be true")
    if "ALLOW_PHASE69_READINESS_REVIEW_ONLY" not in gate.get("allowed_decisions", []):
        return abort_result("manual_review_gate.allowed_decisions missing ALLOW_PHASE69_READINESS_REVIEW_ONLY")

    readiness = data.get("phase69_readiness", {})
    if readiness.get("readiness_status") != "READY_FOR_PHASE69_READINESS_REVIEW_ONLY":
        return abort_result("phase69_readiness.readiness_status must be READY_FOR_PHASE69_READINESS_REVIEW_ONLY")
    if readiness.get("human_approval_required") is not True:
        return abort_result("phase69_readiness.human_approval_required must be true")
    if readiness.get("can_execute") is not False:
        return abort_result("phase69_readiness.can_execute must be false")
    if readiness.get("execute_allowed") is not False:
        return abort_result("phase69_readiness.execute_allowed must be false")

    return None


def build_report(source: dict, input_path: Path) -> dict:
    evidence_checks = source["review_evidence_checks"]

    checks = [
        {
            "check": "input_phase=Phase 68",
            "passed": source.get("phase") == "Phase 68",
            "actual": source.get("phase"),
        },
        {
            "check": "input_package_type=phase68_preparation_review_report",
            "passed": source.get("package_type") == "phase68_preparation_review_report",
        ...
        {
            "check": "approval_controls_does_not_execute=true",
            "passed": source.get("review_controls", {}).get("does_not_execute") is True,
            "actual": source.get("review_controls", {}).get("does_not_execute"),
        },
        {
            "check": "approval_evidence_checks_does_not_execute=true",
            "passed": evidence_checks.get("does_not_execute") is True,
            "actual": evidence_checks.get("does_not_execute"),
        },
        {
            "check": "manual_approval_gate_does_not_execute=true",
            "passed": source.get("manual_review_gate", {}).get("does_not_execute") is True,
            "actual": source.get("manual_review_gate", {}).get("does_not_execute"),
        },
        {
            "check": "phase69_readiness_status=READY_FOR_PHASE69_READINESS_REVIEW_ONLY",
            "passed": source.get("phase69_readiness", {}).get("readiness_status") == "READY_FOR_PHASE69_READINESS_REVIEW_ONLY",
            "actual": source.get("phase69_readiness", {}).get("readiness_status"),
        },
    ]
    ...
    return {
        ...
        "approval_status": "PASS" if all_checks_passed else "ABORT",
        ...
        "approval_package": {
            ...
            "status": "PREPARATION_APPROVAL_PACKAGE_READY" if all_checks_passed else "ABORT",
        },
        "approval_controls": {
            ...
            "single_file_scope_required": True,
            "does_not_execute": True,
            "status": "PREPARATION_APPROVAL_CONTROLS_READY" if all_checks_passed else "ABORT",
        },
        "approval_evidence_checks": {
            ...
            "does_not_execute": True,
            "status": "PREPARATION_APPROVAL_EVIDENCE_CHECKS_READY" if all_checks_passed else "ABORT",
        },
        "manual_approval_gate": {
            ...
            "does_not_execute": True,
            "status": "MANUAL_PREPARATION_APPROVAL_GATE_READY" if all_checks_passed else "ABORT",
        },
        "phase70_readiness": {
            "readiness_status": "READY_FOR_PHASE70_PLANNING_ONLY" if all_checks_passed else "NOT_READY",
...
def generate_phase69_preparation_approval_report(
    ...
    validation_error = validate_phase68_report(source)
    if validation_error:
    ...
    return {
        ...
        "production_status": "NO_GO",
        "phase70_readiness": report["phase70_readiness"]["readiness_status"],
        "created_at": datetime.now(timezone.utc).isoformat(),
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Phase69 preparation approval report")
    parser.add_argument("--input", type=Path, default=DEFAULT_INPUT, help="Input Phase68 report path")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT, help="Output Phase69 report path")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing output")
    args = parser.parse_args()

    result = generate_phase69_preparation_approval_report(args.input, args.output, overwrite=args.overwrite)
