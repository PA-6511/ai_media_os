#!/usr/bin/env python3
"""Generate Phase 25-4 report for overall Phase 25 closure."""
...
DEFAULT_OUTPUT_JSON = LOG_DIR / "phase25_4_phase25_overall_closure_report.json"
DEFAULT_OUTPUT_MD = LOG_DIR / "phase25_4_phase25_overall_closure_report.md"
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase25_4_phase25_overall_closure_report",
        "phase": "Phase 25-4",
        "status": "ABORT",
        "reason": reason,
        "phase25_overall_closure_status": "ABORT",
...
def build_report(reports: dict[str, dict]) -> dict:
    return {
        "package_type": "phase25_4_phase25_overall_closure_report",
        "phase": "Phase 25-4",
        "title": "Phase 25-4 Phase 25 Overall Closure Report",
        "status": "PASS",
        "phase25_overall_closure_status": "PASS",
        "phase25_overall_status": "PASS",
        "phase_range": "Phase 25-1 through Phase 25-3",
        "execute_live_executed": False,
        "current_decision": "KEEP_NO_GO",
        "publish_candidate_unlocked_for_operator": False,
        "wordpress_publish_execution": "NO_GO",
        ...
        "target_draft_status": "draft",
        "phase_statuses": {
            phase: reports[phase].get("status") for phase in ["25-1", "25-2", "25-3"]
        },
        "still_forbidden": STILL_FORBIDDEN,
        "summary": {
            "phase25_closed_without_execute_live": True,
            "phase25_no_go_boundary_maintained": True,
            "phase26_can_start_from_formal_closure": True,
        },
        "next_step": "phase26_readiness_review_or_start_conditions",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 25-4 Phase 25 Overall Closure Report",
        "",
        f"Generated: {report['created_at']}",
        "",
        f"- status: {report['status']}",
        f"- phase25_overall_closure_status: {report['phase25_overall_closure_status']}",
...
def generate_report(logs_dir: Path, output_json: Path, output_md: Path) -> dict:
    ...
    return {
        "package_type": "phase25_4_phase25_overall_closure_report_result",
        "phase": "Phase 25-4",
        "status": report["status"],
        "phase25_overall_closure_status": report["phase25_overall_closure_status"],
        "phase25_overall_status": report["phase25_overall_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Phase 25-4 overall closure report")
