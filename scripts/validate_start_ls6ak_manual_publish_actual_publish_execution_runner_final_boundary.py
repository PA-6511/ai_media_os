...
STATUS_VALIDATED = "LS6AK_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_VALIDATED_NO_PUBLISH"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--final-boundary-result", default="exchange/runtime/start_ls6ak_manual_publish_actual_publish_execution_runner_final_boundary_result.json")
    parser.add_argument("--final-boundary-lock", default="exchange/locks/start_ls6ak_manual_publish_actual_publish_execution_runner_final_boundary.lock.json")
    parser.add_argument("--run-result", default="exchange/logs/start_ls6ak_manual_publish_actual_publish_execution_runner_final_boundary_result.json")
    ...
    parser.add_argument("--output", default="exchange/logs/start_ls6ak_manual_publish_actual_publish_execution_runner_final_boundary_validation_result.json")
    parser.add_argument("--report", default="reports/start_ls6ak_manual_publish_actual_publish_execution_runner_final_boundary_validation_report.md")
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# LS-6AK Manual Publish Actual Publish Execution Runner Final Boundary Validation Report",
        "",
...
def build_result(run: dict[str, Any], errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "ls6aj_execute_now_gate_validated": bool(run.get("ls6aj_execute_now_gate_validated", False)),
        "actual_publish_execution_runner_final_boundary_ready": bool(run.get("actual_publish_execution_runner_final_boundary_ready", False)),
        "actual_publish_execution_runner_final_boundary_consumed": bool(run.get("actual_publish_execution_runner_final_boundary_consumed", False)),
        "actual_publish_execution_runner_execute_now_recorded": bool(run.get("actual_publish_execution_runner_execute_now_recorded", False)),
        "actual_publish_execution_runner_execute_now_label": run.get("actual_publish_execution_runner_execute_now_label", ""),
        "actual_publish_execution_runner_execute_now_consumed": bool(run.get("actual_publish_execution_runner_execute_now_consumed", False)),
        ...
        "requires_separated_actual_publish_execution_runner_phase": bool(run.get("requires_separated_actual_publish_execution_runner_phase", False)),
...
def main() -> int:
    ...
    runtime = try_load_json(Path(args.final_boundary_result), errors)
    lock = try_load_json(Path(args.final_boundary_lock), errors)
    ...
    req(run.get("status") == "LS6AK_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_PASSED_NO_PUBLISH", "run status mismatch", errors)
    req(runtime.get("status") == "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_READY_NO_PUBLISH", "runtime status mismatch", errors)
    req(lock.get("status") == "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_FINAL_BOUNDARY_LOCKED_NO_PUBLISH", "lock status mismatch", errors)
    ...
    req(ls6aj_execute.get("execute_now_status") == "ACTUAL_PUBLISH_EXECUTION_RUNNER_EXECUTE_NOW_RECORDED_NO_PUBLISH_EXECUTION", "LS-6AJ execute status mismatch", errors)
    ...
    req(ls6ai_final.get("actual_publish_execution_runner_final_preflight_consumed") is False, "LS-6AI preflight consumed must be false", errors)
    ...
    req(ls6ah_boundary.get("actual_publish_execution_runner_boundary_consumed") is False, "LS-6AH consumed must be false", errors)
    req(ls6ah_lock.get("actual_publish_execution_runner_boundary_consumed") is False, "LS-6AH lock consumed must be false", errors)
    req(ls6ag_ready.get("status") == "LS6AG_MANUAL_PUBLISH_ACTUAL_PUBLISH_FINAL_EXECUTION_COMMAND_READY_NO_PUBLISH", "LS-6AG status mismatch", errors)
    ...
    req(ls6af_runtime.get("actual_publish_runner_final_gate_consumed") is False, "LS-6AF consumed must be false", errors)
    ...
    req(ls6ae_final.get("actual_publish_final_preflight_consumed") is False, "LS-6AE consumed must be false", errors)
    ...
    req(ls6ad_boundary.get("actual_publish_execution_boundary_consumed") is False, "LS-6AD consumed must be false", errors)
    ...
    req(ls6aa_final.get("actual_publish_execution_final_preflight_consumed") is False, "LS-6AA consumed must be false", errors)
    ...
    req(run.get("ls6aj_execute_now_gate_validated") is True, "run.ls6aj_execute_now_gate_validated must be true", errors)
    req(run.get("actual_publish_execution_runner_final_boundary_ready") is True, "run.final boundary ready must be true", errors)
    req(run.get("actual_publish_execution_runner_execute_now_recorded") is True, "run.execute-now recorded must be true", errors)
    req(run.get("actual_publish_execution_runner_execute_now_label") == "EXECUTE_NOW_FOR_ACTUAL_PUBLISH_EXECUTION_RUNNER_GATE_ONLY", "run.execute-now label mismatch", errors)
    req(run.get("actual_publish_final_execution_command_recorded") is True, "run.final command recorded must be true", errors)
    req(run.get("actual_publish_final_execution_command_label") == "FINAL_COMMAND_FOR_ACTUAL_PUBLISH_EXECUTION_RUNNER_GATE_ONLY", "run.final command label mismatch", errors)
    req(run.get("actual_publish_execution_runner_blocked") is True, "run.runner blocked must be true", errors)
    req(run.get("requires_separated_actual_publish_execution_runner_phase") is True, "run.requires_separated_actual_publish_execution_runner_phase must be true", errors)
    ...
    req(lock.get("requires_next_phase") == "LS-6AL", "lock.requires_next_phase mismatch", errors)

    next_phase = run.get("next_phase", {})
    req(next_phase.get("phase") == "LS-6AL", "run.next_phase.phase mismatch", errors)
    req(next_phase.get("execution_allowed") is False, "run.next_phase.execution_allowed must be false", errors)
    req(next_phase.get("requires_separated_actual_publish_execution_runner_phase") is True, "run.next_phase.requires_separated_actual_publish_execution_runner_phase must be true", errors)
    req(next_phase.get("publish_execution_still_blocked") is True, "run.next_phase.publish_execution_still_blocked must be true", errors)
