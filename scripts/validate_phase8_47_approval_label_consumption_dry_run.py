...
DEFAULT_POLICY = ROOT / "config/phase8_47_approval_label_consumption_dry_run_policy.json"
DEFAULT_REVIEW = ROOT / "exchange/human_review/phase8_47_approval_label_consumption_dry_run.example.json"
DEFAULT_OUTPUT_JSON = ROOT / "exchange/logs/phase8_47_approval_label_consumption_dry_run_result.json"
DEFAULT_OUTPUT_MD = ROOT / "exchange/logs/phase8_47_approval_label_consumption_dry_run_result.md"
...
def _write_outputs(result: dict[str, Any], output_json_path: Path, output_md_path: Path) -> None:
    ...
    lines = [
        "# Phase 8-47 Approval Label Consumption Dry-Run Result",
        "",
        f"- status: {result.get('status')}",
        f"- decision: {result.get('decision')}",
        f"- approval_label: {result.get('approval_label')}",
        f"- approval_label_present: {result.get('approval_label_present')}",
...
def validate_phase8_47(
    ...
    for flag in [
        "dry_run_is_execution_permission",
        "execution_allowed",
    ...
    approval_label_present = bool(review.get("approval_label_present", False))
    ...
    if not approval_label_present:
        safety_violations.append("approval_label_present must be true")
    ...
    if safety_violations:
        ...
        status = "FAIL"
    elif decision == "VALIDATE_APPROVAL_LABEL_CONSUMPTION_DRY_RUN_ONLY":
        status = "PHASE8_47_APPROVAL_LABEL_CONSUMPTION_DRY_RUN_PASS_NO_EXECUTION"
    elif decision == "REQUEST_FIX":
        status = "PHASE8_47_APPROVAL_LABEL_CONSUMPTION_DRY_RUN_REQUEST_FIX_NO_EXECUTION"
    elif decision == "REJECT":
        status = "PHASE8_47_APPROVAL_LABEL_CONSUMPTION_DRY_RUN_REJECTED_NO_EXECUTION"
    elif decision == "ABORT":
        status = "ABORT"
    else:
        status = "FAIL"

    if status == "PHASE8_47_APPROVAL_LABEL_CONSUMPTION_DRY_RUN_PASS_NO_EXECUTION":
...
def main() -> int:
    result = validate_phase8_47()
    print(json.dumps(result, ensure_ascii=False, indent=2))
    acceptable = {
        "PHASE8_47_APPROVAL_LABEL_CONSUMPTION_DRY_RUN_PASS_NO_EXECUTION",
        "PHASE8_47_APPROVAL_LABEL_CONSUMPTION_DRY_RUN_REQUEST_FIX_NO_EXECUTION",
        "PHASE8_47_APPROVAL_LABEL_CONSUMPTION_DRY_RUN_REJECTED_NO_EXECUTION",
        "FAIL",
    }
