...
DEFAULT_POLICY = ROOT / "config/phase8_34_post_run_evidence_contract_rollback_trigger_design_only_policy.json"
DEFAULT_REQUEST = ROOT / "exchange/examples/phase8_34_post_run_evidence_contract_rollback_trigger_design_only_request.example.json"
DEFAULT_OUTPUT = ROOT / "exchange/logs/phase8_34_post_run_evidence_contract_rollback_trigger_design_only_result.json"


def validate_phase8_34(
    ...
    if not policy_path.exists() or not request_path.exists():
        result = base_result("8-34", "Post-run evidence contract and rollback trigger design only")
    ...
    result = base_result("8-34", policy.get("phase_name", "Post-run evidence contract and rollback trigger design only"))
    result["target_item_count"] = int(request.get("target_item_count", 1))
    result["post_run_evidence_contract_design_only"] = True
    result["rollback_trigger_design_only"] = True
    result["post_run_evidence_generated"] = False
    result["rollback_required_simulated"] = bool(request.get("rollback_required_simulated", False))
    result["rollback_trigger_status"] = "SIMULATED_REQUIRED" if result["rollback_required_simulated"] else "SIMULATED_NOT_REQUIRED"
    ...
    if not found:
        ...
        result["final_status"] = "DESIGN_ONLY_POST_RUN_EVIDENCE_ROLLBACK_SPEC_READY_NO_EXECUTION"
...
def main() -> int:
    result = validate_phase8_34()
    print(json.dumps(result, ensure_ascii=False, indent=2))
    return 0 if result.get("final_status") == "DESIGN_ONLY_POST_RUN_EVIDENCE_ROLLBACK_SPEC_READY_NO_EXECUTION" else 2
