#!/usr/bin/env python3
"""Generate Phase 20-9 report for overall Phase 20 completion."""
...
DEFAULT_OUTPUT_JSON = LOG_DIR / "phase20_9_phase20_overall_completion_report.json"
DEFAULT_OUTPUT_MD = LOG_DIR / "phase20_9_phase20_overall_completion_report.md"
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase20_9_phase20_overall_completion_report",
        "phase": "Phase 20-9",
        "status": "ABORT",
        "reason": reason,
...
def validate_inputs(reports: dict[str, dict]) -> str | None:
    ...
    for phase in ["20-1", "20-2", "20-3", "20-4", "20-5", "20-6", "20-7", "20-8"]:
        data = reports[phase]
...
def build_report(reports: dict[str, dict]) -> dict:
    return {
        "package_type": "phase20_9_phase20_overall_completion_report",
        "phase": "Phase 20-9",
        "title": "Phase 20-9 Phase 20 Overall Completion Report",
        "status": "PASS",
        "phase20_overall_status": "PASS",
        "phase_range": "Phase 20-1 through Phase 20-8",
        "current_decision": "KEEP_NO_GO",
        "publish_candidate_unlocked_for_operator": False,
        "wordpress_publish_execution": "NO_GO",
        "wordpress_write_executed": False,
        ...
        "summary": {
            "phase20_completed_without_execute_live": True,
            "phase20_no_go_boundary_maintained": True,
            "phase21_start_conditions_can_be_designed": True,
        },
        "next_step": "phase21_1_start_conditions_check_design",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 20-9 Phase 20 Overall Completion Report",
        "",
        f"Generated: {report['created_at']}",
        "",
        f"- status: {report['status']}",
        f"- phase20_overall_status: {report['phase20_overall_status']}",
...
def generate_report(logs_dir: Path, output_json: Path, output_md: Path) -> dict:
    ...
    return {
        "package_type": "phase20_9_phase20_overall_completion_report_result",
        "phase": "Phase 20-9",
        "status": report["status"],
        "phase20_overall_status": report["phase20_overall_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Phase 20-9 overall completion report")
