...
DEFAULT_POLICY = ROOT / "config/phase8_33_post_provision_env_recheck_policy.json"
DEFAULT_OUTPUT_JSON = ROOT / "exchange/logs/phase8_33_post_provision_env_recheck_result.json"
DEFAULT_OUTPUT_MD = ROOT / "exchange/logs/phase8_33_post_provision_env_recheck_result.md"
...
def validate_post_provision_env_recheck(
    ...
    for flag in [
        "recheck_is_execution_permission",
    ...
    if safety_violations:
        ...
        for name in policy.get("required_env", []):
            ...
            if not exists:
                all_present = False
        if all_present:
            status = "POST_PROVISION_ENV_READY_NO_SECRET_OUTPUT"
        else:
            status = "POST_PROVISION_ENV_MISSING_NO_SECRET_OUTPUT"
    elif overlay_status == policy.get("not_ready_overlay_status"):
        for name in policy.get("required_env", []):
            credentials[name] = {"exists": bool(os.environ.get(name, ""))}
        status = "POST_PROVISION_ENV_NOT_READY_BY_DECLARATION"
    elif overlay_status is None:
        status = "ABORT"
    else:
        warnings.append(f"unexpected_overlay_status: {overlay_status}")
        status = "ABORT"
...
def _build_result(
    ...
    return {
        ...
        "allowed_next_step": policy.get("allowed_next_step", "Phase 8-34 one-time manual rerun token / lock package"),
        "checked_at": _now_iso(),
    }
...
def main() -> int:
    result = validate_post_provision_env_recheck()
