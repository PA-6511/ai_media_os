...
STATUS_VALIDATED = "LS6AE_MANUAL_PUBLISH_ACTUAL_PUBLISH_FINAL_PREFLIGHT_VALIDATED_NO_PUBLISH"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--wordpress-current-draft-status-result", default="exchange/runtime/start_ls6ae_wordpress_current_draft_status_verification_result.json")
    parser.add_argument("--actual-publish-final-preflight-result", default="exchange/runtime/start_ls6ae_manual_publish_actual_publish_final_preflight_result.json")
    parser.add_argument("--actual-publish-final-preflight-lock", default="exchange/locks/start_ls6ae_manual_publish_actual_publish_final_preflight.lock.json")
    parser.add_argument("--run-result", default="exchange/logs/start_ls6ae_manual_publish_actual_publish_final_preflight_result.json")
    ...
    parser.add_argument("--output", default="exchange/logs/start_ls6ae_manual_publish_actual_publish_final_preflight_validation_result.json")
    parser.add_argument("--report", default="reports/start_ls6ae_manual_publish_actual_publish_final_preflight_validation_report.md")
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# LS-6AE Manual Publish Actual Publish Final Preflight Validation Report",
        "",
...
def build_result(run: dict[str, Any], errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "requires_actual_publish_runner_final_gate": bool(run.get("requires_actual_publish_runner_final_gate", False)),
...
def main() -> int:
    ...
    wp = try_load_json(Path(args.wordpress_current_draft_status_result), errors)
    final_pf = try_load_json(Path(args.actual_publish_final_preflight_result), errors)
    lock = try_load_json(Path(args.actual_publish_final_preflight_lock), errors)
    ...
    req(run.get("status") == "LS6AE_MANUAL_PUBLISH_ACTUAL_PUBLISH_FINAL_PREFLIGHT_PASSED_NO_PUBLISH", "run status mismatch", errors)
    req(wp.get("status") == "WORDPRESS_CURRENT_DRAFT_STATUS_VERIFIED", "wordpress verification status mismatch", errors)
    req(final_pf.get("status") == "MANUAL_PUBLISH_ACTUAL_PUBLISH_FINAL_PREFLIGHT_PASSED_NO_PUBLISH", "final preflight runtime status mismatch", errors)
    req(lock.get("status") == "MANUAL_PUBLISH_ACTUAL_PUBLISH_FINAL_PREFLIGHT_LOCKED_NO_PUBLISH", "lock status mismatch", errors)
    ...
    req(run.get("actual_publish_execution_final_preflight_ready") is True, "run final preflight ready must be true", errors)
    req(run.get("credential_env_read_executed") is True, "run credential_env_read_executed must be true", errors)
    req(run.get("requires_actual_publish_runner_final_gate") is True, "run requires_actual_publish_runner_final_gate must be true", errors)
    req(run.get("publish_execution_still_blocked") is True, "run publish_execution_still_blocked must be true", errors)
    req(safe_get(run, "next_phase", "phase") == "LS-6AF", "run next_phase mismatch", errors)

    validate_false_flags(run, "run", errors)

    req(final_pf.get("current_post_status_verified") is True, "final_preflight.current_post_status_verified must be true", errors)
    req(final_pf.get("returned_post_status") == "draft", "final_preflight.returned_post_status mismatch", errors)
    req(final_pf.get("wordpress_get_executed") is True, "final_preflight.wordpress_get_executed must be true", errors)
    req(final_pf.get("credential_env_read_executed") is True, "final_preflight.credential_env_read_executed must be true", errors)
    validate_false_flags(final_pf, "final_preflight", errors)

    req(wp.get("wordpress_get_executed") is True, "wp.wordpress_get_executed must be true", errors)
    req(wp.get("credential_env_read_executed") is True, "wp.credential_env_read_executed must be true", errors)
    req(wp.get("returned_post_status") == "draft", "wp.returned_post_status mismatch", errors)
    ...
    req(lock.get("requires_next_phase") == "LS-6AF", "lock.requires_next_phase mismatch", errors)
    ...
    req(ls6ad_boundary.get("actual_publish_execution_boundary_consumed") is False, "LS-6AD boundary consumed must remain false", errors)
    req(ls6ad_lock.get("actual_publish_execution_boundary_consumed") is False, "LS-6AD lock boundary consumed must remain false", errors)
    ...
    req(ls6aa_preflight.get("actual_publish_execution_final_preflight_consumed") is False, "LS-6AA consumed must remain false", errors)
    req(ls6aa_lock.get("actual_publish_execution_final_preflight_consumed") is False, "LS-6AA lock consumed must remain false", errors)
