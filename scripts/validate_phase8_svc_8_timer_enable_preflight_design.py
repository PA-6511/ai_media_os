#!/usr/bin/env python3
"""Validate SVC-8 timer-enable preflight design (design-only / no enable)."""
...
DEFAULT_POLICY = ROOT / "config/phase8_svc_8_timer_enable_preflight_policy.json"
DEFAULT_RUNBOOK = ROOT / "docs/runbooks/phase8_svc_8_timer_enable_preflight_design.md"
DEFAULT_OUTPUT = ROOT / "exchange/logs/phase8_svc_8_timer_enable_preflight_design_result.json"
...
def validate_timer_enable_preflight_design(
    ...
    # Treat as ABORT only if line looks executable.
    for pattern in policy.get("forbidden_patterns", []):
        for line in runbook_text.splitlines():
            ...
            if pattern.lower() in normalized and not normalized.startswith("-"):
                if (
                    normalized.startswith("systemctl ")
                    or normalized.startswith("service ")
                    or normalized.startswith("timer ")
                    or normalized.startswith("curl ")
                    or normalized.startswith("requests.")
                ):
    ...
    if forbidden_hits:
        ...
        status = "PASS_DESIGN_ONLY_TIMER_ENABLE_PREFLIGHT"
...
def _build_result(
    ...
    return {
        "phase": "8-SVC-8",
        "phase_name": "Timer Enable Preflight Design",
        "status": status,
...
def main() -> int:
    result = validate_timer_enable_preflight_design()
    print(json.dumps(result, ensure_ascii=False, indent=2))
    return 0 if result.get("status") == "PASS_DESIGN_ONLY_TIMER_ENABLE_PREFLIGHT" else 2
