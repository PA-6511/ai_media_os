...
DEFAULT_POLICY = ROOT / "config/phase8_36_provisioned_overlay_rerun_policy.json"
DEFAULT_REVIEW = ROOT / "exchange/human_review/phase8_36_provisioned_overlay_rerun.example.json"
DEFAULT_OUTPUT_JSON = ROOT / "exchange/logs/phase8_36_provisioned_overlay_rerun_result.json"
DEFAULT_OUTPUT_MD = ROOT / "exchange/logs/phase8_36_provisioned_overlay_rerun_result.md"
...
def validate_provisioned_overlay_rerun(
    ...
    for flag in [
        "overlay_rerun_is_execution_permission",
    ...
    if phase8_35_status not in set(policy.get("allowed_phase8_35_statuses", [])):
        safety_violations.append(f"phase8_35 status not allowed: {phase8_35_status}")
    ...
    if decision not in allowed:
        ...
        status = "ABORT"
    elif decision == "OVERLAY_RERUN_DECLARE_CREDENTIALS_PROVISIONED_OUTSIDE_REPO":
        status = "OVERLAY_RERUN_CREDENTIALS_DECLARED_PROVISIONED_NO_SECRET_OUTPUT"
    elif decision == "OVERLAY_RERUN_DECLARE_CREDENTIALS_STILL_NOT_READY":
        ...
        status = "FAIL"
        errors.append("operator rejected overlay rerun review")
    else:
        status = "ABORT"
...
def _build_result(
    ...
    return {
        ...
        "allowed_next_step": policy.get("allowed_next_step", "Phase 8-37 credential-ready revalidation after operator update"),
        "checked_at": _now_iso(),
    }
...
def _build_markdown(result: dict[str, Any]) -> str:
    ...
    lines.extend([
        ...
        f"- overlay_rerun_is_execution_permission: {result.get('overlay_rerun_is_execution_permission')}",
...
def main() -> int:
    ...
    acceptable = {
        ...
        "OVERLAY_RERUN_CREDENTIALS_DECLARED_NOT_READY_NO_SECRET_OUTPUT",
        "WARN",
        "FAIL",
    }
