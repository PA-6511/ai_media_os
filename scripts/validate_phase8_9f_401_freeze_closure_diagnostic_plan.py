...
DEFAULT_POLICY = ROOT / "config/phase8_9f_401_freeze_closure_diagnostic_plan_policy.json"
DEFAULT_REQUEST = ROOT / "exchange/examples/phase8_9f_401_freeze_closure_diagnostic_plan_request.example.json"
DEFAULT_OUTPUT_JSON = ROOT / "exchange/logs/phase8_9f_401_freeze_closure_diagnostic_plan_result.json"
...
def validate_phase8_9f_401_freeze_closure_diagnostic_plan(
    ...
    if not policy_path.exists() or not request_path.exists():
        result = _build_result(
            ...
            previous_statuses={},
            errors=["missing_policy_or_request"],
            warnings=warnings,
            policy_violations=["missing_required_file"],
    ...
    if request.get("retry_limit_consumed") is not True:
        policy_violations.append("request.retry_limit_consumed must be true")
    if request.get("freeze_required") is not True:
        policy_violations.append("request.freeze_required must be true")
    if request.get("no_auto_retry_confirmed") is not True:
        policy_violations.append("request.no_auto_retry_confirmed must be true")
    if request.get("no_wordpress_api_confirmed") is not True:
        policy_violations.append("request.no_wordpress_api_confirmed must be true")
    ...
    retry_limit_consumed = True
    for rel, expected_status in policy.get("required_previous_statuses", {}).items():
        path = root / rel
        if not path.exists():
            policy_violations.append(f"missing_previous_evidence: {rel}")
            previous_statuses[rel] = None
            retry_limit_consumed = False
            continue
        ...
        if rel.endswith("phase8_9r_401_retry_authorization_gate_result.json"):
            if payload.get("retry_limit") != 1:
                policy_violations.append("phase8_9R.retry_limit must be 1")
            if payload.get("retry_allowed") is not True:
                policy_violations.append("phase8_9R.retry_allowed must be true")
            if payload.get("retry_authorized") is not True:
                policy_violations.append("phase8_9R.retry_authorized must be true")
            if payload.get("secret_leak_findings"):
                policy_violations.append("phase8_9R secret leak findings must be empty")

    if policy.get("required_previous_retry_limit_consumed") is True and retry_limit_consumed is False:
        policy_violations.append("retry_limit must be consumed already")
    ...
    result = _build_result(
        ...
        extra={
            ...
            "human_approval_valid": decision == "APPROVE_FREEZE_CLOSURE_DIAGNOSTIC_PLAN_ONLY",
            "human_decision": decision,
            "next_step": policy.get("allowed_next_step_if_ready", "phase8_9f_root_cause_diagnostic_plan_review_only"),
        },
    )
...
def _build_result(
    ...
    result = {
        "phase": "8-9F",
        "phase_name": "401 freeze closure diagnostic plan",
        ...
        "retry_limit": 0,
        "freeze_required": True,
        "wordPress_api_call_not_permitted": True,
        "wordpress_api_call_allowed": False,
        "wordpress_api_call_attempted": False,
        "wordpress_write_allowed": False,
        "wordpress_write_executed": False,
...
def main() -> int:
    result = validate_phase8_9f_401_freeze_closure_diagnostic_plan()
    print(json.dumps(result, ensure_ascii=False, indent=2))
    acceptable = {
        "401_FREEZE_CLOSURE_DIAGNOSTIC_PLAN_READY_NO_EXECUTION",
