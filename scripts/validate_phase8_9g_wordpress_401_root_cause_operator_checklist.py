...
DEFAULT_POLICY = ROOT / "config/phase8_9g_wordpress_401_root_cause_operator_checklist_policy.json"
DEFAULT_REQUEST = ROOT / "exchange/examples/phase8_9g_wordpress_401_root_cause_operator_checklist_request.example.json"
DEFAULT_OUTPUT_JSON = ROOT / "exchange/logs/phase8_9g_wordpress_401_root_cause_operator_checklist_result.json"
...
def validate_phase8_9g_wordpress_401_root_cause_operator_checklist(
    ...
    if not policy_path.exists() or not request_path.exists():
        result = _build_result("ABORT_POLICY_VIOLATION", {}, errors, warnings, ["missing_required_file"], secret_leak_findings, {})
    ...
    checklist = request.get("operator_checklist", {})
    for key in [
        "base_url_public_site",
        "username_is_login_username",
        "app_password_is_application_password",
        "user_has_editor_or_higher_role",
        "rest_api_or_application_password_not_restricted",
        "server_side_basic_auth_or_rest_block_not_active",
    ]:
        if checklist.get(key) is not True:
            policy_violations.append(f"request.operator_checklist.{key} must be true")
    ...
    for rel in previous_paths:
        ...
        if rel.endswith("phase8_9f_401_freeze_closure_diagnostic_plan_result.json"):
            if payload.get("retry_consumed") is not True:
                policy_violations.append("phase8_9F.retry_consumed must be true")
    ...
    result = _build_result(
        ...
        extra={
            ...
            "human_approval_valid": decision == "APPROVE_401_ROOT_CAUSE_OPERATOR_CHECKLIST_ONLY",
            "human_decision": decision,
            "operator_checklist": checklist,
            "investigation_focus": request.get("investigation_focus", []),
            "next_step": policy.get("allowed_next_step_if_ready", "phase8_9g_root_cause_operator_checklist_review_only"),
        },
    )
...
def main() -> int:
    result = validate_phase8_9g_wordpress_401_root_cause_operator_checklist()
