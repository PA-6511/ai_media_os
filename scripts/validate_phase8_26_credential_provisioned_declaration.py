...
DEFAULT_POLICY = ROOT / "config/phase8_26_credential_provisioned_declaration_policy.json"
DEFAULT_REVIEW = ROOT / "exchange/human_review/phase8_26_credential_provisioned_declaration.example.json"
DEFAULT_OUTPUT_JSON = ROOT / "exchange/logs/phase8_26_credential_provisioned_declaration_result.json"
DEFAULT_OUTPUT_MD = ROOT / "exchange/logs/phase8_26_credential_provisioned_declaration_result.md"
...
def validate_credential_provisioned_declaration(
    ...
    for flag in [
        "declaration_is_execution_permission",
    ...
    for rel in policy.get("required_evidence", []):
        ...
        phase8_25_status = status
        if payload.get("secret_values_written") is True:
            safety_violations.append(f"secret_values_written=true in {rel}")

    if phase8_25_status not in set(policy.get("allowed_phase8_25_statuses", [])):
        safety_violations.append(f"phase8_25 status not allowed: {phase8_25_status}")

    if not review_path.exists():
        errors.append(f"missing_human_review: {review_path}")
        result = _build_result("FAIL", credentials, evidence_summary, errors, warnings, safety_violations, policy)
        _write_outputs(result, output_json_path, output_md_path)
        return result
    ...
    for env_name in policy.get("required_env", []):
        credentials[env_name] = {"exists": bool(os.environ.get(env_name, ""))}
    ...
    if safety_violations:
        result = _build_result("ABORT", credentials, evidence_summary, errors, warnings, safety_violations, policy)
        _write_outputs(result, output_json_path, output_md_path)
        return result
    ...
    if decision not in allowed:
    ...
    elif decision == "DECLARE_CREDENTIALS_PROVISIONED_OUTSIDE_REPO":
        status = "CREDENTIALS_DECLARED_PROVISIONED_NO_SECRET_OUTPUT"
    elif decision == "DECLARE_CREDENTIALS_STILL_NOT_READY":
        ...
        status = "FAIL"
        errors.append("operator rejected declaration")
    else:
        status = "ABORT"
...
def _build_result(
    ...
    return {
        ...
        "allowed_next_step": policy.get("allowed_next_step", "Phase 8-27 credential-ready re-evaluation sequence planner"),
        "checked_at": _now_iso(),
    }
...
def _build_markdown(result: dict[str, Any]) -> str:
    ...
    for name, item in result.get("credentials", {}).items():
        lines.append(f"- {name}: exists={item.get('exists')}")
    ...
    lines.extend([
        ...
        f"- declaration_is_execution_permission: {result.get('declaration_is_execution_permission')}",
...
def main() -> int:
    result = validate_credential_provisioned_declaration()
    print(json.dumps(result, ensure_ascii=False, indent=2))
    acceptable = {
        "CREDENTIALS_DECLARED_PROVISIONED_NO_SECRET_OUTPUT",
