#!/usr/bin/env python3
"""Generate Phase 11-9 report for pre-publish NO_GO maintenance."""
...
DEFAULT_OUTPUT_JSON = LOG_DIR / "phase11_9_pre_publish_no_go_maintenance_report.json"
DEFAULT_OUTPUT_MD = LOG_DIR / "phase11_9_pre_publish_no_go_maintenance_report.md"
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase11_9_pre_publish_no_go_maintenance_report",
        "phase": "Phase 11-9",
        "status": "ABORT",
        "reason": reason,
...
def validate_phase11_8(source: dict) -> str | None:
    if source.get("phase") != "Phase 11-8":
        return "phase11_8 input phase must be Phase 11-8"
    if source.get("status") != "PASS":
        return "phase11_8 status must be PASS"
    if source.get("phase11_8_final_confirmation_status") != "PASS":
        return "phase11_8_final_confirmation_status must be PASS"
...
def build_checks(source: dict) -> list[dict]:
    return [
        ...
        {
            "check": "phase11_8_final_confirmation_status=PASS",
            "passed": source.get("phase11_8_final_confirmation_status") == "PASS",
            "actual": source.get("phase11_8_final_confirmation_status"),
        },
        {
...
def build_report(source: dict, input_path: Path) -> dict:
    ...
    return {
        "package_type": "phase11_9_pre_publish_no_go_maintenance_report",
        "phase": "Phase 11-9",
        "title": "Phase 11-9 Pre Publish NO-GO Maintenance Report",
        "status": "PASS" if all_passed else "ABORT",
        "phase11_9_maintenance_status": "PASS" if all_passed else "ABORT",
        ...
        "summary": {
            "phase11_pre_publish_preparation_completed": True,
            "publish_still_not_executed": True,
            "no_go_boundary_maintained": True,
        },
        "next_step": "phase11_10_phase11_overall_completion_report",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 11-9 Pre Publish NO-GO Maintenance Report",
        "",
        ...
        f"- phase11_9_maintenance_status: {report['phase11_9_maintenance_status']}",
        "",
        "## Validation Checks",
        "",
        "| Check | Result |",
...
def generate_report(input_11_8: Path, output_json: Path, output_md: Path) -> dict:
    if not input_11_8.exists():
        return abort_result(f"required input not found: {input_11_8}", output_json)

    source = load_json(input_11_8)
    err = validate_phase11_8(source)
    if err:
        return abort_result(err, output_json)

    report = build_report(source, input_11_8)
    write_json(output_json, report)
    output_md.parent.mkdir(parents=True, exist_ok=True)
    output_md.write_text(build_markdown(report), encoding="utf-8")

    return {
        "package_type": "phase11_9_pre_publish_no_go_maintenance_report_result",
        "phase": "Phase 11-9",
        "status": report["status"],
        "phase11_9_maintenance_status": report["phase11_9_maintenance_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 11-9 pre-publish NO-GO maintenance report"
    )
    parser.add_argument("--input-11-8", type=Path, default=DEFAULT_INPUT_11_8)
    parser.add_argument("--output-json", type=Path, default=DEFAULT_OUTPUT_JSON)
    parser.add_argument("--output-md", type=Path, default=DEFAULT_OUTPUT_MD)
    args = parser.parse_args()

    result = generate_report(args.input_11_8, args.output_json, args.output_md)
