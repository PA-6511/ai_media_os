#!/usr/bin/env python3
"""Validate SVC-4 activation preflight design (design-only / no execution)."""
...
DEFAULT_POLICY = ROOT / "config/phase8_svc_4_activation_preflight_policy.json"
DEFAULT_RUNBOOK = ROOT / "docs/runbooks/phase8_svc_4_activation_preflight_design.md"
DEFAULT_OUTPUT = ROOT / "exchange/logs/phase8_svc_4_activation_preflight_design_result.json"
...
def validate_activation_preflight_design(
    ...
    # In design document, forbidden patterns can appear as prohibition statements.
    # Only treat as hit if a line looks like an executable command.
    for pattern in policy.get("forbidden_patterns", []):
        for line in runbook_text.splitlines():
            ...
            if pattern.lower() in normalized and not normalized.startswith("-"):
                if normalized.startswith("systemctl ") or normalized.startswith("requests.") or normalized.startswith("curl "):
    ...
    if forbidden_hits:
        ...
        status = "PASS_DESIGN_ONLY_NO_ACTIVATION"
...
def _build_result(
    ...
    return {
        "phase": "8-SVC-4",
        "phase_name": "Activation Preflight Design / No Activation",
        "status": status,
        ...
        "allowed_next_step": policy.get(
            "allowed_next_step",
            "SVC-5 activation preflight execution after SVC-3 pass",
        ),
        "checked_at": _now_iso(),
    }
...
def main() -> int:
    result = validate_activation_preflight_design()
    print(json.dumps(result, ensure_ascii=False, indent=2))
    return 0 if result.get("status") == "PASS_DESIGN_ONLY_NO_ACTIVATION" else 2
