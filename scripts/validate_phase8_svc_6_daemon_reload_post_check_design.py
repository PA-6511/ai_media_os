#!/usr/bin/env python3
"""Validate SVC-6 daemon-reload post-check design (design-only / no activation)."""
...
DEFAULT_POLICY = ROOT / "config/phase8_svc_6_daemon_reload_post_check_policy.json"
DEFAULT_RUNBOOK = ROOT / "docs/runbooks/phase8_svc_6_daemon_reload_post_check_design.md"
DEFAULT_OUTPUT = ROOT / "exchange/logs/phase8_svc_6_daemon_reload_post_check_design_result.json"
...
def validate_daemon_reload_post_check_design(
    ...
    # Design doc may mention forbidden strings as prohibition bullets.
    ...
    if forbidden_hits:
        ...
        status = "PASS_DESIGN_ONLY_DAEMON_RELOAD_POST_CHECK"
...
def _build_result(
    ...
    return {
        "phase": "8-SVC-6",
        "phase_name": "Daemon-Reload Post-Check Design",
        "status": status,
        ...
        "allowed_next_step": policy.get(
            "allowed_next_step",
            "Controlled daemon-reload execution checkpoint (no enable/start)",
        ),
        "checked_at": _now_iso(),
    }
...
def main() -> int:
    result = validate_daemon_reload_post_check_design()
    print(json.dumps(result, ensure_ascii=False, indent=2))
    return 0 if result.get("status") == "PASS_DESIGN_ONLY_DAEMON_RELOAD_POST_CHECK" else 2
