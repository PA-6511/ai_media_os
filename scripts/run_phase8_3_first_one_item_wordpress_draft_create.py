...
DEFAULT_POLICY = ROOT / "config/phase8_3_first_one_item_wordpress_draft_create_policy.json"
DEFAULT_REQUEST = ROOT / "exchange/examples/phase8_2_final_live_preflight_request.example.json"
DEFAULT_OUTPUT_JSON = ROOT / "exchange/logs/phase8_3_first_one_item_wordpress_draft_create_result.json"
DEFAULT_OUTPUT_MD = ROOT / "exchange/logs/phase8_3_first_one_item_wordpress_draft_create_result.md"
...
def run_first_one_item_draft_create(
    ...
    if request.get("approval_token") != "APPROVE_DRAFT_CREATE_ONLY":
        safety_violations.append("request.approval_token must be APPROVE_DRAFT_CREATE_ONLY")
    ...
    if not safety_violations:
        ...
        missing_env = [key for key, value in env_values.items() if not str(value).strip()]
        ...
        if missing_env:
            ...
            if request.get("target_item_count") != 1:
                safety_violations.append("payload generation blocked because target_item_count is not 1")
            ...
            if safety_violations:
                ...
                try:
                    ...
                    if resp.status_code not in {200, 201}:
                        status = "DRAFT_CREATE_FAILED_FREEZE_REQUIRED"
                        freeze_required = True
                        errors.append(f"unexpected_status_code: {resp.status_code}")
                    else:
                        data = resp.json()
                        post_status_value = str(data.get("status", ""))
                        if post_status_value.lower() in {"publish", "published"}:
                            status = "ABORT"
                            freeze_required = True
                        ...
                        elif post_status_value != "draft":
                            status = "DRAFT_CREATE_FAILED_FREEZE_REQUIRED"
                            freeze_required = True
                            errors.append(f"response status must be draft but got {post_status_value}")
                        else:
                            link = data.get("link")
                            guid = data.get("guid")
...
def main() -> int:
    result = run_first_one_item_draft_create()
    print(json.dumps(result, ensure_ascii=False, indent=2))
    return 0 if result.get("status") in {
        "DRAFT_CREATED_PENDING_HUMAN_REVIEW",
        "NOT_EXECUTED_MISSING_CREDENTIALS",
        "DRAFT_CREATE_FAILED_FREEZE_REQUIRED",
    } else 2
