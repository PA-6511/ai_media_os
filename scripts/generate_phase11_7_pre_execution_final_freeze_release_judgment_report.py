#!/usr/bin/env python3
"""Generate Phase 11-7 report for pre-execution final freeze release judgment."""
...
DEFAULT_INPUT_11_6 = LOG_DIR / "phase11_6_execute_live_authorization_gate_report.json"
DEFAULT_OUTPUT_JSON = (
    LOG_DIR / "phase11_7_pre_execution_final_freeze_release_judgment_report.json"
)
DEFAULT_OUTPUT_MD = (
    LOG_DIR / "phase11_7_pre_execution_final_freeze_release_judgment_report.md"
)
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase11_7_pre_execution_final_freeze_release_judgment_report",
        "phase": "Phase 11-7",
        "status": "ABORT",
        "reason": reason,
...
def validate_phase11_6(source: dict) -> str | None:
    if source.get("phase") != "Phase 11-6":
        return "phase11_6 input phase must be Phase 11-6"
    if source.get("status") != "PASS":
        return "phase11_6 status must be PASS"
    if source.get("phase11_6_gate_status") != "PASS":
        return "phase11_6_gate_status must be PASS"
...
def build_checks(source: dict) -> list[dict]:
    return [
        ...
        {
            "check": "phase11_6_gate_status=PASS",
            "passed": source.get("phase11_6_gate_status") == "PASS",
            "actual": source.get("phase11_6_gate_status"),
        },
        {
...
def build_report(source: dict, input_path: Path) -> dict:
    ...
    return {
        "package_type": "phase11_7_pre_execution_final_freeze_release_judgment_report",
        "phase": "Phase 11-7",
        "title": "Phase 11-7 Pre Execution Final Freeze Release Judgment Report",
        "status": "PASS" if all_passed else "ABORT",
        "phase11_7_judgment_status": "PASS" if all_passed else "ABORT",
        ...
        "summary": {
            "freeze_release_judgment_designed": True,
            "final_judgment_is_keep_no_go": True,
            "publish_still_not_executed": True,
        },
        "next_step": "phase11_8_post_execution_evidence_and_relock_final_confirmation",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 11-7 Pre Execution Final Freeze Release Judgment Report",
        "",
        ...
        f"- phase11_7_judgment_status: {report['phase11_7_judgment_status']}",
        f"- phase11_7_final_judgment: {report['phase11_7_final_judgment']}",
        "",
        "## Validation Checks",
        "",
        "| Check | Result |",
...
def generate_report(input_11_6: Path, output_json: Path, output_md: Path) -> dict:
    if not input_11_6.exists():
        return abort_result(f"required input not found: {input_11_6}", output_json)

    source = load_json(input_11_6)
    err = validate_phase11_6(source)
    if err:
        return abort_result(err, output_json)

    report = build_report(source, input_11_6)
    write_json(output_json, report)
    output_md.parent.mkdir(parents=True, exist_ok=True)
    output_md.write_text(build_markdown(report), encoding="utf-8")

    return {
        "package_type": "phase11_7_pre_execution_final_freeze_release_judgment_report_result",
        "phase": "Phase 11-7",
        "status": report["status"],
        "phase11_7_judgment_status": report["phase11_7_judgment_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 11-7 pre-execution final freeze release judgment report"
    )
    parser.add_argument("--input-11-6", type=Path, default=DEFAULT_INPUT_11_6)
    parser.add_argument("--output-json", type=Path, default=DEFAULT_OUTPUT_JSON)
    parser.add_argument("--output-md", type=Path, default=DEFAULT_OUTPUT_MD)
    args = parser.parse_args()

    result = generate_report(args.input_11_6, args.output_json, args.output_md)
