...
def json_object(path: Path) -> dict[str, Any]:
    require(path.is_file(), f"JSON_FILE_MISSING:{path}")
    require(not path.is_symlink(), f"JSON_FILE_SYMLINK:{path}")
...
def validate_candidate(
    ...
    require(
        links == 0 and other == 0,
        f"CANDIDATE_SPECIAL:{root}",
    )
    require(
        len(directories) == 0,
        f"CANDIDATE_DIRECTORY_COUNT:{len(directories)}",
    )
    require(
        len(files) == expected_files,
        f"CANDIDATE_FILE_COUNT:{len(files)}",
    )

    manifest_path = root / "candidate-manifest.json"
    mapping = json_manifest_map(manifest_path)

    require(
        len(mapping) == expected_entries,
        f"CANDIDATE_ENTRY_COUNT:{len(mapping)}",
    )
    require(
        set(mapping) == files - {"candidate-manifest.json"},
        "CANDIDATE_EXACT_SET",
    )
    ...
    for relative, entry in mapping.items():
        ...
        require(
            path.is_file(),
            f"CANDIDATE_PAYLOAD_MISSING:{relative}",
        )
        require(
            not path.is_symlink(),
            f"CANDIDATE_PAYLOAD_SYMLINK:{relative}",
        )
        require(
...
runner_map = validate_candidate(runner_root, 7, 6)
