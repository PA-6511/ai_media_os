...
EVIDENCE_REL="${BASE_REL}/i2f3e-i-writer-freeze-backup-execution-packet-prep-${RUN_ID}"
...
required_inputs=(
  "$E_ROOT/result.json"
  "$E_ROOT/packet-snapshot/packet-manifest.json"
  "$E_ROOT/evidence-manifest.txt"
...
echo "${EXPECTED_E_PACKET_MANIFEST_SHA}  ${E_ROOT}/packet-snapshot/packet-manifest.json" | sha256sum -c -
echo "${EXPECTED_E_EVIDENCE_MANIFEST_SHA}  ${E_ROOT}/evidence-manifest.txt" | sha256sum -c -
...
semantic_validation = {
    "schema_version": "1.0",
    "phase": phase,
    "result": "PASS_3E_I_WRITER_FREEZE_BACKUP_EXECUTION_PACKET_PREPARATION_NO_EXECUTION",
    "process_state": process_state,
    "matching_process_count": len(pids),
...
packet_manifest = {
    ...
    "result": "PASS_3E_I_WRITER_FREEZE_BACKUP_EXECUTION_PACKET_GENERATED",
    ...
    "production_release_decision": "HOLD",
    "release_status": "CANDIDATE_NOT_APPROVED",
    "next_gate": "HUMAN_REVIEW_3E_I_WRITER_FREEZE_BACKUP_EXECUTION_PACKET",
}
...
result = {
    ...
    "result": "PASS_W2B_I2F3E_I_WRITER_FREEZE_BACKUP_EXECUTION_PACKET_PREPARED_NO_EXECUTION",
