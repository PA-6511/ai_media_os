...
bundle_review = bundle_result.get("review_bundle")
if not isinstance(bundle_review, dict):
    raise SystemExit("BUNDLE_RESULT_REVIEW_BUNDLE_INVALID")
if bundle_review.get("candidate_id") != expected_candidate_id:
    raise SystemExit("BUNDLE_RESULT_CANDIDATE_ID_MISMATCH")
if bundle_review.get("review_bundle_id") != expected_review_bundle_id:
