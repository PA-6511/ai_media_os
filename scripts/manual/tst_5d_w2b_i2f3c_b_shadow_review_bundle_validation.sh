...
REVIEW_SHADOW_ROOT="/tmp/tst-5d-w2b-i2f3c-a-review-shadow-20260725T154150-490803"
...
REVIEW_SHADOW_MANIFEST="${REVIEW_SHADOW_ROOT}/config/slack_worker_release_source_manifest.json"
...
write_failure_result() {
    ...
    local review_manifest_after candidate_after closure_after downstream_after db_after
    ...
    review_manifest_after="$(sha256_file "$REVIEW_SHADOW_MANIFEST")"
    ...
    cat > "$RESULT_JSON" <<JSON
{
  ...
  "safety": {
    ...
    "review_shadow_manifest_changed": $([[ "$review_manifest_after" == "$EXPECTED_SHADOW_MANIFEST_SHA" ]] && echo false || echo true),
...
verify_snapshot "$REVIEW_SHADOW_ROOT" "$C_A_SNAPSHOT_MANIFEST"

RUN_ID="$(date +%Y%m%dT%H%M%S)-$$"
CREATED_AT="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
BUNDLE_PARENT="/tmp/tst-5d-w2b-i2f3c-b-review-output-${RUN_ID}"
EVIDENCE_REL="${I2E_ROOT_REL}/i2f3c-b-review-bundle-validation-${RUN_ID}"
EVIDENCE_ROOT="${REPO_ROOT}/${EVIDENCE_REL}"

if [[ -e "$BUNDLE_PARENT" ]]; then
    printf 'ERROR=BUNDLE_PARENT_EXISTS:%s\n' "$BUNDLE_PARENT" >&2
    exit 1
fi
...
mkdir -p "$BUNDLE_PARENT" "$EVIDENCE_ROOT/bundle-snapshot"
...
REVIEW_SHADOW_MANIFEST_AFTER="$(sha256_file "$REVIEW_SHADOW_MANIFEST")"
CANDIDATE_AFTER="$(sha256_file "$CANDIDATE_PATH")"
CLOSURE_AFTER="$(sha256_file "$CLOSURE_PATH")"
DOWNSTREAM_AFTER="$(sha256_file "$DOWNSTREAM_PATH")"
DB_AFTER="$(sha256_file "$DB_PATH")"
...
[[ "$REVIEW_SHADOW_MANIFEST_AFTER" == "$EXPECTED_SHADOW_MANIFEST_SHA" ]] || {
    printf 'ERROR=REVIEW_SHADOW_MANIFEST_CHANGED\n' >&2
    exit 1
}
...
printf 'REVIEW_SHADOW_CHANGED=false\n'
