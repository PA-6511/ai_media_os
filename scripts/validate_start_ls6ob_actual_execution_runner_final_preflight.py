...
STATUS_PASS = "LS6OB_ACTUAL_EXECUTION_RUNNER_FINAL_PREFLIGHT_VALIDATED_NO_WRITE"
STATUS_NOT_READY = "LS6OB_ACTUAL_EXECUTION_RUNNER_FINAL_PREFLIGHT_VALIDATION_NOT_READY"
STATUS_NOT_READY_MISSING_GO = "LS6OB_ACTUAL_EXECUTION_RUNNER_FINAL_PREFLIGHT_VALIDATION_NOT_READY_MISSING_ACTUAL_GO"
...
def build_result(status: str, errors: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "runner_final_preflight_validated": ok,
        "actual_wordpress_go_ready": ok,
        "actual_wordpress_go_consumed": False,
        "one_shot_actual_execution_lock_active": ok,
...
def write_report(result: dict[str, Any], report_path: Path) -> None:
    lines = [
        "# LS-6O-B Actual Execution Runner Final Preflight Gate Validation Report",
        "",
        f"- generated_at: {result['generated_at']}",
        f"- status: {result['status']}",
        f"- execution_mode: {result['execution_mode']}",
        f"- production_status: {result['production_status']}",
        f"- runner_final_preflight_validated: {result['runner_final_preflight_validated']}",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-result", default="exchange/logs/start_ls6ob_actual_execution_runner_final_preflight_gate_result.json")
    parser.add_argument("--preflight-result", default="exchange/runtime/start_ls6ob_actual_execution_runner_final_preflight_result.json")
    ...
    parser.add_argument("--output", default="exchange/logs/start_ls6ob_actual_execution_runner_final_preflight_gate_validation_result.json")
    parser.add_argument("--report", default="reports/start_ls6ob_actual_execution_runner_final_preflight_gate_validation_report.md")
...
def main() -> int:
    ...
    missing_go = not ls6oa_ready_path.exists() or not ls6oa_go_path.exists()
    ...
    require(run_result.get("status") == "LS6OB_ACTUAL_EXECUTION_RUNNER_FINAL_PREFLIGHT_PASSED_NO_WRITE", "run result status mismatch", errors)
    ...
    require(final_execution_preflight.get("status") == "FINAL_EXECUTION_PREFLIGHT_PASSED_NO_WORDPRESS_WRITE", "final execution preflight status mismatch", errors)
    ...
    require(runtime_freeze_lock.get("locked") is True, "runtime freeze lock locked must be true", errors)
    ...
    require(preflight.get("status") == "ACTUAL_EXECUTION_RUNNER_FINAL_PREFLIGHT_PASSED_NO_WRITE", "preflight status mismatch", errors)
    require(preflight.get("actual_wordpress_go_ready") is True, "preflight actual_wordpress_go_ready must be true", errors)
    require(preflight.get("actual_wordpress_go_consumed") is False, "preflight actual_wordpress_go_consumed must be false", errors)
    require(preflight.get("runner_final_preflight_passed") is True, "preflight runner_final_preflight_passed must be true", errors)
    require(preflight.get("one_shot_actual_execution_lock_active") is True, "preflight one_shot_actual_execution_lock_active must be true", errors)
    require(preflight.get("one_shot_actual_execution_lock_consumed") is False, "preflight one_shot_actual_execution_lock_consumed must be false", errors)
    ...
    require(preflight.get("runtime_freeze_restored") is False, "preflight runtime_freeze_restored must be false", errors)
    ...
    validate_false_fields(
        ...
        [
            ...
            "actual_wordpress_go_consumed_by_this_phase",
            "one_shot_actual_execution_lock_consumed_by_this_phase",
            "runtime_freeze_restored_by_this_phase",
            "runner_executed",
    ...
    require(run_result.get("next_phase", {}).get("phase") == "LS-6O-C", "next_phase.phase must be LS-6O-C", errors)
