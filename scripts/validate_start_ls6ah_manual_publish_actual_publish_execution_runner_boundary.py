...
STATUS_VALIDATED = "LS6AH_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_VALIDATED_NO_PUBLISH"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--actual-publish-execution-runner-boundary-result", default="exchange/runtime/start_ls6ah_manual_publish_actual_publish_execution_runner_boundary_result.json")
    parser.add_argument("--actual-publish-execution-runner-boundary-lock", default="exchange/locks/start_ls6ah_manual_publish_actual_publish_execution_runner_boundary.lock.json")
    parser.add_argument("--run-result", default="exchange/logs/start_ls6ah_manual_publish_actual_publish_execution_runner_boundary_result.json")
    ...
    parser.add_argument("--output", default="exchange/logs/start_ls6ah_manual_publish_actual_publish_execution_runner_boundary_validation_result.json")
    parser.add_argument("--report", default="reports/start_ls6ah_manual_publish_actual_publish_execution_runner_boundary_validation_report.md")
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# LS-6AH Manual Publish Actual Publish Execution Runner Boundary Validation Report",
        "",
...
def build_result(run: dict[str, Any], errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "ls6ag_final_execution_command_validated": bool(run.get("ls6ag_final_execution_command_validated", False)),
        ...
        "requires_actual_publish_execution_runner_final_preflight": bool(run.get("requires_actual_publish_execution_runner_final_preflight", False)),
...
def main() -> int:
    ...
    runtime = try_load_json(Path(args.actual_publish_execution_runner_boundary_result), errors)
    lock = try_load_json(Path(args.actual_publish_execution_runner_boundary_lock), errors)
    ...
    req(run.get("status") == "LS6AH_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_PASSED_NO_PUBLISH", "run status mismatch", errors)
    req(runtime.get("status") == "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_READY_NO_PUBLISH", "runtime status mismatch", errors)
    req(lock.get("status") == "MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_LOCKED_NO_PUBLISH", "lock status mismatch", errors)
    ...
    req(ls6ag_ready.get("requires_actual_publish_execution_runner_boundary") is True, "LS-6AG requires runner boundary must be true", errors)
    ...
    req(safe_get(ls6ag_command, "actual_publish_final_execution_command", "actual_publish_final_execution_command_label") == "FINAL_COMMAND_FOR_ACTUAL_PUBLISH_EXECUTION_RUNNER_GATE_ONLY", "LS-6AG command result label mismatch", errors)
    ...
    req(ls6af_runtime.get("actual_publish_runner_final_gate_consumed") is False, "LS-6AF runner gate consumed must be false", errors)
    req(ls6af_lock.get("actual_publish_runner_final_gate_consumed") is False, "LS-6AF lock runner gate consumed must be false", errors)
    ...
    req(run.get("status") == "LS6AH_MANUAL_PUBLISH_ACTUAL_PUBLISH_EXECUTION_RUNNER_BOUNDARY_PASSED_NO_PUBLISH", "run status mismatch 2", errors)
    ...
    req(run.get("ls6ag_final_execution_command_validated") is True, "run.ls6ag_final_execution_command_validated must be true", errors)
    ...
    req(run.get("requires_actual_publish_execution_runner_final_preflight") is True, "run requires_actual_publish_execution_runner_final_preflight must be true", errors)
    ...
    req(lock.get("requires_next_phase") == "LS-6AI", "lock.requires_next_phase mismatch", errors)

    next_phase = run.get("next_phase", {})
    req(safe_get(next_phase, "phase") == "LS-6AI", "run.next_phase.phase mismatch", errors)
    ...
    req(safe_get(next_phase, "requires_actual_publish_execution_runner_final_preflight") is True, "run.next_phase.requires_actual_publish_execution_runner_final_preflight must be true", errors)
