#!/usr/bin/env python3
"""Generate Phase 14-6 report for execute-live pre-command final dry-run audit."""
...
DEFAULT_OUTPUT_MD = (
    LOG_DIR / "phase14_6_execute_live_pre_command_final_dry_run_audit_report.md"
)
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase14_6_execute_live_pre_command_final_dry_run_audit_report",
        "phase": "Phase 14-6",
        "status": "ABORT",
        "reason": reason,
...
def validate_phase14_5(source: dict) -> str | None:
    if source.get("phase") != "Phase 14-5":
        return "phase14_5 input phase must be Phase 14-5"
    if source.get("status") != "PASS":
        return "phase14_5 status must be PASS"
    if source.get("phase14_5_design_status") != "PASS":
        return "phase14_5_design_status must be PASS"
...
def build_report(source: dict, input_path: Path) -> dict:
    return {
        "package_type": "phase14_6_execute_live_pre_command_final_dry_run_audit_report",
        "phase": "Phase 14-6",
        "title": "Phase 14-6 Execute Live Pre Command Final Dry Run Audit Report",
        "status": "PASS",
        ...
        "dry_run_audit": {
            "mode": "DRY_RUN_ONLY",
            "execute_live_payload_schema_checked": True,
        ...
        "summary": {
            "execute_live_pre_command_final_dry_run_audit_completed": True,
            "publish_still_not_executed": True,
            "no_go_boundary_maintained": True,
        },
        "next_step": "phase14_7_execute_live_authorization_gate",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 14-6 Execute Live Pre Command Final Dry Run Audit Report",
        "",
        f"Generated: {report['created_at']}",
        "",
        f"- status: {report['status']}",
        f"- phase14_6_audit_status: {report['phase14_6_audit_status']}",
...
def generate_report(input_14_5: Path, output_json: Path, output_md: Path) -> dict:
    if not input_14_5.exists():
        return abort_result(f"required input not found: {input_14_5}", output_json)

    source = load_json(input_14_5)
    err = validate_phase14_5(source)
    if err:
        return abort_result(err, output_json)

    report = build_report(source, input_14_5)
    write_json(output_json, report)
    output_md.parent.mkdir(parents=True, exist_ok=True)
    output_md.write_text(build_markdown(report), encoding="utf-8")

    return {
        "package_type": "phase14_6_execute_live_pre_command_final_dry_run_audit_report_result",
        "phase": "Phase 14-6",
        "status": report["status"],
        "phase14_6_audit_status": report["phase14_6_audit_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 14-6 execute-live pre-command final dry-run audit report"
    )
    parser.add_argument("--input-14-5", type=Path, default=DEFAULT_INPUT_14_5)
    parser.add_argument("--output-json", type=Path, default=DEFAULT_OUTPUT_JSON)
    parser.add_argument("--output-md", type=Path, default=DEFAULT_OUTPUT_MD)
    args = parser.parse_args()

    result = generate_report(args.input_14_5, args.output_json, args.output_md)
