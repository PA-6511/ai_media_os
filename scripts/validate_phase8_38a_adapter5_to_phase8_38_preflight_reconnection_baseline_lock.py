#!/usr/bin/env python3
"""Phase8-38A: Adapter5->Phase8-38 preflight reconnection baseline lock validator."""
...
POLICY = ROOT / "config/phase8_38a_adapter5_to_phase8_38_preflight_reconnection_baseline_lock_policy.json"
REQUEST = ROOT / "exchange/examples/phase8_38a_adapter5_to_phase8_38_preflight_reconnection_baseline_lock_request.example.json"
OUTPUT = ROOT / "exchange/logs/phase8_38a_adapter5_to_phase8_38_preflight_reconnection_baseline_lock_result.json"
...
def validate(
    ...
    base = {
        "phase": "8-38A",
        "phase_name": "Adapter5 to Phase8-38 Preflight Reconnection Baseline Lock",
        ...
        "checked_at": _now_iso(),
        "status": "PHASE8_38A_RECONNECTION_BASELINE_LOCK_ABORT_NO_EXECUTION",
        "policy_violations": [],
        "missing_reports": [],
        "status_mismatches": [],
        "invariant_failures": [],
    ...
    adapter6 = loaded_reports.get("adapter6_lock", {})
    if isinstance(adapter6, dict):
        if adapter6.get("baseline_locked") is not True:
            invariant_failures.append("adapter6.baseline_locked must be true")
        if adapter6.get("hold_state") != "HOLD":
            invariant_failures.append("adapter6.hold_state must be HOLD")

    for report_key in ("adapter5_result", "phase8_38_from_adapter5"):
    ...
    if missing_reports:
        status = "PHASE8_38A_RECONNECTION_BASELINE_LOCK_ABORT_MISSING_EVIDENCE_NO_EXECUTION"
    elif mismatches:
        status = "PHASE8_38A_RECONNECTION_BASELINE_LOCK_ABORT_STATUS_MISMATCH_NO_EXECUTION"
    elif violations:
        status = "PHASE8_38A_RECONNECTION_BASELINE_LOCK_ABORT_POLICY_VIOLATION_NO_EXECUTION"

    result = {
        **base,
        "status": status,
        "baseline_locked": status == "PHASE8_38A_ADAPTER5_TO_PHASE8_38_RECONNECTION_BASELINE_LOCK_PASS_HOLD_NO_EXECUTION",
        ...
        "next_action": "HOLD_OR_PHASE8_39_DRY_RUN" if status.endswith("PASS_HOLD_NO_EXECUTION") else "FIX_EVIDENCE_OR_POLICY_AND_REVALIDATE",
    }
...
def main() -> int:
    ...
    return 0 if result.get("status") == "PHASE8_38A_ADAPTER5_TO_PHASE8_38_RECONNECTION_BASELINE_LOCK_PASS_HOLD_NO_EXECUTION" else 2
