#!/usr/bin/env python3
"""Generate Phase 11-4 report for pre-publish final NO_GO/GO judgment design."""
...
DEFAULT_OUTPUT_MD = (
    LOG_DIR / "phase11_4_pre_publish_final_no_go_or_go_judgment_design_report.md"
)
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase11_4_pre_publish_final_no_go_or_go_judgment_design_report",
        "phase": "Phase 11-4",
        "status": "ABORT",
        "reason": reason,
        "judgment_design_report_generated": False,
...
def validate_phase11_3(source: dict) -> str | None:
    if source.get("phase") != "Phase 11-3":
        return "phase11_3 input phase must be Phase 11-3"
    if source.get("status") != "PASS":
        return "phase11_3 status must be PASS"
    if source.get("phase11_3_plan_status") != "PASS":
        return "phase11_3_plan_status must be PASS"
...
def build_checks(source: dict) -> list[dict]:
    return [
        ...
        {
            "check": "phase11_3_plan_status=PASS",
            "passed": source.get("phase11_3_plan_status") == "PASS",
            "actual": source.get("phase11_3_plan_status"),
        },
        {
...
def build_report(source: dict, input_path: Path) -> dict:
    ...
    gate_conditions = [
        "phase11_3_plan_status_pass",
        "all_no_go_boundary_keys_unchanged",
        "single_target_draft_is_still_draft",
        "final_dry_run_confirmation_ready",
    ]

    return {
        "package_type": "phase11_4_pre_publish_final_no_go_or_go_judgment_design_report",
        "phase": "Phase 11-4",
        "title": "Phase 11-4 Pre Publish Final NO-GO/GO Judgment Design Report",
        "status": "PASS" if all_passed else "ABORT",
        "phase11_4_judgment_design_status": "PASS" if all_passed else "ABORT",
        ...
        "final_judgment_design": final_judgment_design,
        "gate_conditions": gate_conditions,
        "still_forbidden": STILL_FORBIDDEN,
        "validation_checks": checks,
        "validation_passed": all_passed,
        "summary": {
            "final_judgment_gate_designed": True,
            "publish_still_not_executed": True,
            "no_go_boundary_maintained": True,
        },
        "next_step": "phase11_5_publish_command_final_dry_run_confirmation",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        ...
        f"- phase11_4_judgment_design_status: {report['phase11_4_judgment_design_status']}",
        "",
        "## Final Judgment Design",
        "",
    ]

    for key, value in report["final_judgment_design"].items():
...
def generate_report(input_11_3: Path, output_json: Path, output_md: Path) -> dict:
    if not input_11_3.exists():
        return abort_result(f"required input not found: {input_11_3}", output_json)

    source = load_json(input_11_3)
    err = validate_phase11_3(source)
    if err:
        return abort_result(err, output_json)

    report = build_report(source, input_11_3)
    write_json(output_json, report)
    output_md.parent.mkdir(parents=True, exist_ok=True)
    output_md.write_text(build_markdown(report), encoding="utf-8")

    return {
        "package_type": "phase11_4_pre_publish_final_no_go_or_go_judgment_design_report_result",
        "phase": "Phase 11-4",
        "status": report["status"],
        "phase11_4_judgment_design_status": report["phase11_4_judgment_design_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 11-4 final NO-GO/GO judgment design report"
    )
    parser.add_argument("--input-11-3", type=Path, default=DEFAULT_INPUT_11_3)
    parser.add_argument("--output-json", type=Path, default=DEFAULT_OUTPUT_JSON)
    parser.add_argument("--output-md", type=Path, default=DEFAULT_OUTPUT_MD)
    args = parser.parse_args()

    result = generate_report(args.input_11_3, args.output_json, args.output_md)
