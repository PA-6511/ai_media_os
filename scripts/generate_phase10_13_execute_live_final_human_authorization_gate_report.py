#!/usr/bin/env python3
"""Generate Phase 10-13 report for execute-live final human authorization gate."""
...
DEFAULT_OUTPUT_MD = (
    LOG_DIR
    / "phase10_13_execute_live_final_human_authorization_gate_report.md"
)
...
def validate_phase10_12(source: dict) -> str | None:
    if source.get("phase") != "Phase 10-12":
        return "phase10_12 input phase must be Phase 10-12"
    if source.get("status") != "PASS":
        return "phase10_12 input status must be PASS"
    if source.get("current_decision") != "KEEP_NO_GO":
        return "phase10_12 current_decision must be KEEP_NO_GO"
    if source.get("publish_candidate_unlocked_for_operator") is not False:
        return "phase10_12 publish_candidate_unlocked_for_operator must be false"
    if source.get("wordpress_publish_execution") != "NO_GO":
        return "phase10_12 wordpress_publish_execution must be NO_GO"
    if source.get("wordpress_write_executed") is not False:
        return "phase10_12 wordpress_write_executed must be false"
    if source.get("production_status") != "NO_GO":
        return "phase10_12 production_status must be NO_GO"
    if source.get("wordpress_draft_id") != 110:
        return "phase10_12 wordpress_draft_id must be 110"
    if source.get("target_draft_status") != "draft":
        return "phase10_12 target_draft_status must be draft"

    review = source.get("manual_live_execution_readiness_review", {})
    if review.get("readiness_review_passed") is not True:
        return "phase10_12 readiness_review_passed must be true"
...
def build_checks(source: dict) -> list[dict]:
    return [
        {"check": "phase10_12_status=PASS", "passed": source.get("status") == "PASS", "actual": source.get("status")},
        {
        ...
        {
            "check": "readiness_review_passed=true",
            "passed": source.get("manual_live_execution_readiness_review", {}).get("readiness_review_passed") is True,
            "actual": source.get("manual_live_execution_readiness_review", {}).get("readiness_review_passed"),
        },
    ]
...
def build_report(source: dict, input_path: Path) -> dict:
    ...
    execute_live_final_human_authorization_gate = {
        ...
        "authorization_checks": [
            "single_operator_identity_verified",
            "target_draft_id_is_110",
            "target_draft_status_is_draft",
            "authorization_token_exact_match",
            "authorization_token_not_expired",
            "single_publish_limit_still_one",
            "publish_not_executed_in_phase10_13",
        ],
        "publish_execution_in_phase10_13": "NO_GO",
        "wordpress_write_executed_in_phase10_13": False,
    }

    authorization_gate_outcomes = [
        {
            "authorization_result": "AUTHORIZED_FOR_PHASE10_14_JUDGMENT_ONLY",
            "publish_execution_in_phase10_13": "NO_GO",
            "wordpress_write_executed_in_phase10_13": False,
            "next_step": "phase10_14_pre_execution_final_freeze_release_judgment",
        },
        {
            "authorization_result": "DENIED_KEEP_NO_GO",
            "publish_execution_in_phase10_13": "NO_GO",
            "wordpress_write_executed_in_phase10_13": False,
    ...
    return {
        "package_type": "phase10_13_execute_live_final_human_authorization_gate_report",
        "phase": "Phase 10-13",
        "title": "Phase 10-13 Execute Live Final Human Authorization Gate Report",
        "status": "PASS" if all_passed else "ABORT",
        "phase10_13_gate_status": "PASS" if all_passed else "ABORT",
        "gate_scope": "execute_live_final_human_authorization_gate_only",
        ...
        "wordpress_write_executed": False,
        "production_status": "NO_GO",
        "execute_live_final_human_authorization_gate": execute_live_final_human_authorization_gate,
        "authorization_gate_outcomes": authorization_gate_outcomes,
        "still_forbidden": STILL_FORBIDDEN,
        "validation_checks": checks,
        "validation_passed": all_passed,
        "summary": {
            "execute_live_final_human_authorization_gate_fixed": True,
            "publish_still_not_executed_in_phase10_13": True,
            "wordpress_write_still_not_executed_in_phase10_13": True,
        },
        "next_step": "phase10_14_pre_execution_final_freeze_release_judgment",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        ...
        f"- phase10_13_gate_status: {report['phase10_13_gate_status']}",
        "",
        "## Execute Live Final Human Authorization Gate",
        "",
    ]

    for key, value in report["execute_live_final_human_authorization_gate"].items():
...
def generate_report(input_10_12: Path, output_json: Path, output_md: Path) -> dict:
    if not input_10_12.exists():
        return abort_result(f"required input not found: {input_10_12}", output_json)

    source = load_json(input_10_12)
    err = validate_phase10_12(source)
    if err:
        return abort_result(err, output_json)

    report = build_report(source, input_10_12)
    ...
    return {
        "package_type": "phase10_13_execute_live_final_human_authorization_gate_report_result",
        "phase": "Phase 10-13",
        "status": report["status"],
        "phase10_13_gate_status": report["phase10_13_gate_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 10-13 execute-live final human authorization gate report"
    )
    parser.add_argument("--input-10-12", type=Path, default=DEFAULT_INPUT_10_12)
    parser.add_argument("--output-json", type=Path, default=DEFAULT_OUTPUT_JSON)
    parser.add_argument("--output-md", type=Path, default=DEFAULT_OUTPUT_MD)
    args = parser.parse_args()

    result = generate_report(args.input_10_12, args.output_json, args.output_md)
