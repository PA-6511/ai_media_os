...
DEFAULT_POLICY = ROOT / "config/phase8_30_final_operator_rerun_handoff_policy.json"
DEFAULT_OUTPUT_JSON = ROOT / "exchange/logs/phase8_30_final_operator_rerun_handoff_report.json"
DEFAULT_OUTPUT_MD = ROOT / "exchange/logs/phase8_30_final_operator_rerun_handoff_report.md"
...
def generate_final_operator_rerun_handoff_report(
    ...
    for flag in [
        "handoff_is_execution_permission",
    ...
    if missing_evidence:
        status = "OPERATOR_HANDOFF_NOT_READY"
    elif safety_violations:
        status = "ABORT"
    elif all_ready:
        planned_manual_commands = list(policy.get("planned_manual_commands_if_ready", []))
        status = "OPERATOR_HANDOFF_READY_FOR_PHASE8_6_TO_8_10_RERUN_BUT_NOT_EXECUTED"
    elif any((v in blocked_statuses) for v in phase_statuses.values()):
...
def _build_result(
    ...
    if status == "OPERATOR_HANDOFF_READY_FOR_PHASE8_6_TO_8_10_RERUN_BUT_NOT_EXECUTED":
        allowed_next_step = policy.get("allowed_next_step_if_ready", "")
    elif status == "OPERATOR_HANDOFF_BLOCKED_CREDENTIALS_MISSING":
...
def main() -> int:
    result = generate_final_operator_rerun_handoff_report()
