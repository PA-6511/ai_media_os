...
STATUS_TEMPLATE_READY = "LS6AG_MANUAL_PUBLISH_ACTUAL_PUBLISH_FINAL_EXECUTION_COMMAND_TEMPLATE_READY_NO_PUBLISH"
STATUS_READY = "LS6AG_MANUAL_PUBLISH_ACTUAL_PUBLISH_FINAL_EXECUTION_COMMAND_READY_NO_PUBLISH"
STATUS_NOT_READY = "LS6AG_MANUAL_PUBLISH_ACTUAL_PUBLISH_FINAL_EXECUTION_COMMAND_NOT_READY"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--policy", default="config/start_ls6ag_manual_publish_actual_publish_final_execution_command_policy.json")
    parser.add_argument("--template", default="exchange/human_review/start_ls6ag_manual_publish_actual_publish_final_execution_command.template.json")
    parser.add_argument("--command", default="exchange/human_review/start_ls6ag_manual_publish_actual_publish_final_execution_command.json")
    parser.add_argument("--ls6af-run-result", default="exchange/logs/start_ls6af_manual_publish_actual_publish_runner_final_gate_result.json")
    ...
    parser.add_argument("--output", default="exchange/logs/start_ls6ag_manual_publish_actual_publish_final_execution_command_ready_result.json")
    parser.add_argument("--report", default="reports/start_ls6ag_manual_publish_actual_publish_final_execution_command_ready_report.md")
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# LS-6AG Manual Publish Actual Publish Final Execution Command Report",
        "",
...
def get_ls6x_consumed(doc: dict[str, Any]) -> Any:
    return doc.get(
        ...
        safe_get(doc, "actual_publish_execution_gate", "actual_publish_execution_gate_consumed"),
    )
...
def get_ls6v_consumed(doc: dict[str, Any]) -> Any:
    return doc.get(
        ...
        safe_get(doc, "final_execution_command", "final_execution_command_consumed"),
    )
...
def validate_command_shape(doc: dict[str, Any], *, is_template: bool, errors: list[str]) -> None:
    if is_template:
        req(doc.get("document_type") == "MANUAL_PUBLISH_ACTUAL_PUBLISH_FINAL_EXECUTION_COMMAND_TEMPLATE", "template document_type mismatch", errors)
        req(doc.get("command_status") == "TEMPLATE_NOT_COMMANDED", "template command_status mismatch", errors)
    else:
        req(doc.get("document_type") == "MANUAL_PUBLISH_ACTUAL_PUBLISH_FINAL_EXECUTION_COMMAND", "command document_type mismatch", errors)
        req(doc.get("command_status") == "ACTUAL_PUBLISH_FINAL_EXECUTION_COMMAND_RECORDED_NO_PUBLISH_EXECUTION", "command command_status mismatch", errors)

    target = doc.get("target_post", {})
    cmd = doc.get("actual_publish_final_execution_command", {})
    ...
    req(cmd.get("required_actual_publish_final_execution_command_label") == "FINAL_COMMAND_FOR_ACTUAL_PUBLISH_EXECUTION_RUNNER_GATE_ONLY", "required command label mismatch", errors)
    if is_template:
        req(cmd.get("actual_publish_final_execution_command_label") == "", "template command label must be empty", errors)
    else:
        req(cmd.get("actual_publish_final_execution_command_label") == "FINAL_COMMAND_FOR_ACTUAL_PUBLISH_EXECUTION_RUNNER_GATE_ONLY", "actual command label mismatch", errors)
        req(cmd.get("requires_next_phase") == "LS-6AH", "command.requires_next_phase mismatch", errors)

    req(cmd.get("actual_publish_final_execution_command_consumed") is False, "actual_publish_final_execution_command_consumed must be false", errors)
    req(cmd.get("actual_publish_runner_final_gate_ready") is True, "actual_publish_runner_final_gate_ready must be true", errors)
    req(cmd.get("actual_publish_runner_final_gate_consumed") is False, "actual_publish_runner_final_gate_consumed must be false", errors)
    req(cmd.get("actual_publish_final_preflight_ready") is True, "actual_publish_final_preflight_ready must be true", errors)
    req(cmd.get("actual_publish_final_preflight_consumed") is False, "actual_publish_final_preflight_consumed must be false", errors)
    req(cmd.get("actual_publish_execution_boundary_ready") is True, "actual_publish_execution_boundary_ready must be true", errors)
    req(cmd.get("actual_publish_execution_boundary_consumed") is False, "actual_publish_execution_boundary_consumed must be false", errors)
    req(cmd.get("actual_publish_execute_now_final_confirmation_consumed") is False, "actual_publish_execute_now_final_confirmation_consumed must be false", errors)
    req(cmd.get("explicit_execute_now_for_actual_publish_required") is True, "explicit_execute_now_for_actual_publish_required must be true", errors)
    req(cmd.get("explicit_execute_now_for_actual_publish_received") is True, "explicit_execute_now_for_actual_publish_received must be true", errors)
    req(cmd.get("explicit_execute_now_for_actual_publish_consumed") is False, "explicit_execute_now_for_actual_publish_consumed must be false", errors)
    req(cmd.get("actual_publish_execution_runner_ready") is True, "actual_publish_execution_runner_ready must be true", errors)
    req(cmd.get("actual_publish_execution_runner_executed") is False, "actual_publish_execution_runner_executed must be false", errors)
    req(cmd.get("actual_publish_execution_runner_blocked") is True, "actual_publish_execution_runner_blocked must be true", errors)
    req(cmd.get("actual_publish_execution_allowed_by_this_phase") is False, "actual_publish_execution_allowed_by_this_phase must be false", errors)
    req(cmd.get("actual_runner_execution_allowed_by_this_phase") is False, "actual_runner_execution_allowed_by_this_phase must be false", errors)
    req(cmd.get("manual_publish_allowed_by_this_phase") is False, "manual_publish_allowed_by_this_phase must be false", errors)
    req(cmd.get("manual_publish_execution_allowed_by_this_phase") is False, "manual_publish_execution_allowed_by_this_phase must be false", errors)
    req(cmd.get("manual_publish_executed") is False, "manual_publish_executed must be false", errors)
...
def build_result(status: str, command_doc: dict[str, Any], ls6af_run: dict[str, Any], errors: list[str]) -> dict[str, Any]:
    target = command_doc.get("target_post", {})
    cmd = command_doc.get("actual_publish_final_execution_command", {})
    upstream = command_doc.get("upstream_consumption_state", {})
    current = command_doc.get("current_phase_execution", {})
    ...
    return {
        ...
        "returned_post_status": ls6af_run.get("returned_post_status", "draft"),
        "ls6af_runner_final_gate_validated": True,
        "ls6ae_final_preflight_validated": True,
        "ls6ae_draft_verified": True,
        "actual_publish_final_execution_command_recorded": command_doc.get("command_status") == "ACTUAL_PUBLISH_FINAL_EXECUTION_COMMAND_RECORDED_NO_PUBLISH_EXECUTION",
        "command_status": command_doc.get("command_status", ""),
        "actual_publish_final_execution_command_label": cmd.get("actual_publish_final_execution_command_label", ""),
        "actual_publish_final_execution_command_consumed": bool(cmd.get("actual_publish_final_execution_command_consumed", False)),
        "actual_publish_runner_final_gate_ready": bool(cmd.get("actual_publish_runner_final_gate_ready", False)),
        "actual_publish_runner_final_gate_consumed": bool(cmd.get("actual_publish_runner_final_gate_consumed", False)),
        "actual_publish_final_preflight_ready": bool(cmd.get("actual_publish_final_preflight_ready", False)),
        "actual_publish_final_preflight_consumed": bool(cmd.get("actual_publish_final_preflight_consumed", False)),
        "actual_publish_execution_boundary_ready": bool(cmd.get("actual_publish_execution_boundary_ready", False)),
        "actual_publish_execution_boundary_consumed": bool(cmd.get("actual_publish_execution_boundary_consumed", False)),
        "actual_publish_execute_now_final_confirmation_label": ls6af_run.get("actual_publish_execute_now_final_confirmation_label", "CONFIRMED_FOR_ACTUAL_PUBLISH_EXECUTE_NOW_FINAL_GATE_ONLY"),
        "actual_publish_execute_now_final_confirmation_consumed": bool(cmd.get("actual_publish_execute_now_final_confirmation_consumed", False)),
        "explicit_execute_now_for_actual_publish_required": bool(cmd.get("explicit_execute_now_for_actual_publish_required", False)),
        "explicit_execute_now_for_actual_publish_received": bool(cmd.get("explicit_execute_now_for_actual_publish_received", False)),
        "explicit_execute_now_for_actual_publish_consumed": bool(cmd.get("explicit_execute_now_for_actual_publish_consumed", False)),
        "actual_publish_execution_runner_ready": bool(cmd.get("actual_publish_execution_runner_ready", False)),
        "actual_publish_execution_runner_executed": bool(cmd.get("actual_publish_execution_runner_executed", False)),
        "actual_publish_execution_runner_blocked": bool(cmd.get("actual_publish_execution_runner_blocked", False)),
        "final_explicit_publish_execution_command_label": ls6af_run.get("final_explicit_publish_execution_command_label", "FINAL_EXPLICIT_COMMAND_FOR_ACTUAL_PUBLISH_EXECUTION_GATE_ONLY"),
...
def validate_upstream(args: argparse.Namespace, errors: list[str]) -> tuple[dict[str, Any], dict[str, Any]]:
    policy = try_load_json(Path(args.policy), errors)
    ls6af_run = try_load_json(Path(args.ls6af_run_result), errors)
    ...
    req(policy.get("phase") == "LS-6AG", "policy.phase mismatch", errors)
    req(policy.get("execution_mode") == "ACTUAL_PUBLISH_FINAL_EXECUTION_COMMAND_GATE_ONLY_NO_PUBLISH", "policy.execution_mode mismatch", errors)
    req(policy.get("production_status") == "NO_PUBLISH", "policy.production_status mismatch", errors)

    req(ls6af_run.get("status") == "LS6AF_MANUAL_PUBLISH_ACTUAL_PUBLISH_RUNNER_FINAL_GATE_PASSED_NO_PUBLISH", "LS-6AF run status mismatch", errors)
    ...
    req(ls6af_runtime.get("status") == "MANUAL_PUBLISH_ACTUAL_PUBLISH_RUNNER_FINAL_GATE_READY_NO_PUBLISH", "LS-6AF runtime status mismatch", errors)
    req(ls6af_lock.get("status") == "MANUAL_PUBLISH_ACTUAL_PUBLISH_RUNNER_FINAL_GATE_LOCKED_NO_PUBLISH", "LS-6AF lock status mismatch", errors)
    req(ls6af_run.get("post_id") == 183, "LS-6AF post_id mismatch", errors)
    req(ls6af_run.get("returned_post_status") == "draft", "LS-6AF returned_post_status mismatch", errors)
    req(ls6af_run.get("actual_publish_runner_final_gate_ready") is True, "LS-6AF runner final gate ready must be true", errors)
    req(ls6af_run.get("actual_publish_runner_final_gate_consumed") is False, "LS-6AF runner final gate consumed must be false", errors)
    req(ls6af_runtime.get("actual_publish_runner_final_gate_consumed") is False, "LS-6AF runtime runner final gate consumed must be false", errors)
    req(ls6af_run.get("requires_actual_publish_final_execution_command") is True, "LS-6AF requires_actual_publish_final_execution_command must be true", errors)
    req(ls6af_run.get("publish_execution_still_blocked") is True, "LS-6AF publish_execution_still_blocked must be true", errors)
    req(safe_get(ls6af_run, "next_phase", "phase") == "LS-6AG", "LS-6AF next_phase mismatch", errors)
    ...
    return ls6af_run, policy


def main() -> int:
    args = parse_args()
    errors: list[str] = []
    ls6af_run, _ = validate_upstream(args, errors)

    template_doc = try_load_json(Path(args.template), errors)
    validate_command_shape(template_doc, is_template=True, errors=errors)

    selected_doc = template_doc
    status = STATUS_TEMPLATE_READY if not errors else STATUS_NOT_READY

    if not args.allow_template:
        command_doc = try_load_json(Path(args.command), errors)
        validate_command_shape(command_doc, is_template=False, errors=errors)
        selected_doc = command_doc
        status = STATUS_READY if not errors else STATUS_NOT_READY

    result = build_result(status, selected_doc, ls6af_run, errors)
