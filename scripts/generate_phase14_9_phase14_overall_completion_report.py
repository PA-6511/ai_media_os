#!/usr/bin/env python3
"""Generate Phase 14-9 report for overall Phase 14 completion."""
...
DEFAULT_OUTPUT_JSON = LOG_DIR / "phase14_9_phase14_overall_completion_report.json"
DEFAULT_OUTPUT_MD = LOG_DIR / "phase14_9_phase14_overall_completion_report.md"
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase14_9_phase14_overall_completion_report",
        "phase": "Phase 14-9",
        "status": "ABORT",
        "reason": reason,
...
def validate_inputs(reports: dict[str, dict]) -> str | None:
    ...
    for phase in ["14-1", "14-2", "14-3", "14-4", "14-5", "14-6", "14-7", "14-8"]:
        data = reports[phase]
...
def build_report(reports: dict[str, dict]) -> dict:
    return {
        "package_type": "phase14_9_phase14_overall_completion_report",
        "phase": "Phase 14-9",
        "title": "Phase 14-9 Phase 14 Overall Completion Report",
        "status": "PASS",
        "phase14_overall_status": "PASS",
        "phase_range": "Phase 14-1 through Phase 14-8",
        "current_decision": "KEEP_NO_GO",
        "publish_candidate_unlocked_for_operator": False,
        "wordpress_publish_execution": "NO_GO",
        "wordpress_write_executed": False,
        ...
        "summary": {
            "phase14_completed_without_execute_live": True,
            "phase14_no_go_boundary_maintained": True,
            "phase15_start_conditions_can_be_designed": True,
        },
        "next_step": "phase15_1_start_conditions_check_design",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 14-9 Phase 14 Overall Completion Report",
        "",
        f"Generated: {report['created_at']}",
        "",
        f"- status: {report['status']}",
        f"- phase14_overall_status: {report['phase14_overall_status']}",
...
def generate_report(logs_dir: Path, output_json: Path, output_md: Path) -> dict:
    ...
    return {
        "package_type": "phase14_9_phase14_overall_completion_report_result",
        "phase": "Phase 14-9",
        "status": report["status"],
        "phase14_overall_status": report["phase14_overall_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 14-9 overall completion report"
    )
