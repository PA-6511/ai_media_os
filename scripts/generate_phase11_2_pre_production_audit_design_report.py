#!/usr/bin/env python3
"""Generate Phase 11-2 report for pre-production audit design."""
...
DEFAULT_OUTPUT_JSON = LOG_DIR / "phase11_2_pre_production_audit_design_report.json"
DEFAULT_OUTPUT_MD = LOG_DIR / "phase11_2_pre_production_audit_design_report.md"
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase11_2_pre_production_audit_design_report",
        "phase": "Phase 11-2",
        "status": "ABORT",
        "reason": reason,
        "audit_design_report_generated": False,
...
def validate_phase11_1(source: dict) -> str | None:
    if source.get("phase") != "Phase 11-1":
        return "phase11_1 input phase must be Phase 11-1"
    if source.get("status") != "PASS":
        return "phase11_1 status must be PASS"
    if source.get("phase11_1_design_status") != "PASS":
        return "phase11_1_design_status must be PASS"
...
def build_checks(source: dict) -> list[dict]:
    return [
        ...
        {
            "check": "phase11_1_design_status=PASS",
            "passed": source.get("phase11_1_design_status") == "PASS",
            "actual": source.get("phase11_1_design_status"),
        },
        {
...
def build_report(source: dict, input_path: Path) -> dict:
    ...
    audit_scope = {
        "phase11_2_name": "pre_production_audit_design",
        "execution_type": "design_only",
        "publish_execution_in_phase11_2": "NO_GO",
        "current_decision": "KEEP_NO_GO",
        "publish_candidate_unlocked_for_operator": False,
        "wordpress_write_executed": False,
    ...
    audit_checklist = [
        "phase10_16_and_phase11_1_reports_are_pass",
        "no_go_boundary_is_unchanged",
        "publish_candidate_is_still_locked",
        "wordpress_draft_id_is_fixed_to_110",
        "no_secret_or_env_modification",
    ...
    return {
        "package_type": "phase11_2_pre_production_audit_design_report",
        "phase": "Phase 11-2",
        "title": "Phase 11-2 Pre Production Audit Design Report",
        "status": "PASS" if all_passed else "ABORT",
        "phase11_2_audit_design_status": "PASS" if all_passed else "ABORT",
        ...
        "summary": {
            "phase11_1_input_is_valid": True,
            "phase11_2_keeps_no_go_boundary": True,
            "publish_still_not_executed": True,
            "design_scope_only": True,
        },
        "next_step": "phase11_3_single_publish_final_execution_plan",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 11-2 Pre Production Audit Design Report",
        "",
        ...
        f"- phase11_2_audit_design_status: {report['phase11_2_audit_design_status']}",
        "",
        "## Audit Scope",
        "",
    ]

    for key, value in report["audit_scope"].items():
    ...
    for item in report["audit_checklist"]:
        lines.append(f"- {item}")
...
def generate_report(input_11_1: Path, output_json: Path, output_md: Path) -> dict:
    if not input_11_1.exists():
        return abort_result(f"required input not found: {input_11_1}", output_json)

    source = load_json(input_11_1)
    err = validate_phase11_1(source)
    if err:
        return abort_result(err, output_json)

    report = build_report(source, input_11_1)
    write_json(output_json, report)
    output_md.parent.mkdir(parents=True, exist_ok=True)
    output_md.write_text(build_markdown(report), encoding="utf-8")

    return {
        "package_type": "phase11_2_pre_production_audit_design_report_result",
        "phase": "Phase 11-2",
        "status": report["status"],
        "phase11_2_audit_design_status": report["phase11_2_audit_design_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 11-2 pre-production audit design report"
    )
    parser.add_argument("--input-11-1", type=Path, default=DEFAULT_INPUT_11_1)
    parser.add_argument("--output-json", type=Path, default=DEFAULT_OUTPUT_JSON)
    parser.add_argument("--output-md", type=Path, default=DEFAULT_OUTPUT_MD)
    args = parser.parse_args()

    result = generate_report(args.input_11_1, args.output_json, args.output_md)
