#!/usr/bin/env python3
"""Validate SVC-7 unit/timer removal rollback design (design-only)."""
...
DEFAULT_POLICY = ROOT / "config/phase8_svc_7_unit_timer_removal_rollback_policy.json"
DEFAULT_RUNBOOK = ROOT / "docs/runbooks/phase8_svc_7_unit_timer_removal_rollback_design.md"
DEFAULT_OUTPUT = ROOT / "exchange/logs/phase8_svc_7_unit_timer_removal_rollback_design_result.json"
...
def validate_unit_timer_removal_rollback_design(
    ...
    # Design document can include forbidden strings as prohibition bullets.
    # Treat as ABORT only if executable command line is present.
    for pattern in policy.get("forbidden_patterns", []):
        for line in runbook_text.splitlines():
            ...
            if pattern.lower() in normalized and not normalized.startswith("-"):
                if normalized.startswith("systemctl ") or normalized.startswith("rm ") or normalized.startswith("curl ") or normalized.startswith("requests."):
    ...
    if forbidden_hits:
        ...
        status = "PASS_DESIGN_ONLY_REMOVAL_ROLLBACK_PREPARED"
...
def _build_result(
    ...
    return {
        "phase": "8-SVC-7",
        "phase_name": "Unit/Timer Removal Rollback Design",
        "status": status,
        ...
        "errors": errors,
        "warnings": warnings,
        "allowed_next_step": policy.get("allowed_next_step", "Controlled removal execution checkpoint"),
        "checked_at": _now_iso(),
    }
...
def main() -> int:
    result = validate_unit_timer_removal_rollback_design()
    print(json.dumps(result, ensure_ascii=False, indent=2))
    return 0 if result.get("status") == "PASS_DESIGN_ONLY_REMOVAL_ROLLBACK_PREPARED" else 2
