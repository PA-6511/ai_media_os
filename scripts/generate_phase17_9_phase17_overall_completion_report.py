#!/usr/bin/env python3
"""Generate Phase 17-9 report for overall Phase 17 completion."""
...
DEFAULT_OUTPUT_JSON = LOG_DIR / "phase17_9_phase17_overall_completion_report.json"
DEFAULT_OUTPUT_MD = LOG_DIR / "phase17_9_phase17_overall_completion_report.md"
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase17_9_phase17_overall_completion_report",
        "phase": "Phase 17-9",
        "status": "ABORT",
        "reason": reason,
...
def validate_inputs(reports: dict[str, dict]) -> str | None:
    ...
    for phase in ["17-1", "17-2", "17-3", "17-4", "17-5", "17-6", "17-7", "17-8"]:
        data = reports[phase]
...
def build_report(reports: dict[str, dict]) -> dict:
    return {
        "package_type": "phase17_9_phase17_overall_completion_report",
        "phase": "Phase 17-9",
        "title": "Phase 17-9 Phase 17 Overall Completion Report",
        "status": "PASS",
        "phase17_overall_status": "PASS",
        "phase_range": "Phase 17-1 through Phase 17-8",
        "current_decision": "KEEP_NO_GO",
        "publish_candidate_unlocked_for_operator": False,
        "wordpress_publish_execution": "NO_GO",
        "wordpress_write_executed": False,
        ...
        "summary": {
            "phase17_completed_without_execute_live": True,
            "phase17_no_go_boundary_maintained": True,
            "phase18_start_conditions_can_be_designed": True,
        },
        "next_step": "phase18_1_start_conditions_check_design",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 17-9 Phase 17 Overall Completion Report",
        "",
        f"Generated: {report['created_at']}",
        "",
        f"- status: {report['status']}",
        f"- phase17_overall_status: {report['phase17_overall_status']}",
...
def generate_report(logs_dir: Path, output_json: Path, output_md: Path) -> dict:
    ...
    return {
        "package_type": "phase17_9_phase17_overall_completion_report_result",
        "phase": "Phase 17-9",
        "status": report["status"],
        "phase17_overall_status": report["phase17_overall_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 17-9 overall completion report"
    )
