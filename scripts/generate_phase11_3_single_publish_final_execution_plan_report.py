#!/usr/bin/env python3
"""Generate Phase 11-3 report for single publish final execution plan."""
...
DEFAULT_INPUT_11_2 = LOG_DIR / "phase11_2_pre_production_audit_design_report.json"
DEFAULT_OUTPUT_JSON = (
    LOG_DIR / "phase11_3_single_publish_final_execution_plan_report.json"
)
DEFAULT_OUTPUT_MD = (
    LOG_DIR / "phase11_3_single_publish_final_execution_plan_report.md"
)
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase11_3_single_publish_final_execution_plan_report",
        "phase": "Phase 11-3",
        "status": "ABORT",
        "reason": reason,
        "plan_report_generated": False,
...
def validate_phase11_2(source: dict) -> str | None:
    if source.get("phase") != "Phase 11-2":
        return "phase11_2 input phase must be Phase 11-2"
    if source.get("status") != "PASS":
        return "phase11_2 status must be PASS"
    if source.get("phase11_2_audit_design_status") != "PASS":
        return "phase11_2_audit_design_status must be PASS"
...
def build_checks(source: dict) -> list[dict]:
    return [
        ...
        {
            "check": "phase11_2_audit_design_status=PASS",
            "passed": source.get("phase11_2_audit_design_status") == "PASS",
            "actual": source.get("phase11_2_audit_design_status"),
        },
        {
...
def build_report(source: dict, input_path: Path) -> dict:
    ...
    execution_plan = {
        "plan_name": "single_publish_final_execution_plan",
        "execution_mode": "plan_only",
        "target_count": 1,
        "target_draft_id": source.get("wordpress_draft_id"),
        "target_draft_status": "draft",
        "publish_execution_in_phase11_3": "NO_GO",
        "current_decision": "KEEP_NO_GO",
    }

    readiness_items = [
        "single_target_draft_is_fixed",
        "pre_production_audit_design_completed",
        "human_go_or_no_go_gate_pending",
        "final_dry_run_confirmation_pending",
    ]

    return {
        "package_type": "phase11_3_single_publish_final_execution_plan_report",
        "phase": "Phase 11-3",
        "title": "Phase 11-3 Single Publish Final Execution Plan Report",
        "status": "PASS" if all_passed else "ABORT",
        "phase11_3_plan_status": "PASS" if all_passed else "ABORT",
        ...
        "summary": {
            "single_publish_plan_designed": True,
            "publish_still_not_executed": True,
            "no_go_boundary_maintained": True,
        },
        "next_step": "phase11_4_pre_publish_final_no_go_or_go_judgment_design",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 11-3 Single Publish Final Execution Plan Report",
        "",
        ...
        f"- phase11_3_plan_status: {report['phase11_3_plan_status']}",
        "",
        "## Execution Plan",
        "",
    ]

    for key, value in report["execution_plan"].items():
...
def generate_report(input_11_2: Path, output_json: Path, output_md: Path) -> dict:
    if not input_11_2.exists():
        return abort_result(f"required input not found: {input_11_2}", output_json)

    source = load_json(input_11_2)
    err = validate_phase11_2(source)
    if err:
        return abort_result(err, output_json)

    report = build_report(source, input_11_2)
    write_json(output_json, report)
    output_md.parent.mkdir(parents=True, exist_ok=True)
    output_md.write_text(build_markdown(report), encoding="utf-8")

    return {
        "package_type": "phase11_3_single_publish_final_execution_plan_report_result",
        "phase": "Phase 11-3",
        "status": report["status"],
        "phase11_3_plan_status": report["phase11_3_plan_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate Phase 11-3 single publish final execution plan report"
    )
    parser.add_argument("--input-11-2", type=Path, default=DEFAULT_INPUT_11_2)
    parser.add_argument("--output-json", type=Path, default=DEFAULT_OUTPUT_JSON)
    parser.add_argument("--output-md", type=Path, default=DEFAULT_OUTPUT_MD)
    args = parser.parse_args()

    result = generate_report(args.input_11_2, args.output_json, args.output_md)
