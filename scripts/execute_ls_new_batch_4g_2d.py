...
RESULT_PATH = (
    ROOT
    / "exchange/logs/"
    "ls_new_batch_4g_2d_result.json"
)
REPORT_PATH = (
    ROOT
    / "reports/"
    "ls_new_batch_4g_2d_one_shot_actual_candidate_search_report.md"
)
...
def validate_policy(
    ...
    require(
        ...
        == (
            ...
            "CATEGORY_CANDIDATE_SEARCH_POLICY_V1"
        ),
    ...
    candidates = policy["candidate_contract"]

    require(
        candidates["minimum_candidate_count"] == 1,
        "minimum candidate count mismatch",
    )
    require(
        candidates["maximum_candidate_count"] == 100,
        "maximum candidate count mismatch",
    )
    require(
        candidates[
        ...
        is False,
        "automatic candidate selection must be forbidden",
    )
    require(
        candidates[
        ...
        is False,
        "automatic category mapping must be forbidden",
    )
    require(
        candidates["human_candidate_review_required"]
        is True,
        "human candidate review must be required",
    )
    checks.append("candidate_contract")
...
def validate_source(
    ...
    require(
        ...
        is False,
        "source must not have selected a candidate",
    )
    require(
        source_result.get("category_mapping_fixed")
        is False,
        "source must not have fixed a mapping",
    )
    require(
    ...
    expected_package_digest = source_result[
    ...
    require(
        scope["automatic_candidate_selection_allowed"]
        is False,
        "source automatic selection boundary mismatch",
    )
    require(
        scope["automatic_category_mapping_allowed"]
        is False,
        "source automatic mapping boundary mismatch",
    )
    require(
    ...
    return [
        ...
        "source_candidate_search_scope_verified",
...
def validate_approval(
    ...
    require(
        approval.get(
            "source_candidate_search_scope_digest_sha256"
        )
        == source_result[
...
def reserve_one_shot(
    ...
    *,
    ...
    reservation = {
        ...
        "phase_id": "LS-NEW-BATCH-4G-2D",
        "lock_id": "ls-new-batch-4g-2d-one-shot",
    ...
    try:
        ...
        raise ControlledBlock(
            ...
            reason=(
                "The candidate-search one-shot lock already "
    ...
    *,
    ...
    try:
        ...
        if isinstance(reason, ssl.SSLError):
            status_code = (
                "BLOCKED_WORDPRESS_TLS_FAILURE"
            )
        elif isinstance(reason, socket.timeout):
            status_code = (
                "BLOCKED_WORDPRESS_READ_TIMEOUT"
            )
        elif isinstance(reason, socket.gaierror):
            status_code = (
                "BLOCKED_WORDPRESS_DNS_FAILURE"
            )
        else:
            status_code = (
...
def validate_response(
    ...
    if (
        candidate_count
        < contract["minimum_candidate_count"]
    ):
        raise ControlledBlock(
            status=(
            ...
            reason=(
                "WordPress returned no category candidates "
                "for the approved search term."
            ),
            telemetry={
                "http_status": status_code,
                "http_response_received": True,
    ...
    if (
        candidate_count
        > contract["maximum_candidate_count"]
    ):
        raise ControlledBlock(
            status=(
                "BLOCKED_WORDPRESS_CATEGORY_"
            ...
            reason=(
                "WordPress returned more candidates "
    ...
    for index, item in enumerate(payload):
        if not isinstance(item, dict):
            raise ControlledBlock(
                ...
                reason=(
                    "A WordPress category candidate "
                    "is not an object."
                ),
                telemetry={
                    "http_status": status_code,
        ...
        if category_id in seen_ids:
            raise ControlledBlock(
                ...
                reason=(
                    "WordPress returned a duplicate "
        ...
        candidate_without_digest = {
            ...
            "count": count,
            "exact_search_term_name_match": (
                normalized_name
                == contract["search_term"]
            ),
    ...
    list_without_digest = {
        "search_term": contract["search_term"],
    ...
    candidate_list = copy.deepcopy(
        list_without_digest
    )
    candidate_list[
        "candidate_list_digest_sha256"
    ] = canonical_digest(
        list_without_digest
    )
...
def build_report(
    ...
    candidate_list = result.get(
        "category_candidate_list"
    )

    candidate_lines = (
        "- Candidate list: `not recorded`\n"
    )
    ...
    return f"""# LS-NEW-BATCH-4G-2D One-Shot Actual Category Candidate Search
...
- Automatic candidate selection allowed: `false`
- Automatic category mapping allowed: `false`
- WordPress category creation allowed: `false`
...
def execute(
    *,
    ...
    try:
        ...
        candidate_list = validate_response(
            response,
            policy,
        )
        ...
        result.update(
            {
                ...
                "decision": (
                    "CATEGORY_CANDIDATE_LIST_RECORDED_"
                    "HUMAN_REVIEW_REQUIRED"
                ),
                "candidate_list_received": True,
                ...
                "category_mapping_fixed": False,
                "human_candidate_review_required": True,
                "ready_for_ls_new_batch_4g_2e": True,
                "ready_for_candidate_human_review": True,
        ...
        result["verified_checks"].extend(
            [
                ...
                "candidate_schema_verified",
                "candidate_ids_verified",
                "candidate_slugs_verified",
                "candidate_names_verified",
                "candidate_counts_verified",
                "candidate_id_uniqueness_verified",
                "candidate_slug_uniqueness_verified",
                "candidate_digests_generated",
                "candidate_list_digest_generated",
                "automatic_candidate_selection_absent",
        ...
        result.update(
            {
                ...
                "decision": (
                    "ONE_SHOT_CANDIDATE_SEARCH_BLOCKED_"
                    "APPROVAL_CONSUMED_NO_RETRY"
                ),
                "reason": blocked.reason,
                "ready_for_ls_new_batch_4g_2e": False,
