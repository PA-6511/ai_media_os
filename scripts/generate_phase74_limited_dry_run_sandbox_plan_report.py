...
DEFAULT_INPUT = ROOT / "exchange" / "logs" / "phase73_preparation_lock_report.json"
DEFAULT_OUTPUT = ROOT / "exchange" / "logs" / "phase74_limited_dry_run_sandbox_plan_report.json"
...
def abort_result(reason: str) -> dict:
    return {
        "package_type": "phase74_limited_dry_run_sandbox_plan_report_result",
        "phase": "Phase 74",
        "status": "ABORT",
        "reason": reason,
...
def validate_phase73_report(data: dict) -> dict | None:
    if data.get("phase") != "Phase 73":
        return abort_result("input phase must be 'Phase 73'")
    if data.get("package_type") != "phase73_preparation_lock_report":
        return abort_result("input package_type mismatch")
    if data.get("status") != "PASS":
        return abort_result("input status must be PASS")
    if data.get("lock_status") != "PASS":
        return abort_result("input lock_status must be PASS")
    ...
    lock_package = data.get("lock_package", {})
    if lock_package.get("does_not_execute") is not True:
        return abort_result("lock_package.does_not_execute must be true")

    controls = data.get("lock_controls", {})
    if controls.get("does_not_execute") is not True:
        return abort_result("lock_controls.does_not_execute must be true")

    evidence_checks = data.get("lock_evidence_checks", {})
    if evidence_checks.get("does_not_execute") is not True:
        return abort_result("lock_evidence_checks.does_not_execute must be true")
    ...
    gate = data.get("manual_lock_gate", {})
    if gate.get("does_not_execute") is not True:
        return abort_result("manual_lock_gate.does_not_execute must be true")
    if "ALLOW_PHASE74_PLANNING_ONLY" not in gate.get("allowed_decisions", []):
        return abort_result("manual_lock_gate.allowed_decisions missing ALLOW_PHASE74_PLANNING_ONLY")

    readiness = data.get("phase74_readiness", {})
    if readiness.get("readiness_status") != "READY_FOR_PHASE74_PLANNING_ONLY":
        return abort_result("phase74_readiness.readiness_status must be READY_FOR_PHASE74_PLANNING_ONLY")
    if readiness.get("human_approval_required") is not True:
        return abort_result("phase74_readiness.human_approval_required must be true")
    if readiness.get("can_execute") is not False:
        return abort_result("phase74_readiness.can_execute must be false")
    if readiness.get("execute_allowed") is not False:
        return abort_result("phase74_readiness.execute_allowed must be false")

    return None


def build_report(source: dict, input_path: Path) -> dict:
    evidence_checks = source["lock_evidence_checks"]

    checks = [
        {
            "check": "input_phase=Phase 73",
            "passed": source.get("phase") == "Phase 73",
            "actual": source.get("phase"),
        },
        {
            "check": "input_package_type=phase73_preparation_lock_report",
            "passed": source.get("package_type") == "phase73_preparation_lock_report",
        ...
        {
            "check": "sandbox_plan_controls_does_not_execute=true",
            "passed": source.get("lock_controls", {}).get("does_not_execute") is True,
            "actual": source.get("lock_controls", {}).get("does_not_execute"),
        },
        {
            "check": "sandbox_plan_evidence_checks_does_not_execute=true",
            "passed": evidence_checks.get("does_not_execute") is True,
            "actual": evidence_checks.get("does_not_execute"),
        },
        {
            "check": "manual_sandbox_plan_gate_does_not_execute=true",
            "passed": source.get("manual_lock_gate", {}).get("does_not_execute") is True,
            "actual": source.get("manual_lock_gate", {}).get("does_not_execute"),
        },
        {
            "check": "phase74_readiness_status=READY_FOR_PHASE74_PLANNING_ONLY",
            "passed": source.get("phase74_readiness", {}).get("readiness_status") == "READY_FOR_PHASE74_PLANNING_ONLY",
            "actual": source.get("phase74_readiness", {}).get("readiness_status"),
        },
    ]
    ...
    return {
        ...
        "sandbox_plan_status": "PASS" if all_checks_passed else "ABORT",
        ...
        "sandbox_plan_package": {
            ...
            "status": "LIMITED_DRY_RUN_SANDBOX_PLAN_PACKAGE_READY" if all_checks_passed else "ABORT",
        },
        "sandbox_plan_controls": {
            ...
            "single_file_scope_required": True,
            "does_not_execute": True,
            "status": "LIMITED_DRY_RUN_SANDBOX_PLAN_CONTROLS_READY" if all_checks_passed else "ABORT",
        },
        "sandbox_plan_evidence_checks": {
            ...
            "does_not_execute": True,
            "status": "LIMITED_DRY_RUN_SANDBOX_PLAN_EVIDENCE_CHECKS_READY" if all_checks_passed else "ABORT",
        },
        "manual_sandbox_plan_gate": {
            ...
            "does_not_execute": True,
            "status": "MANUAL_LIMITED_DRY_RUN_SANDBOX_PLAN_GATE_READY" if all_checks_passed else "ABORT",
        },
        "phase75_readiness": {
            "readiness_status": "READY_FOR_PHASE75_PLANNING_ONLY" if all_checks_passed else "NOT_READY",
...
def generate_phase74_limited_dry_run_sandbox_plan_report(
    ...
    validation_error = validate_phase73_report(source)
    if validation_error:
    ...
    return {
        ...
        "sandbox_plan_status": report["sandbox_plan_status"],
        "report_generated": report["status"] == "PASS",
        "json_output": str(output_path),
        "production_status": "NO_GO",
        "phase75_readiness": report["phase75_readiness"]["readiness_status"],
        "created_at": datetime.now(timezone.utc).isoformat(),
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Phase74 limited dry-run sandbox plan report")
    parser.add_argument("--input", type=Path, default=DEFAULT_INPUT, help="Input Phase73 report path")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT, help="Output Phase74 report path")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing output")
    args = parser.parse_args()

    result = generate_phase74_limited_dry_run_sandbox_plan_report(args.input, args.output, overwrite=args.overwrite)
