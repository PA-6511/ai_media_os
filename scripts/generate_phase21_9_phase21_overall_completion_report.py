#!/usr/bin/env python3
"""Generate Phase 21-9 report for overall Phase 21 completion."""
...
DEFAULT_OUTPUT_JSON = LOG_DIR / "phase21_9_phase21_overall_completion_report.json"
DEFAULT_OUTPUT_MD = LOG_DIR / "phase21_9_phase21_overall_completion_report.md"
...
def abort_result(reason: str, output_json: Path) -> dict:
    result = {
        "package_type": "phase21_9_phase21_overall_completion_report",
        "phase": "Phase 21-9",
        "status": "ABORT",
        "reason": reason,
...
def validate_inputs(reports: dict[str, dict]) -> str | None:
    ...
    for phase in ["21-1", "21-2", "21-3", "21-4", "21-5", "21-6", "21-7", "21-8"]:
        data = reports[phase]
...
def build_report(reports: dict[str, dict]) -> dict:
    return {
        "package_type": "phase21_9_phase21_overall_completion_report",
        "phase": "Phase 21-9",
        "title": "Phase 21-9 Phase 21 Overall Completion Report",
        "status": "PASS",
        "phase21_overall_status": "PASS",
        "phase_range": "Phase 21-1 through Phase 21-8",
        "current_decision": "KEEP_NO_GO",
        "publish_candidate_unlocked_for_operator": False,
        "wordpress_publish_execution": "NO_GO",
        "wordpress_write_executed": False,
        ...
        "summary": {
            "phase21_completed_without_execute_live": True,
            "phase21_no_go_boundary_maintained": True,
            "phase22_start_conditions_can_be_designed": True,
        },
        "next_step": "phase22_1_start_conditions_check_design",
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def build_markdown(report: dict) -> str:
    lines = [
        "# Phase 21-9 Phase 21 Overall Completion Report",
        "",
        f"Generated: {report['created_at']}",
        "",
        f"- status: {report['status']}",
        f"- phase21_overall_status: {report['phase21_overall_status']}",
...
def generate_report(logs_dir: Path, output_json: Path, output_md: Path) -> dict:
    ...
    return {
        "package_type": "phase21_9_phase21_overall_completion_report_result",
        "phase": "Phase 21-9",
        "status": report["status"],
        "phase21_overall_status": report["phase21_overall_status"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Phase 21-9 overall completion report")
