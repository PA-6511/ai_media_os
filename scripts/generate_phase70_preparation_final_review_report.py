...
DEFAULT_INPUT = ROOT / "exchange" / "logs" / "phase69_preparation_approval_report.json"
DEFAULT_OUTPUT = ROOT / "exchange" / "logs" / "phase70_preparation_final_review_report.json"
...
def abort_result(reason: str) -> dict:
    return {
        "package_type": "phase70_preparation_final_review_report_result",
        "phase": "Phase 70",
        "status": "ABORT",
        "reason": reason,
...
def validate_phase69_report(data: dict) -> dict | None:
    if data.get("phase") != "Phase 69":
        return abort_result("input phase must be 'Phase 69'")
    if data.get("package_type") != "phase69_preparation_approval_report":
        return abort_result("input package_type mismatch")
    if data.get("status") != "PASS":
        return abort_result("input status must be PASS")
    if data.get("approval_status") != "PASS":
        return abort_result("input approval_status must be PASS")
    ...
    approval_package = data.get("approval_package", {})
    if approval_package.get("does_not_execute") is not True:
        return abort_result("approval_package.does_not_execute must be true")

    controls = data.get("approval_controls", {})
    if controls.get("does_not_execute") is not True:
        return abort_result("approval_controls.does_not_execute must be true")

    evidence_checks = data.get("approval_evidence_checks", {})
    if evidence_checks.get("does_not_execute") is not True:
        return abort_result("approval_evidence_checks.does_not_execute must be true")
    ...
    gate = data.get("manual_approval_gate", {})
    if gate.get("does_not_execute") is not True:
        return abort_result("manual_approval_gate.does_not_execute must be true")
    if "ALLOW_PHASE70_PLANNING_ONLY" not in gate.get("allowed_decisions", []):
        return abort_result("manual_approval_gate.allowed_decisions missing ALLOW_PHASE70_PLANNING_ONLY")

    readiness = data.get("phase70_readiness", {})
    if readiness.get("readiness_status") != "READY_FOR_PHASE70_PLANNING_ONLY":
        return abort_result("phase70_readiness.readiness_status must be READY_FOR_PHASE70_PLANNING_ONLY")
    if readiness.get("human_approval_required") is not True:
        return abort_result("phase70_readiness.human_approval_required must be true")
    if readiness.get("can_execute") is not False:
        return abort_result("phase70_readiness.can_execute must be false")
    if readiness.get("execute_allowed") is not False:
        return abort_result("phase70_readiness.execute_allowed must be false")

    return None


def build_report(source: dict, input_path: Path) -> dict:
    evidence_checks = source["approval_evidence_checks"]

    checks = [
        {
            "check": "input_phase=Phase 69",
            "passed": source.get("phase") == "Phase 69",
            "actual": source.get("phase"),
        },
        {
            "check": "input_package_type=phase69_preparation_approval_report",
            "passed": source.get("package_type") == "phase69_preparation_approval_report",
        ...
        {
            "check": "final_review_controls_does_not_execute=true",
            "passed": source.get("approval_controls", {}).get("does_not_execute") is True,
            "actual": source.get("approval_controls", {}).get("does_not_execute"),
        },
        {
            "check": "final_review_evidence_checks_does_not_execute=true",
            "passed": evidence_checks.get("does_not_execute") is True,
            "actual": evidence_checks.get("does_not_execute"),
        },
        {
            "check": "manual_final_review_gate_does_not_execute=true",
            "passed": source.get("manual_approval_gate", {}).get("does_not_execute") is True,
            "actual": source.get("manual_approval_gate", {}).get("does_not_execute"),
        },
        {
            "check": "phase70_readiness_status=READY_FOR_PHASE70_PLANNING_ONLY",
            "passed": source.get("phase70_readiness", {}).get("readiness_status") == "READY_FOR_PHASE70_PLANNING_ONLY",
            "actual": source.get("phase70_readiness", {}).get("readiness_status"),
        },
    ]
    ...
    return {
        ...
        "final_review_status": "PASS" if all_checks_passed else "ABORT",
        ...
        "final_review_package": {
            ...
            "status": "PREPARATION_FINAL_REVIEW_PACKAGE_READY" if all_checks_passed else "ABORT",
        },
        "final_review_controls": {
            ...
            "single_file_scope_required": True,
            "does_not_execute": True,
            "status": "PREPARATION_FINAL_REVIEW_CONTROLS_READY" if all_checks_passed else "ABORT",
        },
        "final_review_evidence_checks": {
            ...
            "does_not_execute": True,
            "status": "PREPARATION_FINAL_REVIEW_EVIDENCE_CHECKS_READY" if all_checks_passed else "ABORT",
        },
        "manual_final_review_gate": {
            ...
            "does_not_execute": True,
            "status": "MANUAL_PREPARATION_FINAL_REVIEW_GATE_READY" if all_checks_passed else "ABORT",
        },
        "phase71_readiness": {
            "readiness_status": "READY_FOR_PHASE71_PLANNING_ONLY" if all_checks_passed else "NOT_READY",
...
def generate_phase70_preparation_final_review_report(
    ...
    validation_error = validate_phase69_report(source)
    if validation_error:
    ...
    return {
        ...
        "final_review_status": report["final_review_status"],
        "report_generated": report["status"] == "PASS",
        "json_output": str(output_path),
        "production_status": "NO_GO",
        "phase71_readiness": report["phase71_readiness"]["readiness_status"],
        "created_at": datetime.now(timezone.utc).isoformat(),
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Phase70 preparation final review report")
    parser.add_argument("--input", type=Path, default=DEFAULT_INPUT, help="Input Phase69 report path")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT, help="Output Phase70 report path")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing output")
    args = parser.parse_args()

    result = generate_phase70_preparation_final_review_report(args.input, args.output, overwrite=args.overwrite)
