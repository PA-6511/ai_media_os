...
def evaluate_phase63_policy(
    ...
    if controls.get("operator_signoff_controls_does_not_execute") is not True:
        reasons.append("controls.operator_signoff_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("operator_signoff_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.operator_signoff_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("operator_signoff_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.operator_signoff_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("operator_signoff_gate_does_not_execute") is not True:
        reasons.append("manual_gate.operator_signoff_gate_does_not_execute must be True")
