...
WEEKLY_REPORT_PATTERN = re.compile(r"^weekly_report_(\d{4}W\d{2})\.json$")
...
def load_latest_weekly_report(report_dir: Path = DEFAULT_REPORT_DIR) -> tuple[dict[str, Any], Path]:
    """最新の weekly_report_YYYYWww.json を読み込む。"""
    if not report_dir.exists() or not report_dir.is_dir():
        raise FileNotFoundError(f"レポートディレクトリが見つかりません: {report_dir}")

    dated_paths: list[tuple[str, Path]] = []
    for path in report_dir.glob("weekly_report_*.json"):
        matched = WEEKLY_REPORT_PATTERN.match(path.name)
        if matched:
            dated_paths.append((matched.group(1), path))

    if not dated_paths:
        raise FileNotFoundError(
            "weekly_report JSON が見つかりません。先に reporting.run_weekly_report を実行してください。"
        )
    ...
    try:
        ...
        raise ValueError(f"weekly_report JSON が不正です: {latest_path} ({exc})") from exc
    except OSError as exc:
        raise OSError(f"weekly_report の読み込みに失敗しました: {latest_path} ({exc})") from exc

    if not isinstance(payload, dict):
        raise ValueError(f"weekly_report の形式が不正です: {latest_path}")

    return payload, latest_path


def build_weekly_dashboard_model(report: dict[str, Any]) -> dict[str, Any]:
    """週次レポートから週次ダッシュボード表示モデルを構築する。"""
    counts = {
    ...
    return {
        "report_week": str(report.get("report_week", "unknown")),
...
def build_weekly_dashboard_html(model: dict[str, Any]) -> str:
    """週次ダッシュボード HTML を生成する。"""
    report_week = escape(str(model.get("report_week", "unknown")))
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    body {{
      ...
      color: var(--text);
      background: linear-gradient(135deg, #eff6ef, #fafcf8 42%, #eef4fa);
    }}
