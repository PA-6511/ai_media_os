...
def _build_weekly_report_path(report_week: str, output_dir: Path, report_format: str) -> Path:
    return output_dir / f"weekly_report_{report_week}.{report_format}"


def write_weekly_report_json(report: dict[str, Any], output_dir: Path | None = None) -> str:
    """週次レポート JSON を保存してパス文字列を返す。"""
    output = output_dir or DEFAULT_REPORT_DIR
    output.mkdir(parents=True, exist_ok=True)

    report_week = str(report.get("report_week", "unknown"))
    path = _build_weekly_report_path(report_week, output, "json")
...
def write_weekly_report_txt(report: dict[str, Any], output_dir: Path | None = None) -> str:
    """週次レポート TXT を保存してパス文字列を返す。"""
    output = output_dir or DEFAULT_REPORT_DIR
    output.mkdir(parents=True, exist_ok=True)

    report_week = str(report.get("report_week", "unknown"))
    path = _build_weekly_report_path(report_week, output, "txt")
