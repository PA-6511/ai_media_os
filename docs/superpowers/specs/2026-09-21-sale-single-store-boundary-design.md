...
- URL-kind-specific host checks for product, affiliate, and cover URLs
...
- any item normalizes to another store;
- more than one canonical item store exists;
...
The update payload contains `content` only. A readback must reconfirm the post ID, draft status, featured media, marker, store label, and absence of other-store items.
