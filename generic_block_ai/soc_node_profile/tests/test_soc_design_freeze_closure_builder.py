...
from generic_block_ai.soc_node_profile.runtime.soc_design_freeze_closure_builder import (
    build_soc_design_freeze_closure_summary,
    validate_soc14_phase_constraints,
    validate_soc_design_freeze_closure_summary_contract,
)
...
def _build_soc13_checklist() -> dict:
    ...
    return build_soc_periodic_reverification_checklist(retention, soc13_policy)


def test_soc_design_freeze_closure_summary_contract_pass() -> None:
    policy = _load_policy("soc_design_freeze_closure_contract_policy.json")
    checklist = _build_soc13_checklist()

    summary = build_soc_design_freeze_closure_summary(checklist, policy)
    validation = validate_soc_design_freeze_closure_summary_contract(summary, policy)

    assert summary["freeze_declared"] is True
    assert summary["closure_declared"] is True
    assert summary["closure_status"] == "CLOSED_DESIGN_ONLY"
    assert summary["freeze_reason"] == "DESIGN_LINE_CLOSED_REPORTS_ONLY"
    assert summary["reports_only"] is True
    assert summary["record_only"] is True
    assert validation["result"] == "PASS"


def test_soc_design_freeze_closure_contract_fails_on_unsafe_mutation() -> None:
    policy = _load_policy("soc_design_freeze_closure_contract_policy.json")
    checklist = _build_soc13_checklist()

    summary = build_soc_design_freeze_closure_summary(checklist, policy)
    summary["execution_allowed"] = True
...
def test_soc14_phase_constraints_remain_design_only_no_go() -> None:
    manifest = _load_manifest()
    policy = _load_policy("soc_design_freeze_closure_contract_policy.json")

    violations = validate_soc14_phase_constraints(manifest, policy)
    assert violations == []
