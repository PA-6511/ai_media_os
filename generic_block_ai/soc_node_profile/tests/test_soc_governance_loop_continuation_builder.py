...
from generic_block_ai.soc_node_profile.runtime.soc_governance_loop_continuation_builder import (
    build_soc_governance_loop_continuation,
    validate_soc16_phase_constraints,
    validate_soc_governance_loop_continuation_contract,
)
...
def _build_soc15_archive_manifest() -> dict:
    ...
    return build_soc_terminal_archive_manifest(closure, soc15_policy)


def test_soc_governance_loop_continuation_contract_pass() -> None:
    policy = _load_policy("soc_governance_loop_continuation_contract_policy.json")
    archive_manifest = _build_soc15_archive_manifest()

    package = build_soc_governance_loop_continuation(archive_manifest, policy)
    validation = validate_soc_governance_loop_continuation_contract(package, policy)

    assert package["loop_mode"] == "GOVERNANCE_CONTINUATION_REPORTS_ONLY"
    assert package["continuation_ready"] is True
    assert package["next_cycle_mode"] == "MANUAL_OR_SCHEDULED_REVIEW_ONLY"
    assert package["reverification_required"] is True
    assert package["execution_allowed"] is False
    assert package["production_status"] == "NO_GO"
    assert validation["result"] == "PASS"


def test_soc_governance_loop_continuation_contract_fails_on_unsafe_mutation() -> None:
    policy = _load_policy("soc_governance_loop_continuation_contract_policy.json")
    archive_manifest = _build_soc15_archive_manifest()
...
def test_soc16_phase_constraints_remain_design_only_no_go() -> None:
    manifest = _load_manifest()
    policy = _load_policy("soc_governance_loop_continuation_contract_policy.json")

    violations = validate_soc16_phase_constraints(manifest, policy)
    assert violations == []
