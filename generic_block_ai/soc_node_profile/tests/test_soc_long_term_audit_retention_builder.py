...
from generic_block_ai.soc_node_profile.runtime.soc_long_term_audit_retention_builder import (
    build_soc_long_term_audit_retention_package,
    validate_soc12_phase_constraints,
    validate_soc_long_term_audit_retention_package_contract,
)
...
def _build_soc11_gate() -> dict:
    ...
    return build_soc_release_readiness_design_gate(dossier, soc11_policy)


def test_soc_long_term_audit_retention_package_contract_pass() -> None:
    policy = _load_policy("soc_long_term_audit_retention_contract_policy.json")
    gate = _build_soc11_gate()

    package = build_soc_long_term_audit_retention_package(gate, policy)
    validation = validate_soc_long_term_audit_retention_package_contract(package, policy)

    assert package["retention_ready"] is True
    assert package["retention_mode"] == "REPORTS_ONLY"
    assert package["reverify_required"] is True
    assert package["production_status"] == "NO_GO"
    assert validation["result"] == "PASS"


def test_soc_long_term_retention_contract_fails_on_unsafe_mutation() -> None:
    policy = _load_policy("soc_long_term_audit_retention_contract_policy.json")
    gate = _build_soc11_gate()
...
def test_soc12_phase_constraints_remain_design_only_no_go() -> None:
    manifest = _load_manifest()
    policy = _load_policy("soc_long_term_audit_retention_contract_policy.json")

    violations = validate_soc12_phase_constraints(manifest, policy)
    assert violations == []
