...
from generic_block_ai.soc_node_profile.runtime.soc_release_readiness_design_gate import (
    build_soc_release_readiness_design_gate,
    validate_soc11_phase_constraints,
    validate_soc_release_readiness_design_gate_contract,
)
...
def _build_soc10_dossier() -> dict:
    ...
    return build_soc_final_design_dossier_index(
        node_id=node_id,
        request_id=request_id,
...
def test_soc_release_readiness_design_gate_contract_pass() -> None:
    policy = _load_policy("soc_release_readiness_design_gate_contract_policy.json")
    dossier = _build_soc10_dossier()

    gate = build_soc_release_readiness_design_gate(dossier, policy)
    validation = validate_soc_release_readiness_design_gate_contract(gate, policy)

    assert gate["release_ready"] is False
    assert gate["execution_allowed"] is False
    assert gate["external_dispatch_allowed"] is False
    assert gate["reason"] == "DESIGN_GATE_ONLY"
    assert validation["result"] == "PASS"


def test_soc_release_readiness_design_gate_contract_fails_on_unsafe_mutation() -> None:
    policy = _load_policy("soc_release_readiness_design_gate_contract_policy.json")
    dossier = _build_soc10_dossier()
...
def test_soc11_phase_constraints_remain_design_only_no_go() -> None:
    manifest = _load_manifest()
    policy = _load_policy("soc_release_readiness_design_gate_contract_policy.json")

    violations = validate_soc11_phase_constraints(manifest, policy)
    assert violations == []
