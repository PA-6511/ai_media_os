...
from generic_block_ai.soc_node_profile.runtime.soc_final_closure_audit_builder import (
    build_soc_final_closure_audit_declaration,
    validate_soc17_phase_constraints,
    validate_soc_final_closure_audit_declaration_contract,
)
...
def _build_soc16_governance_continuation() -> dict:
    ...
    return build_soc_governance_loop_continuation(terminal_archive, soc16_policy)


def test_soc_final_closure_audit_declaration_contract_pass() -> None:
    policy = _load_policy("soc_final_closure_audit_declaration_contract_policy.json")
    governance = _build_soc16_governance_continuation()

    declaration = build_soc_final_closure_audit_declaration(governance, policy)
    validation = validate_soc_final_closure_audit_declaration_contract(declaration, policy)

    assert declaration["audit_closure_declared"] is True
    assert declaration["closure_state"] == "AUDIT_CLOSED_NO_GO"
    assert declaration["declaration_reason"] == "FINAL_AUDIT_CLOSURE_REPORTS_RECORD_ONLY"
    assert declaration["reports_only"] is True
    assert declaration["record_only"] is True
    assert declaration["no_go_lock_confirmed"] is True
    assert declaration["production_status"] == "NO_GO"
    assert validation["result"] == "PASS"


def test_soc_final_closure_audit_declaration_contract_fails_on_unsafe_mutation() -> None:
    policy = _load_policy("soc_final_closure_audit_declaration_contract_policy.json")
    governance = _build_soc16_governance_continuation()

    declaration = build_soc_final_closure_audit_declaration(governance, policy)
    declaration["execution_allowed"] = True
...
def test_soc17_phase_constraints_remain_design_only_no_go() -> None:
    manifest = _load_manifest()
    policy = _load_policy("soc_final_closure_audit_declaration_contract_policy.json")

    violations = validate_soc17_phase_constraints(manifest, policy)
    assert violations == []
