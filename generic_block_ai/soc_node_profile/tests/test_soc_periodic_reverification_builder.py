...
from generic_block_ai.soc_node_profile.runtime.soc_periodic_reverification_builder import (
    build_soc_periodic_reverification_checklist,
    validate_soc13_phase_constraints,
    validate_soc_periodic_reverification_checklist_contract,
)
...
def _build_soc12_retention_package() -> dict:
    ...
    return build_soc_long_term_audit_retention_package(gate, soc12_policy)


def test_soc_periodic_reverification_checklist_contract_pass() -> None:
    policy = _load_policy("soc_periodic_reverification_checklist_contract_policy.json")
    retention_package = _build_soc12_retention_package()

    checklist = build_soc_periodic_reverification_checklist(retention_package, policy)
    validation = validate_soc_periodic_reverification_checklist_contract(checklist, policy)

    assert checklist["schedule_type"] == "PERIODIC_CHECKLIST_ONLY"
    assert checklist["reverify_required"] is True
    assert checklist["execution_allowed"] is False
    assert checklist["production_status"] == "NO_GO"
    assert checklist["next_reverification_trigger"] == "MANUAL_OR_SCHEDULED_REVIEW_ONLY"
    assert validation["result"] == "PASS"


def test_soc_periodic_reverification_contract_fails_on_unsafe_mutation() -> None:
    policy = _load_policy("soc_periodic_reverification_checklist_contract_policy.json")
    retention_package = _build_soc12_retention_package()

    checklist = build_soc_periodic_reverification_checklist(retention_package, policy)
    checklist["execution_allowed"] = True
...
def test_soc13_phase_constraints_remain_design_only_no_go() -> None:
    manifest = _load_manifest()
    policy = _load_policy("soc_periodic_reverification_checklist_contract_policy.json")

    violations = validate_soc13_phase_constraints(manifest, policy)
    assert violations == []
