...
from generic_block_ai.soc_node_profile.runtime.soc_final_dossier_index_builder import (
    build_soc_final_design_dossier_index,
    validate_soc10_phase_constraints,
    validate_soc_final_design_dossier_index_contract,
)
...
def _build_soc9_outputs() -> tuple[dict, dict, str, str]:
    ...
    return approval, seal, node_id, request_id


def test_soc_final_dossier_index_contract_pass() -> None:
    policy = _load_policy("soc_final_design_dossier_index_contract_policy.json")
    approval, seal, node_id, request_id = _build_soc9_outputs()

    index_doc = build_soc_final_design_dossier_index(
        node_id=node_id,
        request_id=request_id,
    ...
    validation = validate_soc_final_design_dossier_index_contract(index_doc, policy)

    assert index_doc["dossier_status"] == "COMPLETE"
    assert index_doc["integrity_summary"]["missing_keys"] == []
    assert validation["result"] == "PASS"


def test_soc_final_dossier_index_contract_fails_on_unsafe_mutation() -> None:
    policy = _load_policy("soc_final_design_dossier_index_contract_policy.json")
    approval, seal, node_id, request_id = _build_soc9_outputs()
    ...
    index_doc["actual_execution"] = True
...
def test_soc10_phase_constraints_remain_design_only_no_go() -> None:
    manifest = _load_manifest()
    policy = _load_policy("soc_final_design_dossier_index_contract_policy.json")

    violations = validate_soc10_phase_constraints(manifest, policy)
    assert violations == []
