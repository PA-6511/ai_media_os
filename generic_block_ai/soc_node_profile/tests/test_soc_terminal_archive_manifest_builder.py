...
from generic_block_ai.soc_node_profile.runtime.soc_terminal_archive_manifest_builder import (
    build_soc_terminal_archive_manifest,
    validate_soc15_phase_constraints,
    validate_soc_terminal_archive_manifest_contract,
)
...
def _build_soc14_summary() -> dict:
    ...
    return build_soc_design_freeze_closure_summary(checklist, soc14_policy)


def test_soc_terminal_archive_manifest_contract_pass() -> None:
    policy = _load_policy("soc_terminal_archive_manifest_contract_policy.json")
    summary = _build_soc14_summary()

    manifest_doc = build_soc_terminal_archive_manifest(summary, policy)
    validation = validate_soc_terminal_archive_manifest_contract(manifest_doc, policy)

    assert manifest_doc["archive_mode"] == "REPORTS_ONLY_TERMINAL_ARCHIVE"
    assert manifest_doc["archive_scope"] == "SOC_0_TO_SOC_14"
    assert manifest_doc["archive_sealed"] is True
    assert manifest_doc["record_only"] is True
    assert manifest_doc["reports_only"] is True
    assert manifest_doc["production_status"] == "NO_GO"
    assert validation["result"] == "PASS"


def test_soc_terminal_archive_manifest_contract_fails_on_unsafe_mutation() -> None:
    policy = _load_policy("soc_terminal_archive_manifest_contract_policy.json")
    summary = _build_soc14_summary()

    manifest_doc = build_soc_terminal_archive_manifest(summary, policy)
    manifest_doc["execution_allowed"] = True
...
def test_soc15_phase_constraints_remain_design_only_no_go() -> None:
    manifest = _load_manifest()
    policy = _load_policy("soc_terminal_archive_manifest_contract_policy.json")

    violations = validate_soc15_phase_constraints(manifest, policy)
    assert violations == []
