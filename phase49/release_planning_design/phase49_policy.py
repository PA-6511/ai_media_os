...
def evaluate_phase49_policy(
    ...
    if controls.get("release_planning_controls_does_not_execute") is not True:
        reasons.append("controls.release_planning_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("release_planning_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.release_planning_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("release_planning_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.release_planning_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("release_planning_gate_does_not_execute") is not True:
        reasons.append("manual_gate.release_planning_gate_does_not_execute must be True")
