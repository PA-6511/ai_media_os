...
def evaluate_phase60_policy(
    ...
    if controls.get("runbook_controls_does_not_execute") is not True:
        reasons.append("controls.runbook_controls_does_not_execute must be True")
    ...
    if stop_conditions.get("runbook_stop_conditions_does_not_execute") is not True:
        reasons.append("stop_conditions.runbook_stop_conditions_does_not_execute must be True")
    ...
    if evidence_requirements.get("runbook_evidence_does_not_execute") is not True:
        reasons.append("evidence_requirements.runbook_evidence_does_not_execute must be True")
    if manual_gate.get("gate_required") is not True:
        reasons.append("manual_gate.gate_required must be True")
    if manual_gate.get("runbook_gate_does_not_execute") is not True:
        reasons.append("manual_gate.runbook_gate_does_not_execute must be True")
