from phase41.certification_design.phase41_policy import evaluate_phase41_policy


def _base_certification_package() -> dict:
    return {
        "mode": "DRY_RUN",
...
def test_policy_fails_when_max_files_invalid() -> None:
    pkg = _base_certification_package()
    pkg["max_files_to_execute"] = 2
    result = evaluate_phase41_policy(
...
def test_policy_fails_when_controls_non_execute_false() -> None:
    ...
    controls["certification_controls_does_not_execute"] = False
    result = evaluate_phase41_policy(
        _base_certification_package(), controls, _base_stop_conditions(), _base_evidence(), _base_gate()
    )
    assert result["policy_status"] == "FAIL"
...
def test_policy_fails_when_stop_non_execute_false() -> None:
    ...
    stop["certification_stop_conditions_does_not_execute"] = False
    result = evaluate_phase41_policy(
        _base_certification_package(), _base_controls(), stop, _base_evidence(), _base_gate()
    )
    assert result["policy_status"] == "FAIL"
...
def test_policy_fails_when_evidence_non_execute_false() -> None:
    ...
    evidence["certification_evidence_does_not_execute"] = False
    result = evaluate_phase41_policy(
        _base_certification_package(), _base_controls(), _base_stop_conditions(), evidence, _base_gate()
    )
    assert result["policy_status"] == "FAIL"
...
def test_policy_fails_when_allow_does_not_execute_false() -> None:
    ...
    result = evaluate_phase41_policy(
        _base_certification_package(), _base_controls(), _base_stop_conditions(), _base_evidence(), gate
    )
    assert result["policy_status"] == "FAIL"
